...
本文件记录 `com.ruixue.unitysdk.minigame.weixin` 的版本变更。
...
- 微信小游戏新增 `GetDirectAdStatusSync` 和 `OnDirectAdStatusChange` 直玩广告状态接口
- 新增 GDT 通用上报 `ReportGdt` 及 `LoadFinish`、`Subscribe`、`TutorialStart`
- 新增 `RegisterGdtMenuEventListeners`，由 CP 在初始化成功后显式注册收藏和右上角分享监听

### Fixed

- 修复 `ReportUpdateLevel` Bridge 错误调用支付上报的问题
- JSSDK 4.0.2 支付改为自动补报，停用手动 `ReportPurchase` 以避免重复
- 移除 SDK 初始化成功后的收藏和右上角分享隐式监听
