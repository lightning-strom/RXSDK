...
using RuiXueLitJson.Extensions;
...
namespace RuiXueLitJson
{
    ...
    public class JsonMapper
    {
                                        ...
                                        int depth)
        {
            ...
            foreach (PropertyMetadata p_data in props) {
                if (p_data.IsField) {
                    ...
                    if (value == null)
                    {
                        if(Nullable.GetUnderlyingType(obj.GetType().GetField(p_data.Info.Name).FieldType) != null)
                        {
                            continue;
                        }
                    }
                    ...
                    if (p_info.PropertyType.FullName.StartsWith(excludeActionName))
                    {
                        continue;
                    }
                    //UnityEngine.Debug.Log(p_info.PropertyType);
                    //UnityEngine.Debug.Log(typeof(System.Action<>).IsAssignableFrom(p_info.PropertyType));
        ...
        private static readonly string excludeActionName = "System.Action";

        static bool IsNullableEmpty<T>(T obj)
        {
            return Nullable.GetUnderlyingType(typeof(T)) != null && obj == null;
        }
