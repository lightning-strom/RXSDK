...
function docusaurusPluginLLMs(context, options = {}) {
    ...
    // Set default options
    const { generateLLMsTxt = true, generateLLMsFullTxt = true, docsDir, ignoreFiles = [], title, description, llmsTxtFilename = 'llms.txt', llmsFullTxtFilename = 'llms-full.txt', includeBlog = false, pathTransformation, includeOrder = [], includeUnmatchedLast = true, customLLMFiles = [], excludeImports = false, removeDuplicateHeadings = false, generateMarkdownFiles = false, keepFrontMatter = [], rootContent, fullRootContent, addMdExtension = true, logLevel = 'normal', preserveDirectoryStructure = true, processingBatchSize = 100, warnOnIgnoredFiles = false, } = options;
    // Normalize docsDir into docsSections array
    const docsSections = Array.isArray(docsDir) && docsDir.length > 0
        ? docsDir
        : [{
