 ...
 */
async function generateLLMFile(docs, outputPath, fileTitle, fileDescription, includeFullContent, version, customRootContent, batchSize = 100, addMdExtension = true) {
...
${doc.content}`;
        ...
        if (docsHaveSections) {
            ...
            for (const doc of docs) {
                ...
                sectionMap.get(sectionKey).push(`- [${doc.title}](${linkUrl})${cleanedDescription ? `: ${cleanedDescription}` : ''}`);
            }
            const sectionBlocks = [];
            for (const [sectionLabel, items] of sectionMap) {
                const heading = (0, utils_1.isNonEmptyString)(sectionLabel) ? `## ${sectionLabel}\n\n` : '';
        ...
        const llmFileContent = (0, utils_1.createMarkdownContent)(fileTitle, `${fileDescription}${versionInfo}`, `${rootContent}\n\n${tocContent}`, true // include metadata (description)
        );
        try {
            await (0, utils_1.writeFile)(outputPath, llmFileContent);
        }
...
async function generateStandardLLMFiles(context, allDocFiles) {
    ...
    const { generateLLMsTxt, generateLLMsFullTxt, llmsTxtFilename = 'llms.txt', llmsFullTxtFilename = 'llms-full.txt', includeOrder = [], includeUnmatchedLast = true, version, generateMarkdownFiles = false, rootContent, fullRootContent, processingBatchSize = 100, addMdExtension = true } = options;
    ...
    if (generateLLMsTxt) {
        ...
        version, rootContent, processingBatchSize, addMdExtension);
    }
    ...
    if (generateLLMsFullTxt) {
        ...
        version, fullRootContent, processingBatchSize, addMdExtension);
    }
}
/**
...
async function generateCustomLLMFiles(context, allDocFiles) {
    const { outDir, siteUrl, docTitle, docDescription, options } = context;
    const { customLLMFiles = [], ignoreFiles = [], generateMarkdownFiles = false, processingBatchSize = 100, addMdExtension = true } = options;
    ...
    for (const customFile of customLLMFiles) {
        ...
        if (customDocs.length > 0) {
            ...
            await generateLLMFile(customDocs, customFilePath, customTitle, customDescription, customFile.fullContent, customFile.version, customFile.rootContent, processingBatchSize, addMdExtension);
...
async function collectDocFiles(context) {
    ...
    for (const section of docsSections) {
        ...
        try {
            ...
            const docFiles = await (0, utils_1.readMarkdownFiles)(fullDocsDir, siteDir, ignoreFiles, section.path, warnOnIgnoredFiles);
    ...
    if (includeBlog) {
        ...
        try {
            ...
            const blogFiles = await (0, utils_1.readMarkdownFiles)(blogDir, siteDir, ignoreFiles, context.docsDir, warnOnIgnoredFiles);
