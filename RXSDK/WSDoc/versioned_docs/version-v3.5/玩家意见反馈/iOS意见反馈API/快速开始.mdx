...
已经接入过**RXSDK_Pure**组件，无需重复接入，只需要更新到**3.5.28及以上版本**即可。
未接入过，则可通过 **pod** 方式接入 **RXSDK_Pure** 组件，安装**3.5.28及以上版本**，并使用 **RXSDK_Pure** 公告相关功能。
在需要使用功能的类中引用头文件 <code>#import </code>
:::

## SDK 集成

- 编辑 Podfile 文件，添加如下配置，并注意安装**3.5.28及以上版本**：
...
```objectivec
[[RXApiService sharedSDK] feedbackCreateWithContent:@"反馈内容" attachments:@["反馈附件地址"] phone:@"your phoneNum" tags:@[@"游戏透传标识"] complete:^(NSDictionary * _Nullable response, RX_CommonRequestError * _Nullable error) {
                if (!error) {
                    NSLog(@"请求成功：%@",response);
                }else{
...
```objectivec
[[RXApiService sharedSDK] getFeedbackListWithPage:page size:size status:0 complete:^(NSDictionary * _Nullable response, RX_CommonRequestError * _Nullable error) {
                if (!error) {
                    NSLog(@"请求成功：%@",response);
                }else{
                    NSLog(@"请求失败：%@",error.responesObject);
