...
| data | [DataNode](/docs/v3.5/社交/Unity 接入/快速接入) | 否   | 结果数据集 |     |
...
```csharp
    ...
    public void RelationAdd(string target, Dictionary types, string target_remarks,
...
```csharp
    ...
    public void RelationDelete(string target, Dictionary types, RequestResponseDelegate onResponse,
                RequestErrorDelegate onError);
...
| code | Number                         | 是   | 响应状态码，0为成功，非0表示出错 | 0   |
| msg | String                         | 否   | 响应信息，仅在status为非0时不为空 |     |
| data | [Node](/docs/v3.5/社交/Unity 接入/快速接入) | 否   | 结果数据集 |     |
