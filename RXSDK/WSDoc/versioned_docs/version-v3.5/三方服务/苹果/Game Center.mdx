...
:::tip
接入请前先做好准备工作。可通过 pod 方式接入 **RXGameCenterSDK** 组件，并使用 **RXGameCenterSDK** 相关功能。
...
![image.png](https://haiqi-ruixue-test.oss-cn-beijing.aliyuncs.com/docOnline/image-1758776864342.png)

### 配置App Store Connect

1. 登录 [App Store Connect](https://security.feishu.cn/link/safety?target=https%3A%2F%2Fappstoreconnect.apple.com&scene=ccm&logParams=%7B%22location%22%3A%22ccm_drive%22%7D&lang=zh-CN)
2. 选择你的应用
3. 进入"功能" → "Game Center"
4. 启用Game Center功能
5. 创建排行榜和成就
...
建议在游戏启动时或者其他恰当的时机进行GameCenter认证登录，确保在提交分数或解锁成就前已完成认证。
**接口原型**
