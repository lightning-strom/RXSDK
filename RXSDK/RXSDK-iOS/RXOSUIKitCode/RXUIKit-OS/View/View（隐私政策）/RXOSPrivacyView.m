...
@interface RXOSPrivacyView () <UITableViewDelegate, UITableViewDataSource, WKNavigationDelegate, WKUIDelegate, UIScrollViewDelegate>
...
@implementation RXOSPrivacyView
...
- (instancetype)initWithKey:(NSString *)key legalData:(NSDictionary *)legalData
{
    ...
    if (self) {
        ...
        RXOSLegalData *model = [RXOSLegalData rxu_modelWithDictionary:dic];
...
- (void)layoutViews
{
    ...
    RXOSLegalData_term *term = self.privacyArr[0];
...
//    _webView.frame = CGRectMake(CGRectGetMaxX(_mTableView.frame) + 8, 42, bgW - tableW - 8 - 4, bgH - 42);
    ...
    for (int i = 0; i < self.privacyArr.count; i++) {
        RXOSLegalData_term *term = self.privacyArr[i];
...
#pragma mark -增大字体大小
    if (self.orientation == 1) {
        ...
        [webView evaluateJavaScript:@"document.getElementsByTagName('body')[0].style.webkitTextSizeAdjust= '65%'" completionHandler:nil];
    }
#pragma mark -修改字体颜色
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    RXOSLegalData_term *term = self.privacyArr[indexPath.row];
