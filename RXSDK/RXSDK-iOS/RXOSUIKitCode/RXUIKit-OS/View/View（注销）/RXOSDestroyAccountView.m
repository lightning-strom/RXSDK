...
@interface RXOSDestroyAccountView () <UIScrollViewDelegate>
...
@implementation RXOSDestroyAccountView
...
- (void)setUI
{
    ...
    [self.bgView addSubview:self.desScrollView];
    [self.desScrollView addSubview:self.desLbl];
...
- (void)layoutViews
{
    ...
    _desScrollView.frame = CGRectMake(0, RXAC ? 71 : 71, CGRectGetWidth(self.bgView.frame) - (RXAC ? 13 : 13), RXAC ? 124 : 124);
    
    CGFloat desH = [[RXLocation osLaunguage:_desLbl.text] heightForFont:_desLbl.font width:bgW - (RXAC ? 58 : 50)] + 5;
    
    _desLbl.frame = CGRectMake(RXAC ? 29 : 25, 0, bgW - (RXAC ? 58 : 50), desH);
    _desScrollView.contentSize = CGSizeMake(0, desH);

    if ([RXOSCommonTool isRTL]) {
        _button2.frame = CGRectMake(RXAC ? 29 : 25, RXAC ? 208 : 208, bgW - (RXAC ? 58 : 50), RXAC ? 43 : 48);
        
        _button1.frame = CGRectMake(CGRectGetMinX(_button2.frame), CGRectGetMaxY(_button2.frame) + 7, CGRectGetWidth(_button2.frame), CGRectGetHeight(_button2.frame));
    } else {
        _button1.frame = CGRectMake(RXAC ? 29 : 25, RXAC ? 217 : 217, bgW - (RXAC ? 58 : 50), RXAC ? 43 : 48);
        
        _button2.frame = CGRectMake(CGRectGetMinX(_button1.frame), CGRectGetMaxY(_button1.frame) + 7, CGRectGetWidth(_button1.frame), CGRectGetHeight(_button1.frame));
    }
    
    _closeBtn.frame = CGRectMake(bgW - RXUScaleWidth(40), 16, RXUScaleWidth(16), RXUScaleWidth(16));
...
- (UILabel *)desLbl
{
    if (!_desLbl) {
        ...
        if (self.desType == DestroyType_unRepeal || self.desType == DestroyType_repeal) {
            text = @"尊敬的玩家，该账号已提交了注销申请，请您不要登录和使用该账号，以确保注销操作的顺利完成，如需撤销注销申请，请点击下方按钮进行撤销操作。";
        } else {
    ...
    if (!_button1) {
        ...
        if (self.isDIY) {
            ...
            if (self.desType == DestroyType_unRepeal) {
                ...
                [_button1 setBackgroundImage:[UIImage rxOSBundleImageNamed:@"btnBg_unSelect"] forState:UIControlStateNormal];
            } else if (self.desType == DestroyType_repeal) {
                _button1.userInteractionEnabled = YES;
                [_button1 setBackgroundImage:[UIImage rxOSBundleImageNamed:@"btnBg2"] forState:UIControlStateNormal];
            } else if (self.desType == DestroyType_refuse) {
                title = @"知道了";
            }
        }
        
        [_button1 setTitle:[RXLocation osLaunguage:title] forState:UIControlStateNormal];
...
//        [_button2 setBackgroundImage:[UIImage rxOSBundleImageNamed:@"btnBg1"] forState:UIControlStateNormal];
        ...
        [_button2 setTitle:[RXLocation osLaunguage:title] forState:UIControlStateNormal];
...
- (UIButton *)closeBtn
{
    if (!_closeBtn) {
        ...
        [_closeBtn setImage:[UIImage rxOSBundleImageNamed:@"close"] forState:UIControlStateNormal];
...
{
    if (!_desScrollView) {
        _desScrollView = [[UIScrollView alloc] init];
        _desScrollView.delegate = self;
    }
    return _desScrollView;
}
