...
@interface RXOSGiftPopView ()<WKUIDelegate, WKNavigationDelegate>
...
@implementation RXOSGiftPopView
...
- (void)setPopViewDic:(NSDictionary *)dic{
    ...
    if (is_permanent == 0) {
        self.priLbl.text = [RXLocation osLaunguage:@"永久有效"];
    }else{
        NSString *daysString = [NSString stringWithFormat:@"%@", dic[@"time_limit"]];
        NSString *text = [NSString stringWithFormat:@"%@ %@ %@",[RXLocation osLaunguage:@"还有"], daysString, [RXLocation osLaunguage:@"天到期"]];
        NSString *language =  [RXOSCommonTool getLanguage];
        if ([language isEqualToString:@"ja"]) {
            text = [NSString stringWithFormat:@"%@%@", daysString, [RXLocation osLaunguage:@"天到期"]];
        }
