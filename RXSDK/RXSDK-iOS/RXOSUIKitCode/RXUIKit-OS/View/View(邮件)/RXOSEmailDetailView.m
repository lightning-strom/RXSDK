...
#import "RXOSTopAlignedLabel.h"
#import "RXOSGiftPopView.h"
#import "RXOSEmailBrowserView.h"
#import <RXSDK_Pure/RX_CommonNetworkExcuteManager.h>

@interface RXOSEmailDetailView ()<WKUIDelegate, WKNavigationDelegate,WKScriptMessageHandler,UIScrollViewDelegate>
...
@property (nonatomic, strong) RXOSTopAlignedLabel *topNameLabel;//竖屏
...
@property (nonatomic, strong) RXOSGiftPopView *giftPopView;
...
@implementation RXOSEmailDetailView
...
- (void)layoutViews
{
    ...
    if (RXAC) {//横屏 左右100 上下30，间距尽量不动，其他等比例
        ...
        if ([RXOSCommonTool isRTL]) {
            self.receiveBtn.frame = CGRectMake(15, bgViewHeight - 13 - RXUScaleWidth(38), RXUScaleWidth(135), RXUScaleWidth(38));
            
            self.deleteBtn.frame = CGRectMake(15, bgViewHeight - 13 - RXUScaleWidth(38), RXUScaleWidth(135), RXUScaleWidth(38));
            
            CGFloat viewHeight = 40;
            CGFloat totalWidth = (14 * 5) + (viewHeight * 6);
            
            self.scrollView.frame = CGRectMake(bgViewWidth - totalWidth - 36, bgViewHeight - 13 - 40, totalWidth, viewHeight);
            
            self.leftImageView.frame = CGRectMake(self.scrollView.frame.origin.x - 8 - 16, self.scrollView.frame.origin.y + 12 , 16, 16);
            ...
            [self.backBtn setImage:flipImage forState:UIControlStateNormal];
...
#pragma mark - 测试数据,需要删除
    ...
    if (RXAC) {
        ...
        if ([RXOSCommonTool isRTL]) {
            ...
            if (propsCount > 6) {
                [self.scrollView setContentOffset:CGPointMake((propsCount - 6) * (viewWidth + horizontalSpacing), 0) animated:NO];
            }
            
            for (int i = 0; i < propsCount; i ++) {
                RXOSEmailGiftView *giftView = [[RXOSEmailGiftView alloc] init];
                giftView.tag = i;
                [giftView setGiftDic:propsArray[i]];
                [giftView setStatus:[self.detailDic[@"status"] integerValue]];
                giftView.frame = CGRectMake(scrollViewTotalWidth - (i * horizontalSpacing + (i + 1 ) * viewWidth), 0, viewWidth, viewHeight);
        ...
        if ([RXOSCommonTool isRTL]) {
            if (propsCount > 4) {
                [self.scrollView setContentOffset:CGPointMake((propsCount - 4) * (viewWidth + horizontalSpacing), 0) animated:NO];
            }
            ...
            for (int i = 0; i < propsCount; i ++) {
                ...
                CGFloat startX = totalWidth;//大于等于4起点
                if (propsCount < 4) {//小于4起点
                    startX = (scrollViewFrameWidth - totalWidth)/2;
                }
                
                CGFloat xPosition = startX - (viewWidth * (i + 1) + horizontalSpacing * (i));
                CGFloat yPosition = 0;
...
- (RXOSTopAlignedLabel *)topNameLabel{
    if (!_topNameLabel) {
        _topNameLabel = [[RXOSTopAlignedLabel alloc] init];
        _topNameLabel.text = @"";
...
- (UIButton *)receiveBtn{
    if (!_receiveBtn) {
        ...
        [_receiveBtn setTitle:[RXLocation osLaunguage:@"领取道具"] forState:UIControlStateNormal];
...
- (UIButton *)deleteBtn{
    if (!_deleteBtn) {
        ...
        [_deleteBtn setTitle:[RXLocation osLaunguage:@"删除邮件"] forState:UIControlStateNormal];
...
- (RXOSGiftPopView *)giftPopView{
    if (!_giftPopView) {
        _giftPopView = [[RXOSGiftPopView alloc] init];
...
- (UIImageView *)leftImageView{
    if (!_leftImageView) {
        ...
        _leftImageView.image = [UIImage rxOSBundleImageNamed:@"rx_emailLeft"];
    }
...
- (UIImageView *)rightImageView{
    if (!_rightImageView) {
        ...
        _rightImageView.image = [UIImage rxOSBundleImageNamed:@"rx_emailRight"];
    }
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler{
    ...
    if (navigationAction.navigationType == WKNavigationTypeLinkActivated) {
        // 通过浏览器访问
        RXOSCommonWKWebView *webView = [[RXOSCommonWKWebView alloc] initWithUrl:absoluteString title:[RXLocation osLaunguage:@"链接"] content:nil];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    if ([message.name isEqualToString:@"imageClicked"]) {
        ...
        RXOSEmailBrowserView *view = [[RXOSEmailBrowserView alloc] initWithPicArray:self.imageUrlArr index:clickedIndex];
    }
}
