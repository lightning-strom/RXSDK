...
@interface RXOSAddLoginView () <RXOSAttributeLabelDelegate, UITextFieldDelegate>
...
@implementation RXOSAddLoginView
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if (_accountTF.placeholderLbl.hidden && _passwordTF.placeholderLbl.hidden) {
        ...
        [_confirmBtn setBackgroundImage:[UIImage rxOSBundleImageNamed:@"btnBg2"] forState:UIControlStateNormal];
    } else {
        _confirmBtn.userInteractionEnabled = NO;
        [_confirmBtn setBackgroundImage:[UIImage rxOSBundleImageNamed:@"btnBg_unSelect"] forState:UIControlStateNormal];
    }
...
- (void)confimBtnAction
{
    ...
    if (_accountTF.tf.text.length <= 0) {
        ...
        [RXOSHUD showErrorText:@"请勾选同意用户隐私协议"];
        return;
    }
...
- (void)selectBtnAction:(UIButton *)btn
{
    ...
    if (btn.isSelected) {
        [_selectBtn setImage:[UIImage rxOSBundleImageNamed:@"greenSelect"] forState:UIControlStateNormal];
    } else {
        [_selectBtn setImage:[UIImage rxOSBundleImageNamed:@"greenUnSelect"] forState:UIControlStateNormal];
    }
}

- (void)priTapAction:(UITapGestureRecognizer *)tap
{
//    RXOSPrivacyView *priView = [[RXOSPrivacyView alloc] initWithKey:@"00001"];
    [self endEditing:YES];
}

- (void)registAction
{
    RXOSRegistView *registView = [[RXOSRegistView alloc] initWithType:RegistViewType_regist extDic:self.extDic complete:self.block];
    [self endEditing:YES];
}
...
{
    if (!_accountTF) {
        _accountTF = [[RXOSTextField alloc] initWithPlaceholder:@"请输入账号" type:TFType_clear keyboardType:UIKeyboardTypeNumbersAndPunctuation];
...
{
    if (!_passwordTF) {
        _passwordTF = [[RXOSTextField alloc] initWithPlaceholder:@"请输入密码" type:TFType_clear keyboardType:UIKeyboardTypeDefault];
...
- (UIButton *)confirmBtn
{
    if (!_confirmBtn) {
        ...
        _confirmBtn.userInteractionEnabled = NO;
        [_confirmBtn setBackgroundImage:[UIImage rxOSBundleImageNamed:@"btnBg_unSelect"] forState:UIControlStateNormal];
        [_confirmBtn setTitle:[RXLocation osLaunguage:@"登录"] forState:UIControlStateNormal];
...
- (UIButton *)registBtn
{
    if (!_registBtn) {
        ...
        [_registBtn setBackgroundImage:[UIImage rxOSBundleImageNamed:@"btnBg1"] forState:UIControlStateNormal];
