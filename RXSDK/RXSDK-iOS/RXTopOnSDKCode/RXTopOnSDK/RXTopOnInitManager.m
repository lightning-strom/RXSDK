//
//  RXTopOnInitManager.m
//  RXTopOnSDK
//
//  Created by root11 on 2024/5/24.
//

#import "RXTopOnInitManager.h"

@interface RXTopOnInitManager ()

@property (nonatomic, assign) BOOL isInit;

@end

@implementation RXTopOnInitManager

static RXTopOnInitManager *sharedSDK = nil;
static dispatch_once_t onceToken;

+ (instancetype)sharedSDK
{
    dispatch_once(&onceToken, ^{
        sharedSDK = [[RXTopOnInitManager alloc] init];
    });
    return sharedSDK;
}
...
/**
 * 设置是否打印日志，YES打印，NO不打印，默认为NO； 打包上线时最好关闭
 */
...
- (BOOL)startWithAppID:(NSString *)appID
         ...
         sdkConfigures:(ATSDKConfiguration *)sdkConfigure
                 error:(NSError**)error{
    return [[ATAPI sharedInstance] startWithAppID:appID appKey:appKey sdkConfigures:sdkConfigure error:nil];
}
...
- (void)customData:(NSDictionary *)customData{
    [ATAPI sharedInstance].customData = customData;
}
...
- (void)setExludeAppleIdArray:(NSArray *)appleIdArray{
    [[ATAPI sharedInstance] setExludeAppleIdArray:appleIdArray];
}
...
- (void)setDeniedUploadInfoArray:(NSArray *)uploadInfoArray{
    [[ATAPI sharedInstance] setDeniedUploadInfoArray:uploadInfoArray];
}
...
- (void)setExludePlacementid:(NSString *)placementid
    ...
    [[ATAdManager sharedManager] setExludePlacementid:placementid unitIDArray:unitIDArray];
}
...
- (void)setExludePlacementid:(NSString *)placementid
    ...
    [[ATAdManager sharedManager] setExludePlacementid:placementid networkFirmIDArray:networkFirmIDArray];
}
...
- (void)setSystemPlatformType:(ATSystemPlatformType)type{
    [[ATAPI sharedInstance] setSystemPlatformType:type];
}
...
+ (void)setDebuggerConfig:(void(^_Nullable)(ATDebuggerConfig * _Nullable debuggerConfig))debuggerConfigBlock{
    [ATAPI setDebuggerConfig:debuggerConfigBlock];
}
...
+ (void)setDeviceInfoConfig:(void(^_Nullable)(ATDeviceInfoConfig * _Nullable deviceInfoConfig))deviceInfoConfigBlock{
    [ATAPI setDeviceInfoConfig:deviceInfoConfigBlock];
}

/**
 获取用户当前位置是否在欧盟以内，如果在欧盟以内，需要设置Google UMP流程设置欧盟的GDPR(《通用数据保护条例》)等级。
 调用‘showGDPRConsentDialogInViewController:dismissalCallback:’方法进行授权申请
...
- (void)getUserLocationWithCallback:(void(^)(ATUserLocation location))callback{
    [[ATAPI sharedInstance] getUserLocationWithCallback:callback];
}

/**
 展示UMP流程下GDPR授权。在'getUserLocationWithCallback:'后，如果处于欧盟，则调用此方法授权；不属于欧盟则正常调用初始化
...
- (void)showGDPRConsentDialogInViewController:(UIViewController *)viewController
    ...
    [[ATAPI sharedInstance] showGDPRConsentDialogInViewController:viewController dismissalCallback:dismissCallback];
}
...
- (void)presentDataConsentDialogInViewController:(UIViewController *)viewController
                          loadingFailureCallback:(void(^)(NSError *error))loadingFailureCallback
                               dismissalCallback:(void(^)(void))dismissCallback{
    [[ATAPI sharedInstance] presentDataConsentDialogInViewController:viewController loadingFailureCallback:loadingFailureCallback dismissalCallback:dismissCallback];
}
...
- (void)presentDataConsentDialogInViewController:(UIViewController *)viewController
                               dismissalCallback:(void(^)(void))dismissCallback{
    [[ATAPI sharedInstance] presentDataConsentDialogInViewController:viewController dismissalCallback:dismissCallback];
}
...
- (void)setDataConsentSet:(ATDataConsentSet)dataConsentSet
    ...
    [[ATAPI sharedInstance] setDataConsentSet:dataConsentSet consentString:consentString];
}
