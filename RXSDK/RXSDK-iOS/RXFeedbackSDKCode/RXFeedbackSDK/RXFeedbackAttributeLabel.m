...
@interface RXFeedbackAttributeLabel () <UITextViewDelegate>
...
@implementation RXFeedbackAttributeLabel
...
- (void)addGestureRecognizer:(UITapGestureRecognizer *)gestureRecognizer
{
    if([gestureRecognizer isKindOfClass:[UITapGestureRecognizer class]]) {
        ...
        if (url) {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(rxAttributeClick:)])
            {
                [self.delegate rxAttributeClick:(NSString *)url];
            }
        }
    }
}
