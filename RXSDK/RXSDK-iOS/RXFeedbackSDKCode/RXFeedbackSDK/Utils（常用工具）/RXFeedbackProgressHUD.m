...
#define MBMainThreadAssert() NSAssert([NSThread isMainThread], @"RXFeedbackProgressHUD needs to be accessed on the main thread.");
...
@interface RXFeedbackProgressHUD ()
...
@interface RXFeedbackProgressHUDRoundedButton : UIButton
@end


@implementation RXFeedbackProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    RXFeedbackProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    RXFeedbackProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (RXFeedbackProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            RXFeedbackProgressHUD *hud = (RXFeedbackProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = RXFeedbackProgressHUDAnimationFade;
    _mode = RXFeedbackProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(RXFeedbackProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == RXFeedbackProgressHUDAnimationZoom) {
        type = animatingIn ? RXFeedbackProgressHUDAnimationZoomIn : RXFeedbackProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == RXFeedbackProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == RXFeedbackProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == RXFeedbackProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == RXFeedbackProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    RXFeedbackProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<RXFeedbackProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    backgroundView.style = RXFeedbackProgressHUDBackgroundStyleSolidColor;
    ...
    UIButton *button = [RXFeedbackProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    RXFeedbackProgressHUDMode mode = self.mode;
    if (mode == RXFeedbackProgressHUDModeIndeterminate) {
    ...
    else if (mode == RXFeedbackProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[MBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == RXFeedbackProgressHUDModeDeterminate || mode == RXFeedbackProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
        ...
        if (mode == RXFeedbackProgressHUDModeAnnularDeterminate) {
            [(MBRoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == RXFeedbackProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == RXFeedbackProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[RXFeedbackProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[RXFeedbackProgressHUD class]]];
#endif
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MBRoundProgressView appearanceWhenContainedIn:[RXFeedbackProgressHUD class], nil];
#else
        appearance = [MBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[RXFeedbackProgressHUD class]]];
#endif
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MBBarProgressView appearanceWhenContainedIn:[RXFeedbackProgressHUD class], nil];
#else
        appearance = [MBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[RXFeedbackProgressHUD class]]];
#endif
...
- (void)setMode:(RXFeedbackProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == RXFeedbackProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style = RXFeedbackProgressHUDBackgroundStyleBlur;
...
- (void)setStyle:(RXFeedbackProgressHUDBackgroundStyle)style {
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
    }
...
- (void)updateForBackgroundStyle {
    ...
    RXFeedbackProgressHUDBackgroundStyle style = self.style;
    if (style == RXFeedbackProgressHUDBackgroundStyleBlur) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == RXFeedbackProgressHUDBackgroundStyleBlur) {
        self.backgroundColor = self.color;
    } else {
        self.backgroundColor = self.color;
    }
...
@implementation RXFeedbackProgressHUDRoundedButton
