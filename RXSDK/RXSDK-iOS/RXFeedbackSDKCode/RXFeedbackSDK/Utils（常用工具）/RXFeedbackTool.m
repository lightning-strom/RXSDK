...
{
    NSMutableArray *accounts = [NSMutableArray arrayWithArray:[RXFeedbackUserUtility sharedManager].accounts];
    ...
    [RXFeedbackUserUtility saveAccounts:accounts];
}
...
+ (NSString *)getIconWithLoginType:(LoginType)loginType username:(NSString *)username
{
    ...
    switch (loginType) {
        ...
        {
            icon = @"rx_login_code";
            if ([RXFeedbackTool validateEmail:username]) {
                icon = @"rx_login_mail";
            }
            break;
        }
...
{
    NSInteger orientation = [RXFeedbackTool getInterfaceOrientation];
    if (!RXAC) { // 竖屏
        return 346;
    } else { // 横屏
        return 381;
 ...
 * 初始化配置文件模型转换
 */
+ (RXFeedbackUILoginConfig *)fetchInitProfile
{
    RXFeedbackUILoginConfig *loginConfig = [[RXFeedbackUILoginConfig alloc] init];
    NSMutableDictionary *initProfile = [RXFeedbackUserUtility sharedManager].inProfile[@"passport"];
    
    // logo
    loginConfig.logoImage = [RXFeedbackTool getImageFromURL:[initProfile valueForKey:@"logo"]];
    ...
    for (int i = 0; i < profile_privacies.allKeys.count; i++) {
        NSString *privaceUrl = [NSString stringWithFormat:@"%@/static/landing/#/v1/legal/terms/%@/%@/%@", [[RXService sharedSDK] getFirstBaseUrl], [RXFeedbackUserUtility sharedManager].productId, [RXFeedbackUserUtility sharedManager].channelId, profile_privacies.allKeys[i]];
...
{
    if ([RXFeedbackUserUtility sharedManager].logoImage) {
        return [RXFeedbackUserUtility sharedManager].logoImage;
    }
    UIImage *result;
    NSData *data = [NSData dataWithContentsOfURL:[NSURL URLWithString:fileURL]];
    result = [UIImage imageWithData:data];
    [RXFeedbackUserUtility sharedManager].logoImage = result;
    return result;
}
 ...
 * 模型转换
 */
+ (RXFeedbackUILoginConfig *)toConfig:(RXLoginUIModel *)model
{
    RXFeedbackUILoginConfig *config = [[RXFeedbackUILoginConfig alloc] init];
