...
@interface RXAddLoginView () <UITextFieldDelegate>
...
- (void)confimBtnAction
{
    ...
    [RXUserUtility savePhone:self.accountTF.tf.text];
    [RXUserUtility sharedManager].password = self.passwordTF.tf.text;
    
    [[NSNotificationCenter defaultCenter] postNotificationName:noti_addLogin object:nil];
...
- (UIButton *)registBtn
{
    if (!_registBtn) {
        ...
        [_registBtn setBackgroundImage:[UIImage bundleImageNamed:@"btnBg1"] forState:UIControlStateNormal];
