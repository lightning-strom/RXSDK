            ...
            if ([PNSCodeSuccess isEqualToString:[resultDic objectForKey:@"resultCode"]] == YES) {
                [[TXCommonHandler sharedInstance] accelerateLoginPageWithTimeout:3.0 complete:^(NSDictionary * _Nonnull resultDic) {
                    ...
                    if ([PNSCodeSuccess isEqualToString:[resultDic objectForKey:@"resultCode"]] == NO) {
                        [RXProgressHUD showHUDWithText:@"获取授权失败，请使用其他方式登录" afterDelay:1.5];
                    } else {
...
//            [SVProgressHUD show];
            ...
            [RXUserUtility sharedManager].aliAuthToken = token;
            
            NSMutableDictionary *extDic = [NSMutableDictionary dictionaryWithDictionary:[RXUserUtility sharedManager].extDic];
            [extDic setValue:token forKey:@"access_token"];
            
            [RXLoginManager loginWithExtDic:extDic username:@"" password:@"" loginOpenId:@"" loginType:LoginTypeAuth complete:^(NSDictionary * _Nonnull response, RXCommonRequestError * _Nonnull error) {
                if (error) {
                    if ([RXUserUtility sharedManager].loginCompleteBlock) {
                        [RXUserUtility sharedManager].loginCompleteBlock(nil, error);
                    }
                    [[TXCommonHandler sharedInstance] cancelLoginVCAnimated:YES complete:^{
                    }];
                } else {
                    if ([RXUserUtility sharedManager].loginCompleteBlock) {
                        [RXUserUtility sharedManager].loginCompleteBlock(response, nil);
                    }
                    [[TXCommonHandler sharedInstance] cancelLoginVCAnimated:YES complete:^{
                        NSLog(@"关闭一键登录页面");
                    }];
