...
@protocol RXModelFactory <NSObject>
- (id)rx_modelForPropertyName:(NSString *)propertyName withDictionary:(NSDictionary *)dictionary;
@end

@implementation NSObject (RXAddition)
...
+ (instancetype)rx_modelWithDictionary:(NSDictionary *)dictionary {
    ...
    for (unsigned int i = 0; i < count; i++) {
        ...
        if ([value isKindOfClass:[NSDictionary class]] && ![propertyType hasPrefix:@"NS"]) {
            ...
            if (modelClass != nil) {
                // 将被嵌套字典数据也转化成Model
                value = [modelClass rx_modelWithDictionary:value];
            }
        }
        ...
        if ([value isKindOfClass:[NSArray class]] && perpertyTypeDic) {
            ...
            for (NSDictionary *itemDic  in value) {
                id model = [itemModelClass rx_modelWithDictionary:itemDic];
