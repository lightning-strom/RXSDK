...
@implementation NSObject (RXWXAddition)
...
+ (instancetype)rxwx_modelWithDictionary:(NSDictionary *)dictionary {
    ...
    for (unsigned int i = 0; i < count; i++) {
        ...
        if ([value isKindOfClass:[NSDictionary class]] && ![propertyType hasPrefix:@"NS"]) {
            ...
            if (modelClass != nil) {
                // 将被嵌套字典数据也转化成Model
                value = [modelClass rxwx_modelWithDictionary:value];
            }
        }
        ...
        if ([value isKindOfClass:[NSArray class]] && perpertyTypeDic) {
            ...
            for (NSDictionary *itemDic  in value) {
                id model = [itemModelClass rxwx_modelWithDictionary:itemDic];
