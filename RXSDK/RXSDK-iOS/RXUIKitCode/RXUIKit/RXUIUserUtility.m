...
static NSString *const keyUserData_simplePassword = @"rx_simplePassword"; // 设置密码强度
static NSString *const keyUserData_pwdPattern = @"rx_pwdPattern"; // 密码正则
static NSString *const keyUserData_wxAppid = @"rx_wxAppid";
static NSString *const keyUserData_logoImage = @"rx_logoImage";

@implementation RXUIUserUtility

static RXUIUserUtility *sharedSDK = nil;
static dispatch_once_t onceToken;

+ (instancetype)sharedManager
{
    dispatch_once(&onceToken, ^{
        sharedSDK = [[RXUIUserUtility alloc] init];
    });
    return sharedSDK;
}
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.allowExtensionPointIdentifier = YES;
    }
    return self;
}
...
- (RXPasswordStrength)passwordType
{
    RXPasswordStrength pwdType = [[[NSUserDefaults standardUserDefaults] valueForKey:keyUserData_simplePassword] longValue];
    return pwdType;
}

- (NSString *)pwdPattern
{
    return [[NSUserDefaults standardUserDefaults] valueForKey:keyUserData_pwdPattern];
}

/** 保存微信appid */
+ (void)saveWXAppid:(NSString *)wxAppid
{
    [[NSUserDefaults standardUserDefaults] setValue:wxAppid forKey:keyUserData_wxAppid];
}

- (NSString *)wxAppid
{
    return [[NSUserDefaults standardUserDefaults] valueForKey:keyUserData_wxAppid];
}
