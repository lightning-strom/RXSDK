...
+ (BOOL)checkPasswordWithPwd:(NSString *)pwd
{
    ...
    // 密码强度设置
    RXPasswordStrength passwordType = [RXUIUserUtility sharedManager].passwordType;
    if (passwordType == Default || passwordType == Average) {
        regex = @"^.{6,32}$";
    } else if (passwordType == Custom) {
        regex = [RXUIUserUtility sharedManager].pwdPattern;
    }
...
+ (RXUserType)getUserType:(NSString *)userTypeStr
{
    ...
    } else if ([userTypeStr isEqualToString:@"auth"] || [userTypeStr isEqualToString:@"quickphone"]) {
...
{
    NSInteger orientation = [RXUICommonTool getInterfaceOrientation];
    if (!RXAC) { // 竖屏
        return 345;
    } else { // 横屏
        return 366;
...
{
    RXLoginUIConfig *loginConfig = [[RXLoginUIConfig alloc] init];
    NSMutableDictionary *initProfile = [RXUIUserUtility sharedManager].inProfile[@"passport"];
    
    // logo
    loginConfig.logoImage = [RXUICommonTool getImageFromURL:[initProfile valueForKey:@"logo"]];
    ...
    // 登录后是否强制实名认证
//    loginConfig.needRealAuth = [initProfile valueForKey:@"realauth"];
    ...
    for (int i = 0; i < profile_privacies.allKeys.count; i++) {
        if (i > 2) break;
        NSString *privaceUrl = [NSString stringWithFormat:@"%@/static/landing/#/v1/legal/terms/%@/%@", [[RXService sharedSDK] getFirstBaseUrl], [RXUIUserUtility sharedManager].channelId, profile_privacies.allKeys[i]];
        [privacies addObject:privaceUrl];
        
        // 取标题
        NSDictionary *privaceDic = profile_privacies.allValues[i];
        [privaceTitles addObject:privaceDic[@"zh"]];
    }
...
{
    if ([RXUIUserUtility sharedManager].logoImage) {
        return [RXUIUserUtility sharedManager].logoImage;
    }
    UIImage *result;
    NSData *data = [NSData dataWithContentsOfURL:[NSURL URLWithString:fileURL]];
    result = [UIImage imageWithData:data];
    [RXUIUserUtility sharedManager].logoImage = result;
    return result;
}
...
//        [[RXWXService sharedSDK] syncInfoWithWXAppid:[RXUIUserUtility sharedManager].wxAppid complete:^(NSDictionary * _Nonnull response, RX_CommonRequestError * _Nonnull error) {
//            if (!error) {
//                NSMutableDictionary *loginData = [NSMutableDictionary dictionaryWithDictionary:[RXUIUserUtility sharedManager].loginData];
//                NSMutableDictionary *loginData1 = [NSMutableDictionary dictionaryWithDictionary:[RXUIUserUtility sharedManager].apiLoginData];
...
//                }
//                if (loginData1 && loginData1.allKeys.count > 0) {
//                    [RXUIUserUtility sharedManager].apiLoginData = loginData1;
//                }
//                if (loginData2 && loginData2.allKeys.count > 0) {
...
//    config.loginMode = model.loginMode;
    config.loginOpenid = model.loginOpenid;
    config.method = model.method;
    config.setCustomParams = model.setCustomParams;
    config.signFields = model.signFields;
    config.migrateArgs = model.migrateArgs;
    config.setCustomExt = model.setCustomExt;
    config.guestTitle = model.guestTitle;
