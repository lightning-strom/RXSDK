...
#import "RXWebLoadingView.h"
#import "RXDragView.h"
#import <RXSDK_Pure/RX_CommonNetworkExcuteManager.h>

@interface RXWKWebView () <WKNavigationDelegate, WKUIDelegate>

@property (nonatomic, strong) UIImageView *logoImageView;
@property (nonatomic, strong) RXWebLoadingView *loadingView;
...
//        [[UIApplication sharedApplication].keyWindow addSubview:self];
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(closeWebView:) name:RXUINoti_closeWebView1 object:nil];
...
- (void)syncInfo:(NSNotification *)noti
{
    ...
    [RXUIUserUtility sharedManager].loginData = loginDic;
...
{
    if (![self.webView isEqual:noti.object]) {
        return;
    }
    NSDictionary *dic = noti.userInfo;
...
//    _logoImageView.center = _naviBar.center;
    ...
    _backBtn.frame = CGRectMake(RXAC ? 33 : 22, RXAC ? 8 : kRXStatusBarHeight + 2, 28, 28);
    
    _closeBtn.frame = CGRectMake(CGRectGetWidth(_naviBar.frame) - (RXAC ? 65 : 54), RXAC ? 8 : kRXStatusBarHeight + 2, 28, 28);
    ...
    //    _logoImageView.center = _naviBar.center;
        _titleLbl.frame = CGRectMake(0, RXAC ? 0 : kRXStatusBarHeight, CGRectGetWidth(_naviBar.frame), 57);
        _line.frame = CGRectMake(0, CGRectGetMaxY(_titleLbl.frame), CGRectGetWidth(_naviBar.frame), 1);
        
        _backBtn.frame = CGRectMake(RXAC ? 29 : 25, CGRectGetHeight(_naviBar.frame) / 2 - 6, 28, 28);
        
        _closeBtn.frame = CGRectMake(CGRectGetWidth(_naviBar.frame) - 52, CGRectGetHeight(_naviBar.frame) / 2 - 6, 28, 28);
...
//        _loadingView = [[RXOSWebLoadingView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_naviBar.frame), CGRectGetWidth(window.frame), CGRectGetHeight(window.frame) - CGRectGetHeight(_naviBar.frame))];
    ...
    if (!_loadingView) {
        _loadingView = [[RXWebLoadingView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_naviBar.frame), CGRectGetWidth(window.frame), CGRectGetHeight(window.frame) - CGRectGetHeight(_naviBar.frame))];
...
- (void)registJSFunction
{
    ...
    // 用户行为统计
    [self.webView.configuration.userContentController addScriptMessageHandler:self name:@"trackUserAction"];
    
    // 支付宝 IIFAA 实名认证（跳转支付宝，返回前台后查询认证结果并通过 iifaaResult 回传 JS）
    [self.webView.configuration.userContentController addScriptMessageHandler:self name:@"openIIFAAAuth"];
}
// oc调用js
- (void)regisOCToJS
{
...
- (NSString *)setCookie
{
    ...
    [baseInfo setValue:@"zh" forKey:@"language"];
    [baseInfo setValue:[RXUICommonTool getCountryCode] forKey:@"country_code"];
    ...
    NSMutableDictionary *userCenterProfile = [NSMutableDictionary dictionaryWithDictionary:[RXUIUserUtility sharedManager].inProfile[@"user_center"]];
    ...
    NSString *jsonStr3 = [RXUICommonTool getJsonString:userCenterProfile];
    ...
    NSString *jsonStr4 = [RXUICommonTool getJsonString:device];
    ...
    RXUserCenterConfig *userConfig = [RXUIUserUtility sharedManager].userCenterConfig;
    ...
    NSString *jsonStr6 = [RXUICommonTool getJsonString:cpData];
    ...
    switch ([RXUIUserUtility sharedManager].passwordType) {
        case 0:
            pwdType = @"default";
            break;
        case 1:
    ...
    [passwordStrength setValue:[RXUIUserUtility sharedManager].pwdPattern forKey:@"pattern"];
    
    NSString *jsonStr7 = [RXUICommonTool getJsonString:passwordStrength];
    ...
    NSString *jsonStr8 = [RXUICommonTool getJsonString:protocolDic];
    ...
    if ([requestHeaderDic isKindOfClass:[NSDictionary class]] && requestHeaderDic.allKeys.count > 0) {
        NSString *jsonStr10 = [RXUICommonTool getJsonString:requestHeaderDic];
...
- (void)webView:(WKWebView *)webView didStartProvisionalNavigation:(WKNavigation *)navigation
{
    ...
    if (!_visibleHUD) {
        [RXHUD showHUDWithOffset:CGRectMake(CGRectGetWidth(window.frame) / 2 - 20, CGRectGetMinY(self.frame) + CGRectGetMinY(self.webView.frame) + (CGRectGetHeight(self.webView.frame) / 2) - 20, 40, 40)];
    } else {
...
//    if ([scheme isEqualToString:@"rx://openWebView?info=jsonstring"]) {
