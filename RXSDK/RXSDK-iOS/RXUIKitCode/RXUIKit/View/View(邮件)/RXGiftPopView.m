...
@interface RXGiftPopView ()<WKUIDelegate, WKNavigationDelegate>
...
- (void)setPopViewDic:(NSDictionary *)dic{
    ...
    if (is_permanent == 0) {
        self.priLbl.text = @"永久有效";
    }else{
        NSString *daysString = [NSString stringWithFormat:@"%@", dic[@"time_limit"]];
        NSString *text = [NSString stringWithFormat:@"%@ %@ %@",@"还有", daysString, @"天到期"];
