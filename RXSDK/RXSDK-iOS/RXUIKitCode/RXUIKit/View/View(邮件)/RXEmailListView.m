...
@interface RXEmailListView ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation RXEmailListView
...
- (void)setDeleBtnEnable:(BOOL)enable{
    ...
    if (enable) {
        [self.deleteBtn setImage:[UIImage rxBundleImageNamed:@"rx_deleteEmail"] forState:UIControlStateNormal];
    }else{
        [self.deleteBtn setImage:[UIImage rxBundleImageNamed:@"rx_deleteEmail_disable"] forState:UIControlStateNormal];
    }
}
...
{
    RXEmailListViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
    if (!cell) {
        cell = [[RXEmailListViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"cell"];
    }
    ...
    if (self.emailListArray.count > 0) {
        ...
        if ([dic[@"status"] integerValue] == 3) {//未读
            cell.customImageView.image = [UIImage rxBundleImageNamed:@"rx_mailUnRead"];
            cell.titleLabel.text = dic[@"title"];
            cell.titleLabel.font = [UIFont boldSystemFontOfSize:RXUScaleWidth(14)];
            cell.timeLabel.text = dic[@"send_time"];
            cell.timeLabel.font = [UIFont boldSystemFontOfSize:RXUScaleWidth(12)];
        }else{
            cell.customImageView.image = [UIImage rxBundleImageNamed:@"rx_mailRead"];
            cell.titleLabel.text = dic[@"title"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.emailListArray.count > 0) {
        NSDictionary *dic = self.emailListArray[indexPath.row];
        RXEmailDetailView *view = [[RXEmailDetailView alloc] initWithCpUserId:self.cpUserId emailId:dic[@"rx_mail_id"]];
    }
}
...
- (UIButton *)deleteBtn{
    if (!_deleteBtn) {
        ...
        [_deleteBtn setImage:[UIImage rxBundleImageNamed:@"rx_deleteEmail"] forState:UIControlStateNormal];
...
- (UIButton *)oneClickReceiveBtn{
    if (!_oneClickReceiveBtn) {
        ...
        [_oneClickReceiveBtn setTitle:@"一键领取" forState:UIControlStateNormal];
