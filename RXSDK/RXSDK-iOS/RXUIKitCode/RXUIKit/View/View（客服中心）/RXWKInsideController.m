...
@interface RXWKInsideController ()
...
@implementation RXWKInsideController
...
- (void)viewDidLoad {
    ...
    if (!_webView) {
        ...
        _webView = [[RXInsideWebView alloc] initWithFrame:window.frame];
    ...
    if (self.type == InsideServiceType_center) {
...
//    self.webView.urlStr = [NSString stringWithFormat:@"%@static/passport/#/userCenter", domain];
    if (self.type == InsideServiceType_chat) {
