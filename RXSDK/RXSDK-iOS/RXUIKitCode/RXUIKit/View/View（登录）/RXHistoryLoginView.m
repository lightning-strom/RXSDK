...
@interface RXHistoryLoginView () <RXAttributeLabelDelegate>
...
@implementation RXHistoryLoginView
...
//        self.addAccountBlock = addAccount;
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(authLoginAction:) name:RXUINoti_authLogin object:nil];
    }
    return self;
}
...
- (void)authLoginAction:(NSNotification *)noti
{
    NSDictionary *userInfo = noti.userInfo;
    
    [[RXUIKitService sharedSDK] setNormalLoginViewWithConfig:self.loginConfig isAuth:YES loginEvent:self.loginTypeBlock complete:self.loginComplete];
//    self.loginConfig.loginViewType = 1;
//    RXLoginView *loginView = [[RXLoginView alloc] initWithConfig:self.loginConfig loginEvent:self.loginTypeBlock complete:self.loginComplete];
//    loginView.username = userInfo[@"username"];
}
...
- (void)layoutViews
{
    ...
    _bgView.frame = CGRectMake(0, 0, [RXUICommonTool getScreenWidth], RXAC ? 224 : 248);
...
//    _priLbl.frame = CGRectMake(CGRectGetMaxX(_selectBtn.frame), CGRectGetMaxY(self.accountView.frame) + 13, 233, 40);
    ...
    _moreLoginBtn.frame = CGRectMake(CGRectGetWidth(_bgView.frame) / 2 - 66, RXAC ? 171 : 195, 132, 34);
    ...
    // 底部快速登录按钮
//    RXQuickLoginView *quickLoginView = [[RXQuickLoginView alloc] initWithLoginConfig:self.loginConfig viewType:RXUserType_history loginEvent:self.loginTypeBlock complete:self.loginComplete];
...
- (UIView *)setAccountView
{
    ...
    leftImgView.image = [UIImage rxBundleImageNamed:[RXUICommonTool getIconWithLoginType:[userInfo[@"loginType"] integerValue]]];
    [accountView addSubview:leftImgView];
    
    UILabel *titleLbl = [[UILabel alloc] init];
    titleLbl.tag = 90001;
    titleLbl.text = userInfo[@"nickname"];
    
    long loginType = [userInfo[@"loginType"] longValue];
    if (loginType == 1 || loginType == 10) {    
        if ([RXUICommonTool validateMobile:userInfo[@"nickname"]]) {
            titleLbl.text = [RXUICommonTool usernameSec:userInfo[@"nickname"]];
        }
    }
    ...
    rightImgView.image = [UIImage rxBundleImageNamed:@"rx_login_change"];
...
{
    RXHistoryListLoginView *historyListView = [[RXHistoryListLoginView alloc] initWithLoginConfig:self.loginConfig viewType:0 selectAccount:^(NSMutableDictionary * _Nonnull userInfo) {
        ...
        for (UIView *subView in self.accountView.subviews) {
            if (subView.tag == 90000) {
                ...
                imgView.image = [UIImage rxBundleImageNamed:[RXUICommonTool getIconWithLoginType:[userInfo[@"loginType"] integerValue]]];
            }
            if (subView.tag == 90001) {
                UILabel *label = (UILabel *)subView;
                label.text = userInfo[@"nickname"];
                if ([RXUICommonTool validateMobile:userInfo[@"nickname"]]) {
                    label.text = [RXUICommonTool usernameSec:userInfo[@"nickname"]];
                }
            }
        }
    }];
...
//                }
//
//                [[RXLoginViewManager sharedSDK] loginWithLoginType:[weakSelf.selectUserInfo[@"loginType"] longValue] loginInfo:weakSelf.selectUserInfo complete:weakSelf.loginComplete];
//            }
//        };
//    } else {
        NSDictionary *loginExt = [NSDictionary dictionary];
        [RXUIUserUtility sharedManager].username = self.selectUserInfo[@"username"];
        if (self.loginTypeBlock) {
            ...
            [[RXLoginViewManager sharedSDK] loginWithLoginType:[self.selectUserInfo[@"loginType"] longValue] loginInfo:self.selectUserInfo complete:self.loginComplete];
...
- (UIButton *)loginBtn
{
    if (!_loginBtn) {
        ...
        [_loginBtn setTitle:@"开始游戏" forState:UIControlStateNormal];
