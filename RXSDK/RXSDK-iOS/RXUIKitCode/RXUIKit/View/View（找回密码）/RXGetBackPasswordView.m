...
@interface RXGetBackPasswordView () <UITextFieldDelegate, UITextViewDelegate>
...
- (void)confimBtnAction
{
    ...
    if (self.type == GetBackPasswordType_code) {
        ...
        if ([RXUICommonTool validateEmail:self.passwordTF1.tf.text]) {
        ...
        if (![self.passwordTF1.tf.text isEqual:self.passwordTF2.tf.text]){
            [RXHUD showErrorText:@"两次密码输入不一致，请重新输入"];
            return;
        }
        if (![RXUICommonTool checkPasswordWithPwd:self.passwordTF1.tf.text]) {
            [RXHUD showErrorText:msg];
            return;
        }
...
//                        [SVProgressHUD showSuccessWithStatus:@"修改成功"];
                        ...
                        RXUCommonView *successV = [[RXUCommonView alloc] initWithDesStr:@"密码重置成功" title:@"重置密码" image:@"rx_common_success" complete:^{
//                            if (self.doneBlock) {
...
//                            [[NSNotificationCenter defaultCenter] postNotificationName:@"test" object:nil userInfo:@{@"username" : self.phoneStr}];
                        ...
                        [RXUICommonTool saveAccountWithUsername:self.phoneStr password:self.passwordTF1.tf.text];
...
- (void)codeAction
{
    ...
    if (self.type == GetBackPasswordType_code) {
        ...
        if (account_type == 0 || account_type == 1) {
            if (![RXUICommonTool validateMobile:self.passwordTF1.tf.text]) {
...
- (RXTextField *)passwordTF1
{
    if (!_passwordTF1) {
        ...
        if (self.type == GetBackPasswordType_code) {
            ...
            placeStr = @"密码输入6-32位，包含数字+字母+特殊符号";
            // 是否开启简单密码
            RXPasswordStrength passwordType = [RXUIUserUtility sharedManager].passwordType;
            if (passwordType == Default || passwordType == Average) {
                placeStr = @"请输入6-32位密码";
        ...
        _passwordTF1 = [[RXTextField alloc] initWithPlaceholder:placeStr type:tfType keyboardType:keyboardType];
...
//        } else {
            imageStr = @"rx_back";
//        }
        [_closeBtn setImage:[UIImage rxBundleImageNamed:imageStr] forState:UIControlStateNormal];
