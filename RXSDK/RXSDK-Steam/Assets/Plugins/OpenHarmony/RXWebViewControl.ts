...
export class RXWebViewControl {
  public close(): void {
    workerPort.postMessage({ type: "RXWebViewControl", funcName: "Close" });
  }

  public openUrl(url: string, initParam: string, isLight?: boolean, title?: string): void {
    workerPort.postMessage({
      type: "RXWebViewControl",
      funcName: "OpenUrl",
      url: url,
      initParam: initParam,
      title: title,
  ...
  public runJavaScript(script: string): void {
    workerPort.postMessage({ type: "RXWebViewControl", funcName: "RunJavaScript", script: script });
  }
