...
import com.ruixue.websocket.util.AppUtils;
import com.ruixue.websocket.util.ThreadUtils;
import com.ruixue.websocket.util.WsLogUtil;
...
//        }
    ...
    private void addClient(WsClient wsClient) {
        if (clientMap.containsKey(wsClient.getWsKey())) {
            Objects.requireNonNull(clientMap.get(wsClient.getWsKey())).close();
...
//        registerNetworkChangedReceiver();
        for (Map.Entry<String, WsClient> wsClientEntry : clientMap.entrySet()) {
            try {
                WsClient ws = wsClientEntry.getValue();
                if (ws.isOpen()) {
                ...
                e.printStackTrace();
                clientMap.remove(wsClientEntry.getKey());
            }
        }
    }
