...
public class GroupDecoration extends RecyclerView.ItemDecoration {
    ...
    public GroupDecoration(Context context) {
        ...
        mItemHeaderHeight = AppUtils.dp2px(context, 40);
        mTextPaddingLeft = AppUtils.dp2px(context, 6);
        mDp1 = AppUtils.dp2px(context, 1);
        mTextRect = new Rect();
        ...
        mTextPaint.setColor(Color.parseColor("#A5CDCB"));

        mLinePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mLinePaint.setColor(Color.parseColor("#E2F2F1"));
    }
        ...
        if (parent.getAdapter() instanceof GroupAdapter) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (isHeader) {
                    ...
                    c.drawText(adapter.getGroupName(position), mTextPaddingLeft, (view.getTop() - mItemHeaderHeight) + (mItemHeaderHeight / 2.0f) + mTextRect.height() / 2.0f, mTextPaint);
                    c.drawRect(0, view.getTop() - mDp1, parent.getWidth(), view.getTop(), mLinePaint);
//                    RXLogger.i(" 0," + (view.getTop() - mDp1) + ", " + parent.getWidth() + ", " + view.getTop());
                }
//                else {
...
//                }
    ...
    public void onDrawOver(@NonNull Canvas c, @NonNull RecyclerView parent, @NonNull RecyclerView.State state) {
        if (parent.getAdapter() instanceof GroupAdapter) {
            ...
            if (position >= 0) {
                View view = Objects.requireNonNull(parent.findViewHolderForAdapterPosition(position), " Null error findViewHolderForAdapterPosition " + position).itemView;
                ...
                if (isHeader) {
                    ...
                    mTextPaint.getTextBounds(adapter.getGroupName(position), 0, adapter.getGroupName(position).length(), mTextRect);
                    c.drawText(adapter.getGroupName(position), mTextPaddingLeft, mItemHeaderHeight / 2f + mTextRect.height() / 2f - (mItemHeaderHeight - bottom), mTextPaint);
                } else {
                    c.drawRect(0, 0, parent.getWidth(), mItemHeaderHeight, mItemHeaderPaint);
                    mTextPaint.getTextBounds(adapter.getGroupName(position), 0, adapter.getGroupName(position).length(), mTextRect);
                    c.drawText(adapter.getGroupName(position), mTextPaddingLeft, mItemHeaderHeight / 2f + mTextRect.height() / 2f, mTextPaint);
                    c.drawRect(0, mItemHeaderHeight - mDp1, parent.getWidth(), mItemHeaderHeight, mLinePaint);
                 }
            } else {
                Log.i("rxsdk", "findFirstVisibleItemPosition " + position);
            }
        }
    }
}
