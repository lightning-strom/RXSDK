 ...
 */
  class JDJH {
    ...
    public static void doPay(Activity activity, @NonNull JSONObject jsonObject, RXJSONCallback callback) {
        ...
        try {
            String returnUrl = ObjectUtils.requireNonNull(extObj).optString("ReturnUrl");
            if (TextUtils.isEmpty(returnUrl)) {
                JSONObject result = ObjectUtils.requireNonNull(extObj.optJSONObject("Result"));
                callback.onFailed(JSONUtil.toJSONObject(RXErrorCode.HQ_DATA_ERROR.getValue(), result.optString("code") + " " + result.optString("desc")));
            } else {
                doPay(activity, returnUrl, jsonObject.optBoolean("openBrowser"), callback);
            }
        } catch (Exception e) {
            RXLogger.e("调起失败:" + e.getClass());
            e.printStackTrace();
            ...
            if (openBrowser) {
                ...
                WebViewClient wxWebClient = new WebViewClient() {
                    ...
                    public boolean shouldOverrideUrlLoading(WebView view, String url) {
                        ...
                        if (url.startsWith("openapp.jdmobile://virtual?") || !url.startsWith("http")) {
                            if (callback != null && isHandleCallback.compareAndSet(false, true)) {
