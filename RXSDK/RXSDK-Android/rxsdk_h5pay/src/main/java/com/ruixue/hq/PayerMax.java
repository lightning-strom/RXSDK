 ...
 */
  class PayerMax {
    ...
    private static class PayerMaxBean {
        /**
         * 商户订单号
         */
        String outTradeNo;
    ...
    public static void doPay(Activity activity, @NonNull JSONObject jsonObject, RXJSONCallback callback) {
        try {
            ...
            PayerMaxBean payerMaxBean = new Gson().fromJson(extObj.toString(), PayerMaxBean.class);
            doPay(activity, payerMaxBean.redirectUrl, jsonObject.optBoolean("openBrowser"), callback);
    ...
    private static void doPay(Activity activity, String payUrl, boolean openBrowser, RXJSONCallback callback) {
//        payUrl = "https://jdpaycert.jd.com/pay/initSdkSuccessNew?token=ab6134ea044f45aea322757a2f0f114b";
                ...
                WebViewClient wxWebClient = new WebViewClient() {
                    ...
                    public boolean shouldOverrideUrlLoading(WebView view, String url) {
                        ...
                        if (!url.startsWith("http")) {
                            if (callback != null && isHandleCallback.compareAndSet(false, true)) {
                                try {
                                    JSONObject data = URLHelper.queryToJSONObject(Uri.parse(url).getQuery());
                                    String status = data.optString("status", "");
                                    int code = status.equalsIgnoreCase("SUCCESS") ? RXErrorCode.SUCCESS.getValue() : RXErrorCode.HQ_DATA_ERROR.getValue();
                                    JSONObject jsonObject = JSONUtil.toJSONObject(code, "please queries the payment result from server.");
