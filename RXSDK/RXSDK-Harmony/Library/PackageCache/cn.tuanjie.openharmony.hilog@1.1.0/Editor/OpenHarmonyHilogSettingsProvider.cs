...
{
    class OpenHarmonyHilogSettingsProvider : SettingsProvider
    {
        internal static readonly string kSettingsPath = "Preferences/Analysis/OpenHarmony Hilog Settings";
        ...
        internal class Styles
        {
            ...
            public static GUIContent requestIntervalMS = new GUIContent("Request Interval ms",
                $"How often to request memory dump from the device? The minimum value is {OpenHarmonyHilogSettings.kMinMemoryRequestIntervalMS} ms");
        ...
        private OpenHarmonyHilogSettings Settings => m_Runtime.Settings;


        public OpenHarmonyHilogSettingsProvider(string path, SettingsScope scope)
            : base(path, scope)
        {
        ...
        [SettingsProvider]
        public static SettingsProvider CreateOpenHarmonyHilogSettingsProvider()
        {
            if (!OpenHarmonyBridge.OpenHarmonyExtensionsInstalled)
                return null;
            var provider = new OpenHarmonyHilogSettingsProvider(kSettingsPath, SettingsScope.User);
            return provider;
        }
    }
}
