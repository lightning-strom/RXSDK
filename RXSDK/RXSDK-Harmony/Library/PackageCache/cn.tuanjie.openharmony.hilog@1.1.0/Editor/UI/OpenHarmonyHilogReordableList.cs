...
namespace Tuanjie.OpenHarmony.Hilog
{
    ...
    internal class OpenHarmonyHilogReordableList
    {
        ...
        public OpenHarmonyHilogReordableList(List<ReordableListItem> dataSource)
        {
            ShowEntryGUI = true;
        ...
        void DoListGUI()
        {
            ...
            for (int i = 0; i < m_DataSource.Count; i++)
            {
                ...
                EditorGUILayout.BeginHorizontal();
                var toggleRect = GUILayoutUtility.GetRect(GUIContent.none, OpenHarmonyHilogStyles.tagToggleStyle);
                GUILayout.Space(4);
                var nameRect = GUILayoutUtility.GetRect(GUIContent.none, OpenHarmonyHilogStyles.tagEntryStyle, GUILayout.ExpandWidth(true));
                ...
                if (currentEvent.type == EventType.Repaint)
                {
                    if (m_SelectedIndex == i)
                        OpenHarmonyHilogStyles.tagEntryBackground.Draw(entryRect, false, false, true, false);
                    else
                    {
                        if (i % 2 == 0)
                            OpenHarmonyHilogStyles.tagEntryBackgroundEven.Draw(entryRect, false, false, false, false);
                        else
                            OpenHarmonyHilogStyles.tagEntryBackgroundOdd.Draw(entryRect, false, false, false, false);
                    }
                }

                EditorGUI.BeginChangeCheck();
                item.Enabled = GUI.Toggle(toggleRect, item.Enabled, GUIContent.none, OpenHarmonyHilogStyles.tagToggleStyle);
                if (EditorGUI.EndChangeCheck())
                    m_SelectedIndex = i;
                GUI.Label(nameRect, item.Name, OpenHarmonyHilogStyles.tagEntryStyle);
        ...
        {
            m_DataSource.Insert(0, new ReordableListItem() { Name = name, Enabled = true });
        ...
        void DoButtonsGUI()
        {
            ...
            if (GUILayout.Button(OpenHarmonyHilogStyles.kIconToolbarUp, ButtonStyles))
            {
            ...
            if (GUILayout.Button(OpenHarmonyHilogStyles.kIconToolbarDown, ButtonStyles))
            {
            ...
            if (ShowResetGUI)
            {
                if (GUILayout.Button(OpenHarmonyHilogStyles.kIconReset, ButtonStyles))
                {
        ...
        protected void DoEntryGUI()
        {
            ...
            CurrentItemName = EditorGUILayout.TextField(CurrentItemName, GUILayout.Height(OpenHarmonyHilogStyles.kTagEntryFixedHeight + 2));
        }
