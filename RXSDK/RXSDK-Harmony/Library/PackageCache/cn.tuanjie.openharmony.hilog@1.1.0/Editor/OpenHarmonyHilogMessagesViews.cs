...
{
    internal partial class OpenHarmonyHilogConsoleWindow
    {
        ...
        {
            public HilogEntry[] SelectedEntries;
            public HilogEntry TagProcessIdEntry;
        }
        ...
        private GUIContent kScroll;
        private Vector2 m_ScrollPosition = Vector2.zero;
        private float m_MaxLogEntryWidth = 0.0f;
        private float m_TagPriorityErrorHeight = 0.0f;
        private string m_DisplayNamePriority;
        private float doubleClickStart = -1;

        private static readonly List<HilogEntry> kNoEntries = new List<HilogEntry>();
        private Dictionary<string, Priority> m_TagPriorityOnDevice;
        private ScrollData m_ScrollData;
        private AutoScroll m_ScrollMode;

        public IReadOnlyList<HilogEntry> FilteredEntries
        {
            get
            {
                if (m_Hilog == null)
                    return kNoEntries;
                return m_Hilog.FilteredEntries;
            }
        }
        ...
        public IReadOnlyList<HilogEntry> GetSelectedFilteredEntries(out int minIndex, out int maxIndex)
        {
            ...
            return m_Hilog.GetSelectedFilteredEntries(out minIndex, out maxIndex);
        }

        public IReadOnlyList<HilogEntry> SelectedFilteredEntries => GetSelectedFilteredEntries(out var minIndex, out var maxIndex);

        void CollectTagPrioritiesFromDevice(IOpenHarmonyHilogDevice device)
        {
            m_TagPriorityOnDevice.Clear();
            var d = device;
            //use || to replace &&, there was a bug before?
            if (d == null || d.State != IOpenHarmonyHilogDevice.DeviceState.Connected)
                return;

            var tags = m_Runtime.UserSettings.Tags.GetSelectedTags(true);
            if (tags == null || tags.Length == 0)
                tags = OpenHarmonyHilogTags.DefaultTagNames;
        ...
        private bool DoGUIHeader()
        {
            ...
            var fullHeaderRect = GUILayoutUtility.GetRect(GUIContent.none, OpenHarmonyHilogStyles.columnHeader, GUILayout.ExpandWidth(true));
            ...
            foreach (var c in (Column[])Enum.GetValues(typeof(Column)))
            {
                ...
                if (d.width > 0.0f)
                {
                    ...
                    GUI.Label(buttonRect, d.content, OpenHarmonyHilogStyles.columnHeader);
            ...
            if (!headerDrawn)
            {
                ...
                GUI.Label(fullHeaderRect, GUIContent.none, OpenHarmonyHilogStyles.columnHeader);
            }
            else if (!lastHeaderDrawn)
            {
                ...
                GUI.Label(buttonRect, GUIContent.none, OpenHarmonyHilogStyles.columnHeader);
            }
        ...
        private bool DoGUITagsValidation(IOpenHarmonyHilogDevice device)
        {
            if (!OpenHarmonyHilogSessionSettings.ShowTagPriorityErrors)
                return false;
            ...
            {
                if (t.Value == Priority.Debug)
                    continue;
                message.AppendLine($" Tag '{t.Key}' has priority '{t.Value}'");
                fixCommand.AppendLine($"  HDC shell setprop log.tag.{t.Key} {Priority.Debug}");
            }
            ...
            if (GUILayout.Button(new GUIContent("Fix Me", $"The following commands will be executed:\n{fixCommand}"), opts) && d != null)
            {
                foreach (var t in m_TagPriorityOnDevice)
                {
                    if (t.Value == Priority.Debug)
                        continue;
                    d.SetTagPriority(t.Key, Priority.Debug);
                    result = true;
                }
            }
            if (GUILayout.Button(new GUIContent("Hide", "Hide the error in this Editor session."), opts))
                OpenHarmonyHilogSessionSettings.ShowTagPriorityErrors = false;
            EditorGUILayout.EndVertical();
            EditorGUILayout.EndHorizontal();
            if (result)
                CollectTagPrioritiesFromDevice(device);
            return result;
        }
        ...
        private void PrioritySelection(object userData, string[] options, int selected)
        {
            ...
            m_DisplayNamePriority = string.Format("Priority: {0}", ((Priority)selected).ToString());
        }
        ...
        private void DoIconLogEntryItem(Rect fullView, int index, Column column, string value, GUIStyle style, Vector2 iconSize)
        {
            ...
            var entryHeight = OpenHarmonyHilogStyles.kLogEntryFixedHeight;
        ...
        private void DoLogEntryItem(Rect fullView, int index, Column column, string value, GUIStyle style)
        {
            ...
            var itemRect = Columns[(uint)column].itemSize;
            var rc = new Rect(itemRect.x + kMessageMargin, fullView.y + OpenHarmonyHilogStyles.kLogEntryFixedHeight * index, itemRect.width - kMessageMargin, itemRect.height);
            style.Draw(rc, new GUIContent(value), 0);
        }
        ...
        private GUIStyle GetIconStyle(Priority priority)
        {
            ...
            {
                case Priority.Warn:
                    return OpenHarmonyHilogStyles.warningSmallStyle;
                case Priority.Error:
                case Priority.Fatal:
                    return OpenHarmonyHilogStyles.errorSmallStyle;
                default:
                    return OpenHarmonyHilogStyles.infoSmallStyle;
            }
        }

        private bool DoGUIEntries(IOpenHarmonyHilogDevice device)
        {
            bool requestRepaint = false;
            var e = Event.current;

            var visibleWindowRect = GUILayoutUtility.GetRect(GUIContent.none, OpenHarmonyHilogStyles.priorityDefaultStyle, GUILayout.ExpandWidth(true), GUILayout.ExpandHeight(true));
            var totalWindowRect = visibleWindowRect;
            var maxVisibleItems = (int)(visibleWindowRect.height / OpenHarmonyHilogStyles.kLogEntryFixedHeight);
            ...
            totalWindowRect.height = OpenHarmonyHilogStyles.kLogEntryFixedHeight * (FilteredEntries.Count + kExtraMessageCount);
            ...
            if (m_ScrollMode == AutoScroll.Auto)
            {
            ...
            for (int i = startItem; i - startItem < maxVisibleItems && i < FilteredEntries.Count; i++)
            {
                ...
                var selectionRect = new Rect(visibleWindowRect.x, visibleWindowRect.y + OpenHarmonyHilogStyles.kLogEntryFixedHeight * i, totalWindowRect.width, OpenHarmonyHilogStyles.kLogEntryFixedHeight);

                if (e.type == EventType.Repaint)
                {
                    var le = FilteredEntries[i];
                    if (selected)
                        OpenHarmonyHilogStyles.background.Draw(selectionRect, false, false, true, false);
                    else
                    {
                        if (i % 2 == 0)
                            OpenHarmonyHilogStyles.backgroundEven.Draw(selectionRect, false, false, false, false);
                        else
                            OpenHarmonyHilogStyles.backgroundOdd.Draw(selectionRect, false, false, false, false);
                    }
                    var style = OpenHarmonyHilogStyles.priorityStyles[(int)le.priority];
                    DoIconLogEntryItem(visibleWindowRect, i, Column.Icon, "", GetIconStyle(le.priority), OpenHarmonyHilogStyles.kSmallIconSize);
                    DoLogEntryItem(visibleWindowRect, i, Column.Time, le.dateTime.ToString(HilogEntry.s_TimeFormat), style);
                    ...
                    m_MaxLogEntryWidth = Mathf.Max(m_MaxLogEntryWidth,
                        OpenHarmonyHilogStyles.priorityDefaultStyle.CalcSize(new GUIContent(le.message)).x + Columns[(int)Column.Message].itemSize.x);
                }
                else
                {
                    requestRepaint |= DoMouseEventsForLogEntry(selectionRect, i, selected, controlId, device);
                }
            }
        ...
        private void DoMouseSelection(Event e, int logEntryIndex, bool isLogEntrySelected, int keyboardControlId)
        {
            ...
            else
            {
                ...
                {
                    // Curious behavior with right click. In Tuanjie if you right click on already selected item which is a part of selection list, it doesn't deselect other items
                    // But if you right click on unselected item, the selection list will be cleared
                    if (e.button == 0 ||
                        (e.button == 1 && !entry.Selected))
                    {
                        m_Hilog?.ClearSelectedEntries();
                        entry.Selected = true;
                    }
        ...
        void DoContextMenu(Event e, int logEntryIndex, IOpenHarmonyHilogDevice device)
        {
            var entries = SelectedFilteredEntries;
            var contextMenu = new OpenHarmonyContextMenu<MessagesContextMenu>();
            ...
            if (entries.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(tag))
                {
                    ...
                    var fixedTag = OpenHarmonyHilogUtilities.FixSlashesForIMGUI(tag);
                    contextMenu.Add(MessagesContextMenu.AddTag, $"Add tag '{fixedTag}'", false, IsHilogConnected);
                    contextMenu.Add(MessagesContextMenu.RemoveTag, $"Remove tag '{fixedTag}'", false, IsHilogConnected);
                }
                ...
                {
                    contextMenu.AddSplitter();
                    contextMenu.Add(MessagesContextMenu.FilterByProcessId, $"Filter by process id '{processId}'", false, IsHilogConnected);
                    ...
                    //{
                    //    contextMenu.Add(MessagesContextMenu.SendUnixSignal, $"{prefix}Send Unix signal/{signal} ({(int)signal})", false, IsHilogConnected,
                    //        new KeyValuePair<int, PosixSignal>(processId, signal));
                    //}

                    //contextMenu.Add(MessagesContextMenu.CrashProcess, $"{prefix}Crash", false, IsHilogConnected, processId);
                    contextMenu.Add(MessagesContextMenu.ForceStop, $"{prefix}Force Stop", false, IsHilogConnected, processId);
            ...
            contextMenu.Show(e.mousePosition, MenuSelection, device);
        }

        private void MenuSelection(object userData, string[] options, int selected)
        {
            var contextMenu = (OpenHarmonyContextMenu<MessagesContextMenu>)userData;
            var device = contextMenu.m_SelectedDevice;
            ...
            switch (item.Item)
            {
                ...
                case MessagesContextMenu.SelectAll:
                    m_Hilog?.SelectAllFilteredEntries();
                    break;
                // Save to File
                ...
                case MessagesContextMenu.FilterByProcessId:
                    ...
                    break;
                // Force stop ability by process id
                case MessagesContextMenu.ForceStop:
                    device.AbilityManager.StopProcess((int)item.UserData);
                    break;
            }
        }

        private bool DoMouseEventsForLogEntry(Rect logEntryRect, int logEntryIndex, bool isLogEntrySelected, int keyboardControlId, IOpenHarmonyHilogDevice device)
        {
            bool requestRepaint = false;
            var e = Event.current;
            if (e.type == EventType.MouseDown && logEntryRect.Contains(e.mousePosition))
            {
                // Selection occurs both with Left Click & and Right click, this happens in all Tuanjie windows.
            ...
            if (e.type == EventType.MouseUp && logEntryRect.Contains(e.mousePosition))
            {
                if (e.button == 1)
                {
                    DoContextMenu(e, logEntryIndex, device);
                    requestRepaint = true;
                    e.Use();
                }
            }
        ...
        private void DoNavigation(int direction, bool shiftPressed)
        {
            if (m_Hilog == null)
                return;
            if (m_Hilog.FilteredEntries.Count == 0)
                return;
            ...
            {
                case -1:
                    selectedItem = selectedEntries.Count == 0 ? m_Hilog.FilteredEntries.Count - 1 : Math.Max(0, minIdx - 1);
                    break;
                case 1:
                    selectedItem = selectedEntries.Count == 0 ? 0 : Math.Min(m_Hilog.FilteredEntries.Count - 1, maxIdx + 1);
                    break;
                default:
            ...
            if (shiftPressed)
            {
                for (int i = minIdx; i <= maxIdx; i++)
                    m_Hilog.FilteredEntries[i].Selected = true;
            }
            else
            {
                m_Hilog.ClearSelectedEntries();
            ...
            m_Hilog.FilteredEntries[selectedItem].Selected = true;
            m_ScrollMode = AutoScroll.Disabled;
        ...
        private void DoScrollOptionsGUI()
        {
            ...
            EditorGUI.BeginChangeCheck();
            var scroll = (AutoScroll)EditorGUILayout.EnumPopup(m_ScrollMode, GUILayout.ExpandWidth(false));
        ...
        public bool DoMessageView(IOpenHarmonyHilogDevice device)
        {
            var repaint = DoGUIHeader();
            repaint |= DoGUITagsValidation(device);
            repaint |= DoGUIEntries(device);
            return repaint;
        }

        private void SaveToFile(IEnumerable<HilogEntry> logEntries)
        {
            var contents = LogEntriesToString(logEntries);
            var filePath = EditorUtility.SaveFilePanel("Save selected logs", "", PlayerSettings.applicationIdentifier + "-hilog", "txt");
            if (!string.IsNullOrEmpty(filePath))
                File.WriteAllText(filePath, contents);
        }

        private string LogEntriesToString(IEnumerable<HilogEntry> entries)
        {
            ...
            foreach (var l in entries)
            {
                ...
                for (int i = 0; i < Columns.Length; i++)
                {
                    ...
                    switch ((Column)i)
                    {
                        case Column.Time: entry += l.dateTime.ToString(HilogEntry.s_TimeFormat); break;
        ...
        private void TryToOpenFileFromLogEntry(HilogEntry entry)
        {
