...
namespace Tuanjie.OpenHarmony.Hilog
{
    ...
    class HilogFilterOptions : FilterOptions
    {
        private Regex m_CachedRegex;

        public HilogFilterOptions(FilterOptions options)
            : base(options)
        {
        }
        ...
        protected override void OnUpdate()
        {
            if (m_UseRegularExpressions)
            {
                ...
                {
                    OpenHarmonyHilogInternalLog.Log($"Input search filter '{m_Filter}' is not a valid regular expression.\n{ex}");
