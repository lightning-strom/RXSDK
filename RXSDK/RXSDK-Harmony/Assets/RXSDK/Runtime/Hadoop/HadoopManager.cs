...
namespace RXSDK
{
    class HadoopManager : MonoBehaviour
    {
        ...
        private static HadoopManager sInstance;
        public static HadoopManager Instance { get { return sInstance; } }
        ...
        public bool Track(MonoBehaviour mono, string eventName, Dictionary<string, object> properties, string distinctId = null, string type = null, bool reportAtTime = false)
        {
            ...
            if (string.IsNullOrEmpty(SDKConfig.Instance.CpRoleId))
            {
                properties["#role_id"] = SDKConfig.Instance.CpRoleId;
            }

            if (string.IsNullOrEmpty(SDKConfig.Instance.RegionTag))
            {
                properties["rx_region_tag"] = SDKConfig.Instance.RegionTag;
            }


            if (PlatformProvider.Current.SupportsNativeTrack)
            {
                PlatformProvider.Current.TrackData(RXUtility.ObjectToJson(new TrackDataArgs
                {
            ...
            properties["sdk_version"] = SDKConfig.Instance.SDK_VERSION;
            var hashMap = GetBody(eventName, distinctId, type);
        ...
        {
            if (!RuiXueSdk.IsInited) return false;
