...
namespace RXSDK
{
    public class RXWebView
    {
        ...
        public RXWebView()
        {
            var c = new OpenHarmonyJSClass("RXWebViewControlBridge");
            webviewControlBridge = c.CallStatic<OpenHarmonyJSObject>("getInstance");
        }

        public void CreateWebview(int ml, int mt, int mr, int mb, bool visible)
        {
            var cb = new OpenHarmonyJSCallback(WebviewCallBack);
            webviewControlBridge.Call("CreateWebview", ml, mt, mr, mb, visible, cb);
        }

        public void CreateWebview() => CreateWebview(0, 0, 0, 0, true);
        ...
        {
            if (attached && !string.IsNullOrEmpty(url))
            {
        ...
        public void Reload() => webviewControlBridge.Call("Reload");
        public void StopLoading() => webviewControlBridge.Call("StopLoading");
        public void GoForward() => webviewControlBridge.Call("GoForward");
        public void GoBack() => webviewControlBridge.Call("GoBack");
    }
}
#endif