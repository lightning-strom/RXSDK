...
export class RXWebViewControl {
  public close(): void {
    workerPort.postMessage({ type: "RUN_ON_UI_THREAD_JS", funcName: "WebMessageHandler.Close", args: [] });
  }

  public openUrl(url: string, initParam: string, isLight?: boolean, title?: string): void {
    workerPort.postMessage({
      type: "RUN_ON_UI_THREAD_JS",
      funcName: "WebMessageHandler.OpenUrl",
      args: [{
        url: url,
        initParam: initParam,
        title: title,
  ...
  public runJavaScript(script: string): void {
    workerPort.postMessage({ type: "RUN_ON_UI_THREAD_JS", funcName: "WebMessageHandler.RunJavaScript", args: [script] });
  }
