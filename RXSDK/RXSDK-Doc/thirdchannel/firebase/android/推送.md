...
`RxPushManager` 是瑞雪推送在 Android 平台的接口类。该类提供了一系列静态方法（static）接口，无需实例化直接调用。

:::tip
使用瑞雪 **推送自定义透传参数（deeplink）** 功能获取参数参考下方 `parseDeeplink` 示例函数。
:::
...
```java
 /**
     *  使用瑞雪推送自定义透传参数的解析示例,客户端解析通过通知点击携带的服务端参数。
...
::: tip
如果通过瑞雪集成 firebase 推送，调用上面的瑞雪 API 即可完成
...
具体配置，参照官网。
::: tip
firebase-messaging-directboot 无需自行引入，瑞雪SDK已包含
:::
