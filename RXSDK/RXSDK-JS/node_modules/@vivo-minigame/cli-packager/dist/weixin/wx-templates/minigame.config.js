...
module.exports = function (options) {
  /**
   * 1、externals：使用 externals方式引入模块，请参考webpack externals的用法 https://webpack.js.org/configuration/externals/
   *   /**
   *    * externals外部依赖配置
   ...
   * 2、devtoolConfig：Webpack配置文件的devtool项，可支持除eval以外的项，默认使用cheap-module-source-map。https://webpack.js.org/configuration/devtool/#root
   * 3、fflateEnable：zip打包模式，默认使用fflate进行打包，若打包速度较慢，可尝试设置为false，则使用JSZip进行打包
   * 4、enablePluginPack：提升debug模式下的打包速度，默认为false，debug模式下不将【插件包】进行打包，若需要测试插件包功能，则需要设置为true
   * 5、enableSinglePack：提升debug模式下的打包速度，默认为false，debug模式下不将【原整包】进行打包，若需要测试原整包，则需要设置为true
   *  */

  const externals = []
  const devtoolConfig = 'cheap-module-source-map'
  const fflateEnable = true
  const enablePluginPack = true
  const enableSinglePack = false
