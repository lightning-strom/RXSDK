...
const a=_path.default.parse(f);h=_path.default.posix.join(a.dir,a.name),j=f}b[h]=i,c[a]=`commonjs ${j}`}),{isMain:e,externals:c,entry:b,resConfig:d}}function filter(a=[]){return a.filter(({name:a,root:b})=>{if(!a)return(0,_cliSharedUtils.info)("\u5206\u5305\u914D\u7F6E\u4E2Dname\u4E3A\u7A7A\u7684\u5206\u5305\uFF0C\u5DF2\u7ECF\u88AB\u8FC7\u6EE4\uFF01"),!1;if(!b)return(0,_cliSharedUtils.info)(`分包配置中name=${a}的分包，root不存在，已经被过滤！`),!1;// 检查 root 值对应的目录是否存在
const c=_path.default.join(paths.SRC,b);return _cliSharedUtils.fs.existsSync(c)?!_cliSharedUtils.fs.statSync(c).isDirectory()||_cliSharedUtils.fs.existsSync(_path.default.join(c,constanst.ENTRY))||((0,_cliSharedUtils.info)(`分包配置name=${a}的分包，在${c}目录下不存在${constanst.ENTRY}，已经被过滤！`),!1):((0,_cliSharedUtils.info)(`分包配置name=${a}的分包，在${paths.SRC}目录下不存在${c}，已经被过滤！`),!1)})}