...
__webpack_require__.webpackContext = webpackContext`
  }
...
module.exports = webpackAsyncContext;`
  ...
  getSourceForEmptyContext (id, runtimeTemplate) {
    ...
    function webpackEmptyContext(req) {
      console.warn('webpackEmptyContext req:', req);
      try {
        while(req.startsWith('/')){
          req = req.substring(1)
        }
        if (global && global.webpack_use_moudle){
          let id = global.webpack_use_moudle[req]
          if (!id){
            const arr = req.startsWith('./') ? req.substring(2) : req.startsWith('/') ? req.substring(1) : req
            const realStr = []
            for (const key in global.webpack_use_moudle) {
              if (key.includes(arr)){
                realStr.push(key)
              }
            }
            if (realStr.length > 0){
              if (realStr.length === 1){
                console.warn('webpackEmptyContext req realStr:', realStr[0])
                if (global.webpack_use_moudle[realStr[0]] === -1){
                  req = realStr[0]
                } else {
                  return __webpack_require__(global.webpack_use_moudle[realStr[0]])
                }
              }
            }
          }
			...
			if (__webpack_require__.webpackContext && __webpack_require__.webpackContext.resolve(req)){
				return __webpack_require__.webpackContext(req);
			}
			if (!__webpack_require__.g){
				__webpack_require__.g = global;
			}
			if (__webpack_require__.g && __webpack_require__.g.require){
				return __webpack_require__.g.require(req)
			}
	var e = new Error("Cannot find module '" + req + "'");
	e.code = 'MODULE_NOT_FOUND';
	throw e;
...
module.exports = webpackEmptyAsyncContext;`
  ...
  codeGeneration (context) {
    ...
    for (const block of this.blocks) {
      for (const dep of block.dependencies) { allDeps.push(/** @type {ContextElementDependency} */ (dep)) }
    }
