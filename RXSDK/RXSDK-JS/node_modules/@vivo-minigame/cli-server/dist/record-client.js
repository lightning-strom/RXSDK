 ...
 */function writeRecords(a,b){try{_fs.default.writeFileSync(a,(0,_stringify.default)(b,null,2))}catch(a){(0,_cliSharedUtils.error)(`### App Server ### writeRecords出错: ${a.message}`)}}/**
 * 读取一个项目的设备记录
 ...
 */function recordClient(a,b,c){try{const d=_path.default.dirname(a);let e;if(mkdirsSync(d),_fs.default.existsSync(a)){e=getRecords(a);let d=getProjectClients(e);// 保留最后的4条记录，最多记录5条
...
e.records[_cliSharedUtils.projectPath].push(b),writeRecords(a,e),c&&c(`writeClientLogFile(): after: ${(0,_stringify.default)(e.records)}`)}catch(a){(0,_cliSharedUtils.error)(`### App Server ### recordClient出错: ${a.message}`)}}/**
 * 根据设备sn和ip获取设备的记录
 * @param {String} clientRecordPath - 记录文件地址
 * @param {String} sn - 设备序列号
 * @param {String} ip
 * @returns {Object} - 匹配的设备记录
 */function getRecordClient(a,b,c){try{if(_fs.default.existsSync(a)){const d=getRecords(a),e=getProjectClients(d);return e.find(a=>a.sn===b&&a.ip===c&&a.port)}}catch(a){(0,_cliSharedUtils.error)(`### App Server ### getRecordClient出错: ${a.message}`)}}/**
 * 清除一个项目的设备记录
