...
this._edits=[];}
_innerStyleSheetChanged(content){const scripts=this._inlineScripts();const styles=this._inlineStyles();let text=new TextUtils.Text(content);for(const data of this._edits){const edit=data.edit;if(!edit){continue;}
...
script[offsetSymbol]=scriptOffset.rebaseAfterTextEdit(oldRange,newRange);self.Bindings.debuggerWorkspaceBinding.updateLocations(script);}
...
style[offsetSymbol]=styleOffset.rebaseAfterTextEdit(oldRange,newRange);self.Bindings.cssWorkspaceBinding.updateLocations(style);}}
