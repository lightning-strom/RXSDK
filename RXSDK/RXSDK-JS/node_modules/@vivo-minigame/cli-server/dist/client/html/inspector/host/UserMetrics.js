import{InspectorFrontendHostInstance}from'./InspectorFrontendHost.js';class UserMetricEvent extends Event{constructor(type,init,detail={}){super(type,init);this.detail=detail;}}
function fireEvent(name,detail={},target=window){const evt=new UserMetricEvent(name,{bubbles:true,cancelable:true},detail);target.dispatchEvent(evt);}
export class UserMetrics{constructor(){this._panelChangedSinceLaunch=false;this._firedLaunchHistogram=false;this._launchPanelName='';}
panelShown(panelName){const code=PanelCodes[panelName]||0;const size=Object.keys(PanelCodes).length+1;InspectorFrontendHostInstance.recordEnumeratedHistogram('DevTools.PanelShown',code,size);fireEvent('DevTools.PanelShown',{value:code});this._panelChangedSinceLaunch=true;}
drawerShown(drawerId){this.panelShown('drawer-'+drawerId);}
actionTaken(action){const size=Object.keys(Action).length+1;InspectorFrontendHostInstance.recordEnumeratedHistogram('DevTools.ActionTaken',action,size);fireEvent('DevTools.ActionTaken',{value:action});}
