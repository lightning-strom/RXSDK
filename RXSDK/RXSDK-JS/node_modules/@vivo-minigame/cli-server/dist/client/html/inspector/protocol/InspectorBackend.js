...
class _AgentPrototype {
    ...
    _sendMessageToBackendPromise(method, signature, args) {
        ...
        return new Promise((resolve, reject) => {
            const callback = (error, result) => {
                if (error) {
                    if (!test.suppressRequestErrors && error.code !== DevToolsStubErrorCode && error.code !== _GenericError && error.code !== _ConnectionClosedErrorCode) {
    ...
    _invoke(method, request) {
        return new Promise(fulfill => {
            const callback = (error, result) => {
                if (error && !test.suppressRequestErrors && error.code !== DevToolsStubErrorCode && error.code !== _GenericError && error.code !== _ConnectionClosedErrorCode) {
                    console.error('Request ' + method + ' failed. ' + JSON.stringify(error));
                }
                if (!result) {
                    result = {};
...
class _DispatcherPrototype {
    ...
    dispatch(functionName, messageObject) {
        ...
        if (['Page.removed', 'Page.changed'].includes(messageObject.method)) {
            const appPanel = document.getElementById('app_data')
            if (appPanel) {
                appPanel.contentWindow.postMessage(messageObject)
            }
            return;
        }
        if (!this._eventArgs[messageObject.method]) {
            InspectorBackend.reportProtocolError(`Protocol Error: Attempted to dispatch an unspecified method '${messageObject.method}'`, messageObject);
            return;
        }
        const params = [];
