import*as Common from'../common/common.js';import*as Host from'../host/host.js';import*as SDK from'../sdk/sdk.js';import*as UI from'../ui/ui.js';export class PerformanceMonitorImpl extends UI.Widget.HBox{constructor(){super(true);this.registerRequiredCSS('performance_monitor/performanceMonitor.css');this.contentElement.classList.add('perfmon-pane');this._metricsBuffer=[];this._pixelsPerMs=10/1000;this._pollIntervalMs=500;this._scaleHeight=16;this._graphHeight=90;this._gridColor=self.UI.themeSupport.patchColorText('rgba(0, 0, 0, 0.08)',UI.UIUtils.ThemeSupport.ColorUsage.Foreground);this._controlPane=new ControlPane(this.contentElement);const chartContainer=this.contentElement.createChild('div','perfmon-chart-container');this._canvas=(chartContainer.createChild('canvas'));this._canvas.tabIndex=-1;UI.ARIAUtils.setAccessibleName(this._canvas,Common.UIString.UIString('Graphs displaying a real-time view of performance metrics'));this.contentElement.createChild('div','perfmon-chart-suspend-overlay fill').createChild('div').textContent=Common.UIString.UIString('Paused');this._controlPane.addEventListener(Events.MetricChanged,this._recalcChartHeight,this);self.SDK.targetManager.observeModels(SDK.PerformanceMetricsModel.PerformanceMetricsModel,this);}
wasShown(){if(!this._model){return;}
self.SDK.targetManager.addEventListener(SDK.SDKModel.Events.SuspendStateChanged,this._suspendStateChanged,this);this._model.enable();this._suspendStateChanged();}
willHide(){if(!this._model){return;}
self.SDK.targetManager.removeEventListener(SDK.SDKModel.Events.SuspendStateChanged,this._suspendStateChanged,this);this._stopPolling();this._model.disable();}
...
this._model=null;}
_suspendStateChanged(){const suspended=self.SDK.targetManager.allTargetsSuspended();if(suspended){this._stopPolling();}else{this._startPolling();}
...
_drawHorizontalGrid(ctx){const labelDistanceSeconds=10;const lightGray=self.UI.themeSupport.patchColorText('rgba(0, 0, 0, 0.02)',UI.UIUtils.ThemeSupport.ColorUsage.Foreground);ctx.font='10px '+Host.Platform.fontFamily();ctx.fillStyle=self.UI.themeSupport.patchColorText('rgba(0, 0, 0, 0.55)',UI.UIUtils.ThemeSupport.ColorUsage.Foreground);const currentTime=Date.now()/1000;for(let sec=Math.ceil(currentTime);;--sec){const x=this._width-((currentTime-sec)*1000-this._pollIntervalMs)*this._pixelsPerMs;if(x<-50){break;}
...
const backgroundColor=Common.Color.Color.parse(self.UI.themeSupport.patchColorText('white',UI.UIUtils.ThemeSupport.ColorUsage.Background));for(const path of paths.reverse()){const color=path.color;ctx.save();ctx.fillStyle=backgroundColor.blendWith(Common.Color.Color.parse(color).setAlpha(0.2)).asString(null);ctx.fill(path.path);ctx.strokeStyle=color;ctx.lineWidth=0.5;ctx.stroke(path.path);ctx.restore();}
ctx.fillStyle=self.UI.themeSupport.patchColorText('rgba(0, 0, 0, 0.55)',UI.UIUtils.ThemeSupport.ColorUsage.Foreground);ctx.font=`10px  ${Host.Platform.fontFamily()}`;ctx.fillText(chartInfo.title,8,10);this._drawVerticalGrid(ctx,height-bottomPadding,max,chartInfo);ctx.restore();}
...
let scaleValue=Math.floor(max/base)*base;const span=max;const topPadding=18;const visibleHeight=height-topPadding;ctx.fillStyle=self.UI.themeSupport.patchColorText('rgba(0, 0, 0, 0.55)',UI.UIUtils.ThemeSupport.ColorUsage.Foreground);ctx.strokeStyle=this._gridColor;ctx.beginPath();for(let i=0;i<2;++i){const y=calcY(scaleValue);const labelText=MetricIndicator._formatNumber(scaleValue,info);ctx.moveTo(0,y);ctx.lineTo(4,y);ctx.moveTo(ctx.measureText(labelText).width+12,y);ctx.lineTo(this._width,y);ctx.fillText(labelText,8,calcY(scaleValue)+3);scaleValue/=2;}
ctx.stroke();ctx.beginPath();ctx.moveTo(0,height+0.5);ctx.lineTo(this._width,height+0.5);ctx.strokeStyle=self.UI.themeSupport.patchColorText('rgba(0, 0, 0, 0.2)',UI.UIUtils.ThemeSupport.ColorUsage.Foreground);ctx.stroke();function calcY(value){return Math.round(height-visibleHeight*value/span)+0.5;}}
...
export const Format={Percent:Symbol('Percent'),Bytes:Symbol('Bytes'),};export class ControlPane extends Common.ObjectWrapper.ObjectWrapper{constructor(parent){super();this.element=parent.createChild('div','perfmon-control-pane');this._enabledChartsSetting=self.Common.settings.createSetting('perfmonActiveIndicators2',['TaskDuration','JSHeapTotalSize','Nodes']);this._enabledCharts=new Set(this._enabledChartsSetting.get());const format=Format;this._chartsInfo=[{title:Common.UIString.UIString('CPU usage'),metrics:[{name:'TaskDuration',color:'#999'},{name:'ScriptDuration',color:'orange'},{name:'LayoutDuration',color:'blueviolet'},{name:'RecalcStyleDuration',color:'violet'}],format:format.Percent,smooth:true,stacked:true,color:'red',max:1},{title:Common.UIString.UIString('JS heap size'),metrics:[{name:'JSHeapTotalSize',color:'#99f'},{name:'JSHeapUsedSize',color:'blue'}],format:format.Bytes,color:'blue'},{title:Common.UIString.UIString('DOM Nodes'),metrics:[{name:'Nodes',color:'green'}]},{title:Common.UIString.UIString('JS event listeners'),metrics:[{name:'JSEventListeners',color:'yellowgreen'}]},{title:Common.UIString.UIString('Documents'),metrics:[{name:'Documents',color:'darkblue'}]},{title:Common.UIString.UIString('Document Frames'),metrics:[{name:'Frames',color:'darkcyan'}]},{title:Common.UIString.UIString('Layouts / sec'),metrics:[{name:'LayoutCount',color:'hotpink'}]},{title:Common.UIString.UIString('Style recalcs / sec'),metrics:[{name:'RecalcStyleCount',color:'deeppink'}]}];for(const info of this._chartsInfo){for(const metric of info.metrics){metric.color=self.UI.themeSupport.patchColorText(metric.color,UI.UIUtils.ThemeSupport.ColorUsage.Foreground);}}
