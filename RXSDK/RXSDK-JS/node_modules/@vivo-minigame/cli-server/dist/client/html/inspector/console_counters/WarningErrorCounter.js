import*as Common from'../common/common.js';import*as SDK from'../sdk/sdk.js';import*as UI from'../ui/ui.js';export class WarningErrorCounter{constructor(){WarningErrorCounter._instanceForTest=this;const countersWrapper=createElement('div');this._toolbarItem=new UI.Toolbar.ToolbarItem(countersWrapper);this._counter=createElement('div');this._counter.addEventListener('click',self.Common.console.show.bind(self.Common.console),false);const shadowRoot=UI.Utils.createShadowRootWithCoreStyles(this._counter,'console_counters/errorWarningCounter.css');countersWrapper.appendChild(this._counter);this._violationCounter=createElement('div');this._violationCounter.addEventListener('click',()=>{self.UI.viewManager.showView('lighthouse');});const violationShadowRoot=UI.Utils.createShadowRootWithCoreStyles(this._violationCounter,'console_counters/errorWarningCounter.css');if(Root.Runtime.experiments.isEnabled('spotlight')){countersWrapper.appendChild(this._violationCounter);}
...
this._titles='';this._errorCount=-1;this._warningCount=-1;this._violationCount=-1;this._throttler=new Common.Throttler.Throttler(100);self.SDK.consoleModel.addEventListener(SDK.ConsoleModel.Events.ConsoleCleared,this._update,this);self.SDK.consoleModel.addEventListener(SDK.ConsoleModel.Events.MessageAdded,this._update,this);self.SDK.consoleModel.addEventListener(SDK.ConsoleModel.Events.MessageUpdated,this._update,this);this._update();}
_updatedForTest(){}
...
_updateThrottled(){const errors=self.SDK.consoleModel.errors();const warnings=self.SDK.consoleModel.warnings();const violations=self.SDK.consoleModel.violations();if(errors===this._errorCount&&warnings===this._warningCount&&violations===this._violationCount){return Promise.resolve();}
...
this._counter.title=this._titles;UI.ARIAUtils.setAccessibleName(this._counter,this._titles);self.UI.inspectorView.toolbarItemResized();this._updatingForTest=false;this._updatedForTest();return Promise.resolve();}
