...
interface IgetShareData {
  ...
  custom_ext?: any
  autoReport?: boolean
  auto_report?: boolean // Unity RXShareConfig 序列化字段
  content?: string // 自定义分享标题文案
  image?: string // 自定义分享图片地址
  properties?: Record<string, any> // 自动上报自定义属性
  materialType?: string // 素材类型
  get_qrcode?: boolean // 是否获取二维码
  needShowEntrance?: boolean // 是否显示入口
}

interface IpayParmas {
  //支付的类型 minigame米大师支付 wxpub为ios跳转客服公众号支付 aums: 银联云闪付支付
  pay_type:'minigame' | 'minigame_friend' | 'wxpub' | 'minigame_v2' | 'aums' | 'minigame_vivo' | 'minigame_oppo' | 'midas_game_item' | 'minigame_huawei' | 'jump_miniprogram' | 'wechath5' | 'minigame_douyin_diamond' | 'midas_payment_game_item',
