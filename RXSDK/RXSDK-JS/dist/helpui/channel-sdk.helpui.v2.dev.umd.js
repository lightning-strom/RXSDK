...
(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.channelSDK = {}));
	    ...
	    /***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
	      ...
	      var View = /** @class */ (function (_super) {
	        ...
	        View.prototype.render = function () {
	          var ctx = this.ctx;
	          ctx.save();
	          var _a = this.baseRender(), needStroke = _a.needStroke; _a.needClip; var originX = _a.originX, originY = _a.originY;
	          if (needStroke) {
	            ctx.stroke();
	          }
	          ctx.translate(-originX, -originY);
	    ...
	    /***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
	      ...
	      var Text = /** @class */ (function (_super) {
	        ...
	        Text.prototype.render = function () {
	          ...
	          ctx.save();
	          this.toCanvasData();
	          var _a = this.baseRender('test'), needStroke = _a.needStroke, originX = _a.originX, originY = _a.originY, drawX = _a.drawX, drawY = _a.drawY, width = _a.width; _a.height;
	...
	var engineLayout = unwrapExports(engine);

	const wrapText = (text, fontSize, maxWidth) => {
	  const canvas = __env$1.createCanvas();
	...
	function padArrayToMultipleOfThree$1(arr) {
	  // 计算需要补足的元素数量
	...
	function padArrayToMultipleOfTwo$1(arr) {
	  arr.forEach((item, i) => {
	...
	function getHomeLandscapePage(classes = [], hot_questions = [], nickname, callback) {
	  const paddingX = 51;
	  ...
	  let questionDirectoryList = () => {
		...
		`
	      ...
	      return padArrayToMultipleOfTwo$1(classes).map((item, index) => {
	        return `
		...
		`
	      ...
	      return padArrayToMultipleOfThree$1(classes).map((item, index) => {
	        return `
...
`;
	  ...
	  function homeStyle(screenInfo) {
	    const buttonRect = __env$1.getMenuButtonBoundingClientRect();
	...
	function getCategoryLandscapePage(classes = [], questions = [], currentId, callback) {
	  const paddingX = 51;
...
`;
	...
	function getQuestionLandscapePage (info, resolved, callback) {
		const paddingX = 51;
...
`;
	...
	function getHomePortraitPage(classes = [], hot_questions = [], nickname, callback) {
	  const wrapCategoryTitleTextView = (str) => {
	    const _str = truncateText(str, 7);
	    return wrapText(_str, getRatioPx(14)).map(item => (`
        <text value='${item}' class='questionCategoryText'></text>
...
`;
	...
	function getCategoryPortraitPage(classes = [], questions = [], currentId, callback) {
	  const index = classes.findIndex(item => item.id === currentId);
	  ...
	  const wrapCommonTextView = (str) => {
	    return wrapText(str, getRatioPx(15), getGlobalScreen().screenWidth - getRatioPx(24 + 12 + 22 + 24)).map(item => (`
        <text value='${item}' class='questionItemText'></text>
  	`)).join('')
	  };
...
<view class='container'>
	<view class='header'>
		<view class='btnGroupItem backBtnItem' id='backBtnCategory'>
					<image src='${backBtnImg}' class='backImg' />
		</view>
    <text class='headerTitle' value='问题目录'></text>
	</view>
...
`;
	  ...
	  function watch() {
	    const backBtn = Layout.getElementById('backBtnCategory');
	    const backGameBtn = Layout.getElementById('backGame');
	...
	function getQuestionPortraitPage (info, resolved, callback) {
...
`;
	...
	const __env$1 = GameGlobal.wx || GameGlobal.tt || GameGlobal.swan;
	const sharedCanvas = __env$1.createCanvas();
	const sharedContext = sharedCanvas.getContext('2d');
	const Layout = engineLayout;
	...
	function init({
	  ...
	  switch (platform) {
	    case 'wechat':
	      systemInfo = __env$1.getWindowInfo();
	      break
	    case 'douyin':
	      systemInfo = __env$1.getSystemInfoSync();
	  }
	  const {
	  ...
	  if (landscape) {
	    ...
	    try {
	      globalInfo.landscape = __env$1.getSystemSetting().deviceOrientation === 'landscape';
	    } catch (e) {
	...
	function getRatioPx(px) {
	  return px * (globalInfo.screenWidth / (globalInfo.landscape ? 812 : 375))
	}

	function getGlobalScreen() {
	  return globalInfo
	}

	exports.Layout = Layout;
	exports.__env = __env$1;
	exports.getGlobalScreen = getGlobalScreen;
...
//# sourceMappingURL=channel-sdk.helpui.v2.dev.umd.js.map
