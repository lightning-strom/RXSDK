"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __rest = (this && this.__rest) || function (s, e) {
    var t = {};
    for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p) && e.indexOf(p) < 0)
        t[p] = s[p];
    if (s != null && typeof Object.getOwnPropertySymbols === "function")
        for (var i = 0, p = Object.getOwnPropertySymbols(s); i < p.length; i++) {
            if (e.indexOf(p[i]) < 0 && Object.prototype.propertyIsEnumerable.call(s, p[i]))
                t[p[i]] = s[p[i]];
        }
    return t;
};
Object.defineProperty(exports, "__esModule", { value: true });
var v4_1 = require("uuid/v4");
var config_1 = require("@/config");
var config_2 = require("@/rpk/config");
var const_1 = require("@/config/const");
var utils_1 = require("@/utils/utils");
var day_1 = require("@/utils/day");
var is_1 = require("@/utils/is");
var paramsValid_1 = require("@/utils/paramsValid");
var utils_2 = require("@/rpk/utils");
var checkConfig_1 = require("@/rpk/checkConfig");
var apis_1 = require("@/rpk/apis");
var SdkCommon_1 = require("@/rpk/SdkCommon");
var stOffset_1 = require("@/utils/stOffset");
var PLATFORM = 'douyin';
function formatTimestamp(seconds) {
    try {
        // 创建一个新的Date对象，注意JavaScript使用毫秒时间戳，所以要乘以1000
        var date = new Date(seconds * 1000);
        // 获取各个时间组件
        var year = date.getFullYear();
        var month = String(date.getMonth() + 1).padStart(2, '0'); // 月份从0开始
        var day = String(date.getDate()).padStart(2, '0');
        var hours = String(date.getHours()).padStart(2, '0');
        var minutes = String(date.getMinutes()).padStart(2, '0');
        var secondsStr = String(date.getSeconds()).padStart(2, '0');
        // 获取时区偏移（分钟），然后转换为小时
        var timezoneOffset = date.getTimezoneOffset();
        var timezoneHours = Math.abs(Math.floor(timezoneOffset / 60));
        var timezoneMinutes = Math.abs(timezoneOffset % 60);
        var timezoneSign = timezoneOffset > 0 ? '-' : '+';
        var timezoneStr = "".concat(timezoneSign).concat(String(timezoneHours).padStart(2, '0')).concat(String(timezoneMinutes).padStart(2, '0'));
        // 构造最终格式
        return "".concat(year, "-").concat(month, "-").concat(day, " ").concat(hours, ":").concat(minutes, ":").concat(secondsStr, " ").concat(timezoneStr, " CST");
    }
    catch (e) {
        return seconds;
    }
}
var SdkDouyin = /** @class */ (function (_super) {
    __extends(SdkDouyin, _super);
    function SdkDouyin(initParams) {
        var _this = _super.call(this, PLATFORM) || this;
        _this._hasAd = {
            rewarded: undefined
        };
        _this.funcs = [];
        _this.isPromoter = false;
        // 默认刷新时间 10 分钟
        _this.businessRuleDefaultRefreshTime = 600000;
        // 商业广告规则信息
        _this.businessRulesInfo = {
            // 定时器的编号
            timerId: 0,
            // 时间间隔
            refresh_time: _this.businessRuleDefaultRefreshTime,
            // 主窗口配置信息
            main_window_list: [],
            // 窗口配置信息
            window_list: [],
            // 版本-服务端缓存使用
            version: '',
            // 是否命中缓存
            hit_cache: false
        };
        // 商业化接口是否返回结果
        _this.businessRuleInvoking = false;
        // 条件获取商业化窗口队列
        _this.businessWindowsQueue = [];
        // 上报公共属性接口失败次数
        _this.trackPublicPropsFailCount = 0;
        _this.initConfig = {};
        // 调度埋点
        _this.scheduleInitMap = {};
        // 获取分享数据缓存调度上报参数
        _this.scheuleReportProps = {};
        _this.deviceInfo = null;
        // 子渠道id
        _this.subChannelId = null;
        // 是否为推广员
        _this.is_promoter = false;
        _this.game_id = '';
        // 推广员福利码相关信息
        _this.promoInfo = {
            timer: null,
            refresh_period_exp: 0,
            polling: 0,
            promo_code: ''
        };
        _this.needTrackFeedLogin = false;
        _this.cacheLoginParams = {};
        Object.assign(config_1.SYSTEM_INFO, config_2.SYSTEM_INFO, __assign({}, initParams));
        try {
            var microapp = tt.getEnvInfoSync().microapp;
            if (microapp === null || microapp === void 0 ? void 0 : microapp.mpVersion) {
                config_1.SYSTEM_INFO.miniVersion = microapp === null || microapp === void 0 ? void 0 : microapp.mpVersion;
            }
        }
        catch (e) {
        }
        _this.saveDeviceInfo();
        if (initParams.isMatch) {
            config_1.SYSTEM_INFO.SDK_INIT_FINISHED = true;
            initParams.complete({
                code: 0
            });
        }
        else {
            _this.getInitConfig({ complete: initParams.complete });
        }
        return _this;
    }
    SdkDouyin.prototype.saveDeviceInfo = function () {
        try {
            this.deviceInfo = tt.getSystemInfoSync(false);
        }
        catch (e) {
            console.log('saveDeviceInfo error', e);
            return;
        }
    };
    // public onFeedStatusChange(callback: any) {
    //   tt.onFeedStatusChange((res: any) => {
    //     if (res.type === 'feedEnter') {
    //       if (this.needTrackFeedLogin) {
    //         console.info('触发从Feed流进入小游戏事件上报login')
    //         this.trackLogin()
    //       }
    //     }
    //     callback(res)
    //   })
    // }
    SdkDouyin.prototype.offFeedStatusChange = function (callback) {
        tt.offFeedStatusChange(callback);
    };
    SdkDouyin.prototype.trackLogin = function () {
        var _this = this;
        if (this.needTrackFeedLogin) {
            console.info('触发从Feed流进入小游戏事件上报login');
            (0, apis_1.getIpApi)().catch(function (e) {
                var _a;
                var ip = e.client_ip;
                _this.needTrackFeedLogin = false;
                var language = 'zh-CN';
                var model = '';
                var platform = '';
                var appId = '';
                try {
                    var systemInfoSync = tt.getSystemInfoSync();
                    language = systemInfoSync.language;
                    model = systemInfoSync.model;
                    platform = systemInfoSync.platform;
                }
                catch (e) {
                }
                try {
                    var envInfo = tt.getEnvInfoSync();
                    appId = (_a = envInfo.microapp) === null || _a === void 0 ? void 0 : _a.appId;
                }
                catch (e) {
                }
                var params = {
                    event: '#login',
                    ip: ip,
                    properties: {
                        client_distinct_id: _this.cacheLoginParams.distinct_id,
                        logintime: Date.now(),
                        platformid: 4,
                        traceid: (0, v4_1.default)(),
                        method: 'douyinh5',
                        username: 'douyinh5_' + config_1.USER_INFO.tid,
                        external_openid: config_1.USER_INFO.tid,
                        thirdparty_appid: appId,
                        cp_userid: config_1.USER_INFO.cp_user_id,
                        sex: config_1.USER_INFO.sex,
                        accept_language: language,
                        '#rx_sdk_version': config_1.SYSTEM_INFO.__RX_SDK_VERSION,
                        openid: config_1.USER_INFO.openid,
                        channelid: config_1.SYSTEM_INFO.channelId,
                        subchannelid: config_1.USER_INFO.subchannelid,
                        last_login_time: formatTimestamp(config_1.USER_INFO.last_login_time),
                        rx_device_model: model,
                        rx_os: platform
                    }
                };
                _this.track(params, {
                    complete: function (data) {
                        console.info('share error add complete func when tracked:', data);
                    }
                });
            });
        }
    };
    SdkDouyin.prototype.login = function (params, callback) {
        var _a, _b, _c, _d, _e, _f, _g;
        return __awaiter(this, void 0, void 0, function () {
            var user_source, source_ad, distinct_idLocal, distinct_id, now, reqLogin, queryJson, loginResult, _h, custom_ext, rest_ext, launchOptions, loginTokenRx, reflowEnabled, loginRx, err_1;
            return __generator(this, function (_j) {
                switch (_j.label) {
                    case 0:
                        _j.trys.push([0, 7, , 8]);
                        return [4 /*yield*/, (0, paramsValid_1.pubCheck)(checkConfig_1.douyinLoginParamsCheck, callback, params)];
                    case 1:
                        _j.sent();
                        user_source = this.getLoginQsAndGenerateStruct();
                        source_ad = this.getAttributionData();
                        params.method = params.method || 'douyinh5';
                        distinct_idLocal = (0, utils_2.customGetStorageSync)('rx_distinct_id');
                        distinct_id = distinct_idLocal || (0, v4_1.default)();
                        if (!distinct_idLocal) {
                            (0, utils_2.customSetStorageSync)('rx_distinct_id', distinct_id);
                        }
                        now = new Date().getTime();
                        reqLogin = __assign(__assign(__assign({}, (0, is_1.omit)(params, 'force')), user_source), { distinct_id: distinct_id, ts: now });
                        try {
                            if (this.subChannelId !== null) {
                                queryJson = (0, utils_2.getSearchQueries)();
                                reqLogin.user_source = {
                                    guide: __assign(__assign({}, user_source), { subchannelid: this.subChannelId })
                                };
                                if (queryJson) {
                                    reqLogin.user_source.guide = __assign(__assign({}, reqLogin.user_source.guide), queryJson);
                                }
                            }
                        }
                        catch (err) {
                        }
                        return [4 /*yield*/, (0, utils_1.asyncFunc)(tt.login, {
                                force: params.force
                            })];
                    case 2:
                        loginResult = _j.sent();
                        _h = reqLogin.ext || {}, custom_ext = _h.custom_ext, rest_ext = __rest(_h, ["custom_ext"]);
                        reqLogin.custom_ext = custom_ext || {};
                        reqLogin.ext = __assign(__assign({}, (rest_ext || {})), (0, is_1.pick)(loginResult, ['anonymousCode', 'code']));
                        try {
                            launchOptions = tt.getLaunchOptionsSync();
                            console.info(launchOptions);
                            if (((_a = launchOptions.scene) === null || _a === void 0 ? void 0 : _a.endsWith('3041')) || launchOptions.query.is_preload_test === '3041') {
                                this.needTrackFeedLogin = true;
                                this.cacheLoginParams = reqLogin;
                                reqLogin.is_preload = true;
                            }
                        }
                        catch (e) {
                            console.log(e);
                        }
                        if (!params.login_openid) return [3 /*break*/, 4];
                        return [4 /*yield*/, (0, apis_1.loginByTokenApi)(this.ActivePrefix(reqLogin))];
                    case 3:
                        loginTokenRx = _j.sent();
                        Object.assign(config_1.USER_INFO, loginTokenRx.data);
                        (0, utils_2.customSetStorageSync)('rxToken', loginTokenRx.data.token);
                        (0, utils_2.customSetStorageSync)('rx-loginState', 1);
                        try {
                            if ((((_b = loginTokenRx === null || loginTokenRx === void 0 ? void 0 : loginTokenRx.data) === null || _b === void 0 ? void 0 : _b.user_flag) & 1) == 1) {
                                this.isPromoter = true;
                                this.game_id = (_c = loginTokenRx === null || loginTokenRx === void 0 ? void 0 : loginTokenRx.data) === null || _c === void 0 ? void 0 : _c.cp_user_id;
                            }
                        }
                        catch (e) {
                        }
                        callback.complete(loginTokenRx);
                        return [3 /*break*/, 6];
                    case 4:
                        reflowEnabled = ((_e = (_d = this.initConfig) === null || _d === void 0 ? void 0 : _d.advertise_switch) === null || _e === void 0 ? void 0 : _e.switch) === 1;
                        reqLogin = reflowEnabled ? __assign(__assign({}, reqLogin), { device: source_ad }) : __assign({}, reqLogin);
                        console.log('reqLogin', reqLogin);
                        return [4 /*yield*/, (0, apis_1.loginByCredentialApi)(this.ActivePrefix(reqLogin))];
                    case 5:
                        loginRx = _j.sent();
                        Object.assign(config_1.USER_INFO, loginRx.data);
                        (0, utils_2.customSetStorageSync)('rxToken', loginRx.data.token);
                        (0, utils_2.customSetStorageSync)('rx-loginState', 1);
                        try {
                            if ((((_f = loginRx === null || loginRx === void 0 ? void 0 : loginRx.data) === null || _f === void 0 ? void 0 : _f.user_flag) & 1) == 1) {
                                this.isPromoter = true;
                                this.game_id = (_g = loginRx === null || loginRx === void 0 ? void 0 : loginRx.data) === null || _g === void 0 ? void 0 : _g.cp_user_id;
                            }
                        }
                        catch (e) {
                        }
                        callback.complete(loginRx);
                        _j.label = 6;
                    case 6: return [3 /*break*/, 8];
                    case 7:
                        err_1 = _j.sent();
                        callback.complete((0, utils_2.handleTrackError)(PLATFORM, 'rxlog_error_login', err_1, const_1.COMMON_ERROR_CODE.LOGIN_FAIL));
                        return [3 /*break*/, 8];
                    case 8: return [2 /*return*/];
                }
            });
        });
    };
    // 同步用户信息
    SdkDouyin.prototype.infoSync = function (callback) {
        var _a, _b;
        return __awaiter(this, void 0, void 0, function () {
            var loginResult, userInfoResult, result, error_1;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        _c.trys.push([0, 4, , 5]);
                        return [4 /*yield*/, (0, utils_1.asyncFunc)(tt.login, {
                                force: true
                            })];
                    case 1:
                        loginResult = _c.sent();
                        return [4 /*yield*/, (0, utils_1.asyncFunc)(tt.getUserInfo, {})];
                    case 2:
                        userInfoResult = _c.sent();
                        return [4 /*yield*/, (0, apis_1.refreshUserInfo)(__assign(__assign({}, (0, is_1.pick)(loginResult, ['anonymousCode', 'code'])), { nickname: ((_a = userInfoResult === null || userInfoResult === void 0 ? void 0 : userInfoResult.userInfo) === null || _a === void 0 ? void 0 : _a.nickName) || '', avatar: ((_b = userInfoResult === null || userInfoResult === void 0 ? void 0 : userInfoResult.userInfo) === null || _b === void 0 ? void 0 : _b.avatarUrl) || '' }))];
                    case 3:
                        result = _c.sent();
                        callback.complete(result);
                        return [3 /*break*/, 5];
                    case 4:
                        error_1 = _c.sent();
                        callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', error_1));
                        return [3 /*break*/, 5];
                    case 5: return [2 /*return*/];
                }
            });
        });
    };
    SdkDouyin.prototype.pay = function (params, callback) {
        var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k, _l, _m, _o, _p, _q, _r, _s, _t, _u, _v;
        return __awaiter(this, void 0, void 0, function () {
            var reqOrder, result, pay_data, pay_data, pay_data, err_2;
            return __generator(this, function (_w) {
                switch (_w.label) {
                    case 0:
                        _w.trys.push([0, 10, , 11]);
                        return [4 /*yield*/, (0, paramsValid_1.pubCheck)(checkConfig_1.douyinPayCheckParams, callback, params)];
                    case 1:
                        _w.sent();
                        reqOrder = __assign(__assign(__assign({}, params), { sub_channel_id: config_1.USER_INFO === null || config_1.USER_INFO === void 0 ? void 0 : config_1.USER_INFO.subchannelid, ext: params.ext || {}, currency: 'CNY', openid: config_1.USER_INFO.openid, is_debug: params.is_debug || 0, env: params.env || 0 }), (!(0, is_1.isEmpty)(this.deviceInfo) ? { device_info: this.deviceInfo } : {}));
                        if (params.isGift) {
                            this.payGift(reqOrder, callback);
                            return [2 /*return*/];
                        }
                        return [4 /*yield*/, (0, apis_1.orderApi)(reqOrder)
                            //unity 兼容逻辑
                        ];
                    case 2:
                        result = _w.sent();
                        //unity 兼容逻辑
                        if (params.onlyGetOrder) {
                            callback.complete({ code: 0, data: result });
                            return [2 /*return*/];
                        }
                        //检查用户的登录态是否有效
                        return [4 /*yield*/, (0, utils_1.asyncFunc)(tt.checkSession)];
                    case 3:
                        //检查用户的登录态是否有效
                        _w.sent();
                        if (!(params.platform == 'ios')) return [3 /*break*/, 5];
                        pay_data = {
                            currencyType: ((_b = (_a = result.data) === null || _a === void 0 ? void 0 : _a.ext) === null || _b === void 0 ? void 0 : _b.currency_type) || (((_d = (_c = result.data) === null || _c === void 0 ? void 0 : _c.ext) === null || _d === void 0 ? void 0 : _d.micro) ? 'DIAMOND_PROP' : 'DIAMOND'),
                            buyQuantity: ((_f = (_e = result.data) === null || _e === void 0 ? void 0 : _e.ext) === null || _f === void 0 ? void 0 : _f.micro) ? (((_g = result.data) === null || _g === void 0 ? void 0 : _g.price) / 10) : (_j = (_h = result.data) === null || _h === void 0 ? void 0 : _h.ext) === null || _j === void 0 ? void 0 : _j.mini_game_amount,
                            zoneId: params.zoneId || '1',
                            customId: (_k = result.data) === null || _k === void 0 ? void 0 : _k.order_no,
                            extraInfo: (_l = result.data) === null || _l === void 0 ? void 0 : _l.transmit_args
                        };
                        console.info('ios支付：', pay_data);
                        return [4 /*yield*/, (0, utils_1.asyncFunc)(tt.openAwemeCustomerService, pay_data)];
                    case 4:
                        _w.sent();
                        return [3 /*break*/, 9];
                    case 5:
                        if (!(tt.canIUse('requestGamePayment.object.goodType') && ((_o = (_m = result.data) === null || _m === void 0 ? void 0 : _m.ext) === null || _o === void 0 ? void 0 : _o.micro))) return [3 /*break*/, 7];
                        pay_data = {
                            goodType: 2,
                            mode: 'game',
                            orderAmount: (_p = result.data) === null || _p === void 0 ? void 0 : _p.price,
                            env: params.env || 0,
                            currencyType: 'CNY',
                            platform: 'android',
                            zoneId: params.zoneId || '1',
                            customId: (_q = result.data) === null || _q === void 0 ? void 0 : _q.order_no,
                            extraInfo: (_r = result.data) === null || _r === void 0 ? void 0 : _r.transmit_args
                        };
                        console.info('安卓小额支付：', pay_data);
                        return [4 /*yield*/, (0, utils_1.asyncFunc)(tt.requestGamePayment, pay_data)];
                    case 6:
                        _w.sent();
                        return [3 /*break*/, 9];
                    case 7:
                        pay_data = {
                            mode: 'game',
                            env: params.env || 0,
                            currencyType: 'CNY',
                            platform: 'android',
                            buyQuantity: (_t = (_s = result.data) === null || _s === void 0 ? void 0 : _s.ext) === null || _t === void 0 ? void 0 : _t.mini_game_amount,
                            zoneId: params.zoneId || '1',
                            customId: (_u = result.data) === null || _u === void 0 ? void 0 : _u.order_no,
                            extraInfo: (_v = result.data) === null || _v === void 0 ? void 0 : _v.transmit_args
                        };
                        console.info('安卓支付：', pay_data);
                        return [4 /*yield*/, (0, utils_1.asyncFunc)(tt.requestGamePayment, pay_data)];
                    case 8:
                        _w.sent();
                        _w.label = 9;
                    case 9:
                        callback.complete({ code: 0 });
                        return [3 /*break*/, 11];
                    case 10:
                        err_2 = _w.sent();
                        if (err_2.errCode == -2 || err_2.errNo == -2) {
                            err_2.code = const_1.COMMON_ERROR_CODE.CANCEL_PAY;
                            err_2.thirdcode = -2;
                            callback.complete((0, utils_2.handleTrackError)(PLATFORM, 'rxlog_error_pay', err_2));
                            return [2 /*return*/];
                        }
                        callback.complete((0, utils_2.handleTrackError)(PLATFORM, 'rxlog_error_pay', err_2, const_1.COMMON_ERROR_CODE.PAY_ERROR));
                        return [3 /*break*/, 11];
                    case 11: return [2 /*return*/];
                }
            });
        });
    };
    //分享
    SdkDouyin.prototype.share = function (params, callback) {
        var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k, _l, _m, _o, _p, _q, _r, _s, _t, _u, _v, _w, _x, _y, _z, _0, _1, _2, _3, _4, _5, _6, _7, _8, _9, _10, _11, _12, _13, _14, _15, _16, _17, _18, _19, _20;
        return __awaiter(this, void 0, void 0, function () {
            var shareData_1, query, title, err_3;
            return __generator(this, function (_21) {
                switch (_21.label) {
                    case 0:
                        _21.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, this.getShareData(params, callback, true)];
                    case 1:
                        shareData_1 = _21.sent();
                        query = utils_1.qs.stringify({
                            type: 'rx',
                            user_source: 'share',
                            platform: ((_a = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _a === void 0 ? void 0 : _a.platform) || '',
                            transmits: encodeURIComponent((params === null || params === void 0 ? void 0 : params.transmits) || ''),
                            landing_id: ((_c = (_b = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _b === void 0 ? void 0 : _b.content) === null || _c === void 0 ? void 0 : _c.landing_id) || '',
                            trigger_id: ((_e = (_d = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _d === void 0 ? void 0 : _d.trigger) === null || _e === void 0 ? void 0 : _e.id) || '',
                            trigger_tag: ((_g = (_f = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _f === void 0 ? void 0 : _f.trigger) === null || _g === void 0 ? void 0 : _g.tag) || '',
                            trigger_type: ((_j = (_h = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _h === void 0 ? void 0 : _h.trigger) === null || _j === void 0 ? void 0 : _j.type) || '',
                            material_type: ((_l = (_k = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _k === void 0 ? void 0 : _k.content) === null || _l === void 0 ? void 0 : _l.material_type) || '',
                            material_id: ((_o = (_m = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _m === void 0 ? void 0 : _m.content) === null || _o === void 0 ? void 0 : _o.material_id) || '',
                            strategy_type: ((_q = (_p = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _p === void 0 ? void 0 : _p.strategy) === null || _q === void 0 ? void 0 : _q.type) || '',
                            strategy_id: ((_s = (_r = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _r === void 0 ? void 0 : _r.strategy) === null || _s === void 0 ? void 0 : _s.id) || '',
                            material_name: ((_u = (_t = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _t === void 0 ? void 0 : _t.content) === null || _u === void 0 ? void 0 : _u.title) || '',
                            trigger_name: ((_w = (_v = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _v === void 0 ? void 0 : _v.trigger) === null || _w === void 0 ? void 0 : _w.title) || '',
                            strategy_name: ((_y = (_x = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _x === void 0 ? void 0 : _x.strategy) === null || _y === void 0 ? void 0 : _y.name) || '',
                            share_time: Math.floor(new Date().getTime() / 1000),
                            share_type: 'mini',
                            inviter_region: config_1.USER_INFO.region || '',
                            inviter_openid: config_1.USER_INFO.openid || '',
                            inviter_productid: config_1.SYSTEM_INFO.productId,
                            inviter_channelid: config_1.SYSTEM_INFO.channelId,
                            inviter_subchannelid: (config_1.USER_INFO === null || config_1.USER_INFO === void 0 ? void 0 : config_1.USER_INFO.subchannelid) || ''
                        });
                        query = params.query ? "".concat(query, "&").concat(params.query) : query;
                        title = params.title || ((_0 = (_z = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _z === void 0 ? void 0 : _z.content) === null || _0 === void 0 ? void 0 : _0.title) || ((_2 = (_1 = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _1 === void 0 ? void 0 : _1.content) === null || _2 === void 0 ? void 0 : _2.content) || '';
                        if (((_4 = (_3 = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _3 === void 0 ? void 0 : _3.content) === null || _4 === void 0 ? void 0 : _4.material_type) === 'video') {
                            tt.shareAppMessage(__assign(__assign({}, params), { channel: 'video', title: title, query: query, extra: __assign(__assign({}, params.extra || {}), { cutTemplateId: ((_5 = params.extra) === null || _5 === void 0 ? void 0 : _5.cutTemplateId) || ((_7 = (_6 = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _6 === void 0 ? void 0 : _6.content) === null || _7 === void 0 ? void 0 : _7.share_id) || '', video_title: ((_8 = params.extra) === null || _8 === void 0 ? void 0 : _8.video_title) || params.title || title, videoTopics: ((_9 = params.extra) === null || _9 === void 0 ? void 0 : _9.videoTopics) || ((_11 = (_10 = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _10 === void 0 ? void 0 : _10.content) === null || _11 === void 0 ? void 0 : _11.video_contents) || [], hashtag_list: ((_12 = params.extra) === null || _12 === void 0 ? void 0 : _12.hashtag_list) || ((_14 = (_13 = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _13 === void 0 ? void 0 : _13.content) === null || _14 === void 0 ? void 0 : _14.video_contents) || [] }), success: function () {
                                    callback.complete(__assign({ code: 0, msg: '分享成功' }, shareData_1));
                                }, fail: function (err) {
                                    (0, utils_2.handleTrackError)(PLATFORM, 'rxlog_error_share', err);
                                    callback.complete({
                                        code: -1,
                                        msg: '分享失败'
                                    });
                                } }));
                        }
                        else if (((_16 = (_15 = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _15 === void 0 ? void 0 : _15.content) === null || _16 === void 0 ? void 0 : _16.material_type) === 'card' || ((_18 = (_17 = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _17 === void 0 ? void 0 : _17.content) === null || _18 === void 0 ? void 0 : _18.material_type) === 'words') {
                            tt.shareAppMessage(__assign(__assign({}, params), { extra: params.extra || {}, templateId: params.templateId || ((_20 = (_19 = shareData_1 === null || shareData_1 === void 0 ? void 0 : shareData_1.data) === null || _19 === void 0 ? void 0 : _19.content) === null || _20 === void 0 ? void 0 : _20.share_id) || '', title: title, query: query, success: function () {
                                    callback.complete(__assign({ code: 0, msg: '分享成功' }, shareData_1));
                                }, fail: function (err) {
                                    (0, utils_2.handleTrackError)(PLATFORM, 'rxlog_error_share', err);
                                    callback.complete({
                                        code: -1,
                                        msg: '分享失败'
                                    });
                                } }));
                        }
                        else {
                            tt.shareAppMessage(__assign(__assign({}, params), { extra: params.extra || {}, title: title, query: query, success: function () {
                                    callback.complete({
                                        code: 0,
                                        msg: '分享成功'
                                    });
                                }, fail: function (err) {
                                    (0, utils_2.handleTrackError)(PLATFORM, 'rxlog_error_share', err);
                                    callback.complete({
                                        code: -1,
                                        msg: '分享失败'
                                    });
                                } }));
                        }
                        return [3 /*break*/, 3];
                    case 2:
                        err_3 = _21.sent();
                        callback.complete((0, utils_2.handleTrackError)(PLATFORM, 'rxlog_error_share', err_3));
                        this.track((0, utils_1.formatTrackParams)({
                            eventName: 'track_err',
                            apiName: 'share',
                            reqParams: params,
                            errorInfo: err_3,
                            loginInfo: config_1.USER_INFO
                        }), {
                            complete: function (data) {
                                console.info('share error add complete func when tracked:', data);
                            }
                        });
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    SdkDouyin.prototype.schedulingAction = function (params, callback) {
        var _a, _b, _c, _d;
        return __awaiter(this, void 0, void 0, function () {
            var func, schedulingRes, scheduling_type, shareData, adUnitId, err_4;
            return __generator(this, function (_e) {
                switch (_e.label) {
                    case 0:
                        _e.trys.push([0, 2, , 3]);
                        func = params === null || params === void 0 ? void 0 : params.func;
                        schedulingRes = this.getShareScheduling({ funcs: [func] });
                        scheduling_type = ((_b = (_a = schedulingRes === null || schedulingRes === void 0 ? void 0 : schedulingRes.data) === null || _a === void 0 ? void 0 : _a[func]) === null || _b === void 0 ? void 0 : _b.scheduling_type) || 'share';
                        console.log('sdk schedulingAction scheduling_type:', func, scheduling_type);
                        return [4 /*yield*/, this.getShareData(params, callback, true)];
                    case 1:
                        shareData = _e.sent();
                        console.log('sdk getShareData:', shareData);
                        if (scheduling_type === 'ad') {
                            adUnitId = params.adUnitId || ((_d = (_c = shareData === null || shareData === void 0 ? void 0 : shareData.data) === null || _c === void 0 ? void 0 : _c.ad_content) === null || _d === void 0 ? void 0 : _d.identify);
                            this.rewardedVideoAd({
                                adUnitId: adUnitId,
                                custom_ext: params.custom_ext
                            }, {
                                complete: 
                                // @ts-ignore
                                function (args) {
                                    callback.complete(__assign({ scheduling_type: 'ad' }, (args || {})));
                                }
                            });
                        }
                        else if (scheduling_type === 'share') {
                            this.share(params, {
                                complete: 
                                // @ts-ignore
                                function (args) {
                                    callback.complete(__assign({ scheduling_type: 'share' }, (args || {})));
                                }
                            });
                        }
                        return [3 /*break*/, 3];
                    case 2:
                        err_4 = _e.sent();
                        callback.complete((0, utils_2.handleTrackError)(PLATFORM, 'rxlog_error_share', err_4));
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    //获得分享内容
    SdkDouyin.prototype.getAdShareData = function (params, callback) {
        return __awaiter(this, void 0, void 0, function () {
            var region, productId, channelId, platform, transmits, func, type, sub_channel_id, open_id, shareData, err_5;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        region = (params === null || params === void 0 ? void 0 : params.region) || config_1.USER_INFO.region || '';
                        productId = config_1.SYSTEM_INFO.productId, channelId = config_1.SYSTEM_INFO.channelId;
                        platform = PLATFORM;
                        transmits = encodeURI(params.transmits || '');
                        func = params.func;
                        type = 'mini';
                        sub_channel_id = config_1.USER_INFO.subchannelid || '';
                        open_id = config_1.USER_INFO.openid;
                        return [4 /*yield*/, (0, apis_1.getAdShareDataApi)({
                                func: func,
                                transmits: transmits,
                                product_id: productId,
                                channel_id: channelId,
                                platform: platform,
                                type: type,
                                region: region,
                                sub_channel_id: sub_channel_id,
                                open_id: open_id,
                                custom_ext: params.custom_ext
                            })];
                    case 1:
                        shareData = _a.sent();
                        callback && callback.complete(shareData);
                        return [2 /*return*/, shareData];
                    case 2:
                        err_5 = _a.sent();
                        callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err_5));
                        return [2 /*return*/, err_5];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    //激励广告
    SdkDouyin.prototype.rewardedVideoAd = function (data, _a) {
        var _b, _c;
        var complete = _a.complete, failCallback = _a.fail;
        return __awaiter(this, void 0, void 0, function () {
            var adShareData, adUnitId, fail, ad_1, onClose_1, catchLoadAndShowError_1, error_2;
            var _this = this;
            return __generator(this, function (_d) {
                switch (_d.label) {
                    case 0:
                        adShareData = {};
                        if (!(!data.adUnitId && data.func)) return [3 /*break*/, 2];
                        return [4 /*yield*/, this.getAdShareData({
                                func: data.func,
                                custom_ext: data.custom_ext || {}
                            })];
                    case 1:
                        adShareData = _d.sent();
                        console.log('ad share data', adShareData);
                        _d.label = 2;
                    case 2:
                        adUnitId = data.adUnitId || ((_c = (_b = adShareData === null || adShareData === void 0 ? void 0 : adShareData.data) === null || _b === void 0 ? void 0 : _b.ad_content) === null || _c === void 0 ? void 0 : _c.identify);
                        fail = function (error) {
                            error.message = error.message || error.errMsg || error.errorMsg || error.err_msg || error.error_msg || error.msg || error.error;
                            var _error = (0, utils_2.handleTrackError)(PLATFORM, 'rxlog_error_ad', error);
                            complete(_error);
                            failCallback && failCallback(_error);
                            _this.track((0, utils_1.formatTrackParams)({
                                eventName: 'track_err',
                                apiName: 'rewardedVideoAd',
                                reqParams: data,
                                errorInfo: error,
                                loginInfo: config_1.USER_INFO
                            }), {
                                complete: function (data) {
                                    console.info('rewardedVideoAd error add complete func when tracked:', data);
                                }
                            });
                        };
                        _d.label = 3;
                    case 3:
                        _d.trys.push([3, 6, , 7]);
                        onClose_1 = function (_a) {
                            var isEnded = _a.isEnded;
                            return __awaiter(_this, void 0, void 0, function () {
                                var _this = this;
                                return __generator(this, function (_b) {
                                    ad_1.offClose(onClose_1);
                                    if (data.destroyAd) {
                                        ad_1.destroy().then(function () {
                                            _this._rewardedVideoAd = null;
                                            if (isEnded) {
                                                complete({
                                                    code: 0,
                                                    data: null,
                                                    msg: isEnded,
                                                    isEnded: isEnded
                                                });
                                            }
                                            else {
                                                complete({
                                                    code: -1,
                                                    data: null,
                                                    msg: isEnded,
                                                    isEnded: isEnded
                                                });
                                            }
                                        }).catch(function (err) {
                                            complete((0, utils_2.handleTrackError)(PLATFORM, 'rxlog_error_ad', err));
                                        });
                                    }
                                    else {
                                        if (isEnded) {
                                            complete({
                                                code: 0,
                                                data: null,
                                                msg: isEnded,
                                                isEnded: isEnded
                                            });
                                        }
                                        else {
                                            complete({
                                                code: -1,
                                                data: null,
                                                msg: isEnded,
                                                isEnded: isEnded
                                            });
                                        }
                                    }
                                    return [2 /*return*/];
                                });
                            });
                        };
                        if (!!this._rewardedVideoAd) return [3 /*break*/, 5];
                        ad_1 = tt.createRewardedVideoAd({
                            adUnitId: adUnitId
                        });
                        return [4 /*yield*/, new Promise(function (resolve, reject) {
                                var timer = setTimeout(function () {
                                    reject({ code: 1000000, msg: 'adLoadTimeout' });
                                    clearTimeout(timer);
                                    timer = null;
                                }, 10000);
                                ad_1.onLoad(function () {
                                    _this._rewardedVideoAd = ad_1;
                                    _this._hasAd.rewarded = true;
                                    resolve();
                                });
                                ad_1.onError(function (error) {
                                    _this._hasAd.rewarded = false;
                                    reject(error);
                                });
                                ad_1.load();
                            })];
                    case 4:
                        _d.sent();
                        _d.label = 5;
                    case 5:
                        ad_1 = this._rewardedVideoAd;
                        if (data.isCheck) {
                            complete(__assign(__assign({ code: 0 }, data), { adUnitId: adUnitId, isEnded: false, ad: ad_1 }));
                        }
                        else {
                            ad_1.onClose(onClose_1);
                            catchLoadAndShowError_1 = function (error) { return __awaiter(_this, void 0, void 0, function () {
                                return __generator(this, function (_a) {
                                    fail(error);
                                    return [2 /*return*/];
                                });
                            }); };
                            if (!this._hasAd.rewarded) {
                                ad_1.load()
                                    .then(function () {
                                    ad_1.show().catch(function () {
                                        ad_1.load()
                                            .then(function () { return ad_1.show(); })
                                            .catch(catchLoadAndShowError_1);
                                    });
                                })
                                    .catch(catchLoadAndShowError_1);
                                return [2 /*return*/];
                            }
                            if (!data.isCheck) {
                                ad_1.show().catch(function () {
                                    ad_1.load()
                                        .then(function () { return ad_1.show(); })
                                        .catch(catchLoadAndShowError_1);
                                });
                            }
                        }
                        return [3 /*break*/, 7];
                    case 6:
                        error_2 = _d.sent();
                        fail(error_2);
                        return [3 /*break*/, 7];
                    case 7: return [2 /*return*/];
                }
            });
        });
    };
    //创建os接口
    SdkDouyin.prototype.openCustomServiceForOs = function (params, callback) {
        return __awaiter(this, void 0, void 0, function () {
            var err_6;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, (0, utils_1.asyncFunc)(tt.openAwemeCustomerService, params)];
                    case 1:
                        _a.sent();
                        callback.complete({
                            code: 0
                        });
                        return [3 /*break*/, 3];
                    case 2:
                        err_6 = _a.sent();
                        (0, utils_2.handleTrackError)(PLATFORM, '', err_6);
                        callback.complete({
                            code: -1
                        });
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    //创建客服按钮
    SdkDouyin.prototype.createContactButton = function (params, callback) {
        return __awaiter(this, void 0, void 0, function () {
            var btn, err_7;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, (0, paramsValid_1.pubCheck)(checkConfig_1.douyinServiceCheckParams, callback, params)];
                    case 1:
                        _a.sent();
                        btn = tt.createContactButton(params);
                        callback.complete(btn);
                        return [3 /*break*/, 3];
                    case 2:
                        err_7 = _a.sent();
                        callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err_7));
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    //跳转客服
    SdkDouyin.prototype.openCustomService = function (params, callback) {
        var _a;
        return __awaiter(this, void 0, void 0, function () {
            var _b, sessionFrom, _params, infoResult, e_1, err_8;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        if (params.type === 3) {
                            tt.openCustomerServiceConversation(__assign(__assign({}, params), { success: function (res) {
                                    callback.complete({
                                        code: 0
                                    });
                                }, fail: function (err) {
                                    callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err, -1));
                                } }));
                            return [2 /*return*/];
                        }
                        _c.label = 1;
                    case 1:
                        _c.trys.push([1, 7, , 8]);
                        _b = params.sessionFrom, sessionFrom = _b === void 0 ? '{}' : _b, _params = __rest(params, ["sessionFrom"]);
                        infoResult = {};
                        _c.label = 2;
                    case 2:
                        _c.trys.push([2, 4, , 5]);
                        return [4 /*yield*/, (0, apis_1._getInfoApi)()];
                    case 3:
                        infoResult = _c.sent();
                        return [3 /*break*/, 5];
                    case 4:
                        e_1 = _c.sent();
                        console.log(e_1);
                        return [3 /*break*/, 5];
                    case 5:
                        console.log('sessionFrom', sessionFrom);
                        if (typeof ((_a = infoResult === null || infoResult === void 0 ? void 0 : infoResult.data) === null || _a === void 0 ? void 0 : _a.r_mode) === 'number') {
                            if (typeof sessionFrom === 'object') {
                                sessionFrom = __assign(__assign({}, sessionFrom), { r_mode: infoResult === null || infoResult === void 0 ? void 0 : infoResult.data.r_mode });
                            }
                            else {
                                try {
                                    sessionFrom = __assign(__assign({}, JSON.parse(sessionFrom)), { r_mode: infoResult === null || infoResult === void 0 ? void 0 : infoResult.data.r_mode });
                                    sessionFrom = JSON.stringify(sessionFrom);
                                }
                                catch (e) {
                                    console.log('e', e);
                                }
                            }
                        }
                        if (sessionFrom) {
                            // @ts-ignore
                            _params.sessionFrom = sessionFrom;
                        }
                        console.log(_params);
                        return [4 /*yield*/, (0, utils_1.asyncFunc)(tt.openCustomerServiceConversation, _params)];
                    case 6:
                        _c.sent();
                        callback.complete({
                            code: 0
                        });
                        return [3 /*break*/, 8];
                    case 7:
                        err_8 = _c.sent();
                        callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err_8, -1));
                        return [3 /*break*/, 8];
                    case 8: return [2 /*return*/];
                }
            });
        });
    };
    //拉起实名认证
    SdkDouyin.prototype.authenticateRealName = function (_a) {
        var complete = _a.complete;
        return __awaiter(this, void 0, void 0, function () {
            var err_9;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        _b.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, (0, utils_1.asyncFunc)(tt.authenticateRealName)];
                    case 1:
                        _b.sent();
                        complete({ code: 0 });
                        return [3 /*break*/, 3];
                    case 2:
                        err_9 = _b.sent();
                        (0, utils_2.handleTrackError)(PLATFORM, '', err_9);
                        complete({ code: -1, data: err_9 });
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    //录制屏幕
    SdkDouyin.prototype.getGameRecorderManager = function (_a) {
        var complete = _a.complete;
        return __awaiter(this, void 0, void 0, function () {
            var GameRecorderManager;
            return __generator(this, function (_b) {
                GameRecorderManager = tt.getGameRecorderManager();
                complete({ code: 0, data: GameRecorderManager });
                return [2 /*return*/];
            });
        });
    };
    SdkDouyin.prototype.setScheuleReportProps = function (data) {
        var _a, _b, _c, _d, _e, _f, _g, _h;
        this.scheuleReportProps = {
            trigger_tag: ((_a = data === null || data === void 0 ? void 0 : data.trigger) === null || _a === void 0 ? void 0 : _a.tag) || '',
            trigger_id: ((_b = data === null || data === void 0 ? void 0 : data.trigger) === null || _b === void 0 ? void 0 : _b.id) || 0,
            trigger_type: ((_c = data === null || data === void 0 ? void 0 : data.trigger) === null || _c === void 0 ? void 0 : _c.type) || 0,
            material_type: ((_d = data === null || data === void 0 ? void 0 : data.content) === null || _d === void 0 ? void 0 : _d.material_type) || '',
            material_id: ((_e = data === null || data === void 0 ? void 0 : data.content) === null || _e === void 0 ? void 0 : _e.material_id) || 0,
            landing_id: ((_f = data === null || data === void 0 ? void 0 : data.content) === null || _f === void 0 ? void 0 : _f.landing_id) || 0,
            strategy_id: ((_g = data === null || data === void 0 ? void 0 : data.strategy) === null || _g === void 0 ? void 0 : _g.id) || 0,
            strategy_type: ((_h = data === null || data === void 0 ? void 0 : data.strategy) === null || _h === void 0 ? void 0 : _h.type) || 0,
            platform: (data === null || data === void 0 ? void 0 : data.platform) || PLATFORM
        };
    };
    // 获得公共属性
    SdkDouyin.prototype.getPublicProperties = function () {
        var data = (0, utils_2.customGetStorageSync)("rx_public_props");
        return { code: 0, data: data };
    };
    /**
     * 设置公共属性
     * 设置后CP无需每次上报都传，由SDK填入properties中。
     */
    SdkDouyin.prototype.setPublicProperties = function (params) {
        if (!(0, is_1.isObject)(params)) {
            var error = new Error('params must be object');
            error.code = const_1.COMMON_ERROR_CODE.PAY_PARAMS_ERROR;
            return (0, utils_2.handleTrackError)(PLATFORM, '', error);
        }
        try {
            (0, utils_2.customSetStorageSync)('rx_public_props', params);
            return { code: 0 };
        }
        catch (error) {
            return (0, utils_2.handleTrackError)(PLATFORM, '', error);
        }
    };
    /**
     * 修改设置的公共数据。
     */
    SdkDouyin.prototype.updatePublicProperties = function (params) {
        if (!(0, is_1.isObject)(params)) {
            var error = new Error('params must be object');
            error.code = const_1.COMMON_ERROR_CODE.PAY_PARAMS_ERROR;
            return (0, utils_2.handleTrackError)(PLATFORM, '', error);
        }
        try {
            var cache = (0, utils_2.customGetStorageSync)('rx_public_props');
            // @ts-ignore
            (0, utils_2.customSetStorageSync)('rx_public_props', __assign(__assign({}, cache), params));
            return { code: 0 };
        }
        catch (error) {
            return (0, utils_2.handleTrackError)(PLATFORM, '', error);
        }
    };
    /**
     * 删除公共属性
     */
    SdkDouyin.prototype.deletePublicProperties = function (params) {
        try {
            var cache = (0, utils_2.customGetStorageSync)('rx_public_props');
            // @ts-ignore
            var rest = (0, is_1.omit)(cache, params);
            (0, utils_2.customSetStorageSync)('rx_public_props', rest);
            return { code: 0 };
        }
        catch (error) {
            return (0, utils_2.handleTrackError)(PLATFORM, '', error);
        }
    };
    //获得分享内容
    SdkDouyin.prototype.getShareData = function (params, callback, stopCallback) {
        return __awaiter(this, void 0, void 0, function () {
            var region, cacheShareData, _a, readCache, cShareData, productId, channelId, platform, transmits, func, type, sub_channel_id, open_id, shareData, err_10;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        _b.trys.push([0, 2, , 6]);
                        region = (params === null || params === void 0 ? void 0 : params.region) || config_1.USER_INFO.region || '';
                        cacheShareData = (0, utils_2.customGetStorageSync)("rx_schedule_".concat(config_1.USER_INFO.tid, "_").concat(params.func, "_").concat(region));
                        _a = params.readCache, readCache = _a === void 0 ? true : _a;
                        if (readCache && cacheShareData) {
                            cShareData = JSON.parse(cacheShareData);
                            console.info('sdk 缓存分享数据：', cShareData);
                            this.setScheuleReportProps(cShareData === null || cShareData === void 0 ? void 0 : cShareData.data);
                            !stopCallback && callback.complete(cShareData);
                            return [2 /*return*/, cShareData];
                        }
                        productId = config_1.SYSTEM_INFO.productId, channelId = config_1.SYSTEM_INFO.channelId;
                        platform = PLATFORM;
                        transmits = encodeURI(params.transmits || '');
                        func = params.func;
                        type = 'mini';
                        sub_channel_id = config_1.USER_INFO.subchannelid || '';
                        open_id = config_1.USER_INFO.openid;
                        return [4 /*yield*/, (0, apis_1.getShareDataApi)({
                                func: func,
                                transmits: transmits,
                                product_id: productId,
                                channel_id: channelId,
                                platform: platform,
                                type: type,
                                region: region,
                                sub_channel_id: sub_channel_id,
                                open_id: open_id,
                                custom_ext: params.custom_ext || {}
                            })];
                    case 1:
                        shareData = _b.sent();
                        if (!stopCallback) {
                            callback.complete(shareData);
                        }
                        this.setScheuleReportProps(shareData === null || shareData === void 0 ? void 0 : shareData.data);
                        return [2 /*return*/, shareData];
                    case 2:
                        err_10 = _b.sent();
                        if (!(err_10.code == 305407)) return [3 /*break*/, 4];
                        return [4 /*yield*/, this.shareSchedulingInit({}, {
                                complete: function () {
                                    if (!stopCallback) {
                                        callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err_10));
                                    }
                                }
                            })];
                    case 3:
                        _b.sent();
                        return [3 /*break*/, 5];
                    case 4:
                        if (!stopCallback) {
                            callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err_10));
                        }
                        _b.label = 5;
                    case 5:
                        this.track((0, utils_1.formatTrackParams)({
                            eventName: 'track_err',
                            apiName: 'getShareData',
                            reqParams: params,
                            errorInfo: err_10,
                            loginInfo: config_1.USER_INFO
                        }), {
                            complete: function (data) {
                                console.info('getShareData error add complete func when tracked:', data);
                            }
                        });
                        return [2 /*return*/, (0, utils_2.handleTrackError)(PLATFORM, '', err_10)];
                    case 6: return [2 /*return*/];
                }
            });
        });
    };
    // 获取埋点调度
    SdkDouyin.prototype.getShareScheduling = function (params) {
        var funcs = params === null || params === void 0 ? void 0 : params.funcs;
        if (!funcs)
            return { code: 0, data: this.scheduleInitMap };
        if (funcs && !(0, is_1.isArray)(funcs)) {
            var error = new Error('funcs must be Array');
            error.code = const_1.COMMON_ERROR_CODE.PARAMS_ERROR;
            return (0, utils_2.handleTrackError)(PLATFORM, '', error);
        }
        try {
            console.log('sdk getShareScheduling: ', params, this.scheduleInitMap);
            var data = (0, is_1.pick)(this.scheduleInitMap, funcs);
            return { code: 0, data: data };
        }
        catch (error) {
            return (0, utils_2.handleTrackError)(PLATFORM, '', error);
        }
    };
    // 分享调度初始化
    SdkDouyin.prototype.shareSchedulingInit = function (params, callback) {
        return __awaiter(this, void 0, void 0, function () {
            var req, res, error_3;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 3, , 4]);
                        return [4 /*yield*/, (0, paramsValid_1.pubCheck)(checkConfig_1.shareScheduleInitParams, callback, params)];
                    case 1:
                        _a.sent();
                        this.funcs = (params === null || params === void 0 ? void 0 : params.funcs) || [];
                        req = {
                            func: this.funcs,
                            type: 'mini',
                            open_id: config_1.USER_INFO.openid || ''
                        };
                        return [4 /*yield*/, (0, apis_1.schedulingInitApi)(req)];
                    case 2:
                        res = _a.sent();
                        this.scheduleInitMap = (res === null || res === void 0 ? void 0 : res.data) || {};
                        (0, utils_2.removeStorageByPrefix)('rx_schedule');
                        callback.complete(res);
                        return [3 /*break*/, 4];
                    case 3:
                        error_3 = _a.sent();
                        return [3 /*break*/, 4];
                    case 4: return [2 /*return*/];
                }
            });
        });
    };
    // 看广告完成上报
    SdkDouyin.prototype.shareSchedulingReport = function (params, callback) {
        var _a, _b, _c;
        return __awaiter(this, void 0, void 0, function () {
            var func, region, sub_channel_id, open_id, scheduling_event, Iparams, result_1, remaining_share_count, error_4;
            return __generator(this, function (_d) {
                switch (_d.label) {
                    case 0:
                        _d.trys.push([0, 8, , 9]);
                        return [4 /*yield*/, (0, paramsValid_1.pubCheck)(checkConfig_1.shareScheduleReportParams, callback, params)];
                    case 1:
                        _d.sent();
                        func = params.func;
                        region = (params === null || params === void 0 ? void 0 : params.region) || config_1.USER_INFO.region || '';
                        sub_channel_id = config_1.USER_INFO.subchannelid || '';
                        open_id = config_1.USER_INFO.openid || '';
                        scheduling_event = (params === null || params === void 0 ? void 0 : params.scheduling_event) === true ? 'done' : 'fail';
                        Iparams = __assign(__assign({ platform: PLATFORM, type: 'mini', sub_channel_id: sub_channel_id, open_id: open_id }, params), { region: region, scheduling_event: scheduling_event, properties: __assign({ region: region }, params === null || params === void 0 ? void 0 : params.properties) });
                        // ad不上报上一次的分享数据
                        if (params.scheduling_type == 'share') {
                            Iparams.properties = __assign(__assign({}, this.scheuleReportProps), Iparams.properties);
                        }
                        return [4 /*yield*/, (0, apis_1.schedulingReportApi)(Iparams)];
                    case 2:
                        result_1 = _d.sent();
                        if (!(0, is_1.isEmpty)(result_1 === null || result_1 === void 0 ? void 0 : result_1.data)) return [3 /*break*/, 4];
                        console.log('上报返回为空，对应埋点删除');
                        this.scheduleInitMap = (0, is_1.omit)(this.scheduleInitMap, func);
                        (0, utils_2.removeStorageSync)("rx_schedule_".concat(config_1.USER_INFO.tid, "_").concat(func, "_").concat(region));
                        return [4 /*yield*/, this.shareSchedulingInit({}, {
                                complete: function () {
                                    console.log('shareSchedulingInit');
                                    callback.complete(result_1);
                                }
                            })];
                    case 3:
                        _d.sent();
                        return [2 /*return*/];
                    case 4:
                        remaining_share_count = ((_b = (_a = result_1 === null || result_1 === void 0 ? void 0 : result_1.data) === null || _a === void 0 ? void 0 : _a.scheduling) === null || _b === void 0 ? void 0 : _b.remaining_share_count) || 0;
                        console.log('上报后剩余次数为' + remaining_share_count);
                        if (!(remaining_share_count <= 0)) return [3 /*break*/, 6];
                        return [4 /*yield*/, this.shareSchedulingInit({}, {
                                complete: function () {
                                    console.log('shareSchedulingInit');
                                    callback.complete(result_1);
                                }
                            })];
                    case 5:
                        _d.sent();
                        return [2 /*return*/];
                    case 6:
                        this.scheduleInitMap[func] = (_c = result_1 === null || result_1 === void 0 ? void 0 : result_1.data) === null || _c === void 0 ? void 0 : _c.scheduling;
                        (0, utils_2.customSetStorageSync)("rx_schedule_".concat(config_1.USER_INFO.tid, "_").concat(func, "_").concat(region), JSON.stringify(result_1));
                        _d.label = 7;
                    case 7:
                        callback.complete(result_1);
                        return [3 /*break*/, 9];
                    case 8:
                        error_4 = _d.sent();
                        callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', error_4));
                        return [3 /*break*/, 9];
                    case 9: return [2 /*return*/];
                }
            });
        });
    };
    SdkDouyin.prototype.getInitConfig = function (callback) {
        var _a, _b, _c, _d, _e, _f, _g;
        return __awaiter(this, void 0, void 0, function () {
            var initParams, res, config, version, _i, _h, key, prop_version, _serverTime, err_11, error;
            return __generator(this, function (_j) {
                switch (_j.label) {
                    case 0:
                        initParams = (0, utils_2.customGetStorageSync)('rx-init-params') || {};
                        _j.label = 1;
                    case 1:
                        _j.trys.push([1, 3, , 4]);
                        return [4 /*yield*/, (0, apis_1.getInitConf)({ version: (_a = initParams === null || initParams === void 0 ? void 0 : initParams.version) !== null && _a !== void 0 ? _a : {} })];
                    case 2:
                        res = _j.sent();
                        config = res.data || {};
                        version = {};
                        for (_i = 0, _h = Object.keys(config); _i < _h.length; _i++) {
                            key = _h[_i];
                            prop_version = (_c = (_b = config[key]) === null || _b === void 0 ? void 0 : _b.version) !== null && _c !== void 0 ? _c : '';
                            if (prop_version) {
                                version[key] = prop_version;
                                this.initConfig[key] = { timerId: 0 };
                            }
                            this.initConfig[key] = config[key];
                        }
                        //检查是否需要传递subchannleid
                        this.publicSubchannelCheck(res);
                        (0, utils_2.customSetStorageSync)('rx-init-params', { version: version });
                        config_1.SYSTEM_INFO.SDK_INIT_FINISHED = true;
                        config_1.SYSTEM_INFO.CP_OF = ((_e = (_d = res === null || res === void 0 ? void 0 : res.data) === null || _d === void 0 ? void 0 : _d.cp) === null || _e === void 0 ? void 0 : _e.of) || false;
                        _serverTime = (_g = (_f = res === null || res === void 0 ? void 0 : res.data) === null || _f === void 0 ? void 0 : _f.server) === null || _g === void 0 ? void 0 : _g.time;
                        if (_serverTime) {
                            config_1.SYSTEM_INFO.st_offset = String(Number(_serverTime) - Date.now());
                        }
                        // 初始化成功后监听应用进入前台，刷新 st_offset
                        (0, stOffset_1.setupStOffsetRefreshForMiniGame)(typeof tt !== 'undefined' ? tt : null, apis_1.getServerTime);
                        this.saveDeviceInfo();
                        // 检查是否需要激活
                        this.checkNeedActivate();
                        // this.loopGetPublicProps()
                        callback.complete({ code: 0, data: this.initConfig });
                        return [3 /*break*/, 4];
                    case 3:
                        err_11 = _j.sent();
                        error = __assign(__assign({}, (err_11 || {})), { msg: '初始化错误，或未初始化', code: const_1.COMMON_ERROR_CODE.INIT_PARAMS_ERROR, thirdcode: err_11.code || err_11.errCode, message: err_11.message || err_11.msg || err_11.errMsg, thirdmsg: err_11.message || err_11.msg || err_11.errMsg });
                        callback.complete((0, utils_2.handleTrackError)(PLATFORM, 'rxlog_error_init', error));
                        return [3 /*break*/, 4];
                    case 4: return [2 /*return*/];
                }
            });
        });
    };
    SdkDouyin.prototype.publicSubchannelCheck = function (res) {
        var _a, _b;
        try {
            var sub_channel = (_b = (_a = res === null || res === void 0 ? void 0 : res.data) === null || _a === void 0 ? void 0 : _a.subcq) === null || _b === void 0 ? void 0 : _b.subc;
            var queryString = (0, utils_2.getSearchQueries)(true);
            var query = queryString ? queryString.split('&') : [];
            if ((sub_channel === null || sub_channel === void 0 ? void 0 : sub_channel.length) && (query === null || query === void 0 ? void 0 : query.length)) {
                for (var a = 0; a < sub_channel.length; a++) {
                    var item = sub_channel[a];
                    var reflectStringArr = item === null || item === void 0 ? void 0 : item.map;
                    if (reflectStringArr === null || reflectStringArr === void 0 ? void 0 : reflectStringArr.length) {
                        var arr = item === null || item === void 0 ? void 0 : item.map;
                        var sub_channel_id = item === null || item === void 0 ? void 0 : item.id;
                        for (var k in arr) {
                            var str = arr[k];
                            for (var c in query) {
                                if (str.includes(query[c])) {
                                    this.subChannelId = sub_channel_id;
                                    return;
                                }
                            }
                        }
                    }
                }
            }
        }
        catch (err) {
        }
    };
    // 获取归因数据
    SdkDouyin.prototype.getAttributionData = function () {
        var universal = (0, utils_2.getSearchQueries)();
        if (universal === null || universal === void 0 ? void 0 : universal.ad_platform) {
            var source_ad = {};
            source_ad.ad_rawargs = (0, is_1.omit)(universal, ['ad_platform']);
            source_ad.ad_platform = universal.ad_platform;
            return source_ad;
        }
        else {
            return {
                ad_rawargs: universal
            };
        }
    };
    SdkDouyin.prototype.checkNeedActivate = function () {
        return __awaiter(this, void 0, void 0, function () {
            var activeResult, source_ad, distinct_id, req, result, err_12;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        activeResult = (0, utils_2.customGetStorageSync)('rx-active-result');
                        if (!!activeResult) return [3 /*break*/, 4];
                        source_ad = this.getAttributionData();
                        distinct_id = (0, v4_1.default)();
                        (0, utils_2.customSetStorageSync)('rx_distinct_id', distinct_id);
                        req = {
                            stage: 'init',
                            distinct_id: distinct_id,
                            source_ad: source_ad
                        };
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 3, , 4]);
                        return [4 /*yield*/, (0, apis_1.activated)(req)];
                    case 2:
                        result = _a.sent();
                        (0, utils_2.customSetStorageSync)('rx-active-result', { isSuccess: true, activeResult: result.data });
                        return [3 /*break*/, 4];
                    case 3:
                        err_12 = _a.sent();
                        (0, utils_2.customSetStorageSync)('rx-active-result', { isSuccess: false, activeResult: req });
                        return [3 /*break*/, 4];
                    case 4: return [2 /*return*/];
                }
            });
        });
    };
    /**
     * 轮训获取公共属性
     *
     */
    SdkDouyin.prototype.loopGetPublicProps = function () {
        var _a;
        return __awaiter(this, void 0, void 0, function () {
            var event_public_attr, repeat, getPublicPropsConfig;
            var _this = this;
            return __generator(this, function (_b) {
                event_public_attr = (_a = this.initConfig) === null || _a === void 0 ? void 0 : _a.event_public_attr;
                if ((0, is_1.isEmpty)(event_public_attr))
                    return [2 /*return*/];
                repeat = function (ms) {
                    event_public_attr.timerId && clearTimeout(event_public_attr.timerId);
                    event_public_attr.timerId = setTimeout(function () { return getPublicPropsConfig(); }, ms || _this.businessRuleDefaultRefreshTime);
                };
                getPublicPropsConfig = function () { return __awaiter(_this, void 0, void 0, function () {
                    var res, _a, _b, refresh, public_attr, _c, version, initParams, error_5;
                    return __generator(this, function (_d) {
                        switch (_d.label) {
                            case 0:
                                _d.trys.push([0, 2, , 3]);
                                return [4 /*yield*/, (0, apis_1.getPublicProps)(event_public_attr.version)];
                            case 1:
                                res = _d.sent();
                                _a = (res === null || res === void 0 ? void 0 : res.data) || {}, _b = _a.refresh, refresh = _b === void 0 ? this.businessRuleDefaultRefreshTime : _b, public_attr = _a.public_attr, _c = _a.version, version = _c === void 0 ? '' : _c;
                                event_public_attr.public_attr = public_attr || event_public_attr.public_attr;
                                event_public_attr.refresh = refresh;
                                event_public_attr.version = version;
                                initParams = (0, utils_2.customGetStorageSync)('rx-init-params');
                                // 获取到最新的version后更新到缓存中，下次初始化的时候用这个最新的version请求初始化配置接口
                                (0, utils_2.customSetStorageSync)('rx-init-params', __assign(__assign({}, initParams), { version: __assign(__assign({}, initParams.version), { event_public_attr: version }) }));
                                repeat(event_public_attr.refresh);
                                return [3 /*break*/, 3];
                            case 2:
                                error_5 = _d.sent();
                                (0, utils_2.handleTrackError)(PLATFORM, '', error_5);
                                if (this.trackPublicPropsFailCount < 1) {
                                    // 首次获取失败3秒后重试
                                    this.trackPublicPropsFailCount += 1;
                                    repeat(3000);
                                }
                                else {
                                    // 再失败每十分钟后重试，直至成功
                                    this.trackPublicPropsFailCount += 1;
                                    repeat(600000);
                                }
                                return [3 /*break*/, 3];
                            case 3: return [2 /*return*/];
                        }
                    });
                }); };
                repeat(event_public_attr === null || event_public_attr === void 0 ? void 0 : event_public_attr.refresh);
                return [2 /*return*/];
            });
        });
    };
    //格式化queryString
    SdkDouyin.prototype.getLoginQsAndGenerateStruct = function () {
        var _a;
        var universal = (0, utils_2.getSearchQueries)();
        var user_source = {};
        if (universal.hasOwnProperty('user_source')) {
            var omitKeys = (universal === null || universal === void 0 ? void 0 : universal.user_source) === 'transmits' ? ['user_source'] : ['user_source', 'type', 'transmits'];
            var leftProps = __assign({}, (0, is_1.omit)(universal, omitKeys));
            /**
             * url 上有user_source字段并且除了'user_source', 'type', 'transmits'等字段外还有属性，则将剩余属性全部放到universal['user_source']属性下
             * 多包了一层'user_source',使用的地方直接 ...
             */
            if (!(0, is_1.isEmpty)(leftProps)) {
                // 用户透传参数
                if ((universal === null || universal === void 0 ? void 0 : universal.user_source) == 'transmits') {
                    user_source = {
                        user_transmits: Object.assign(leftProps, { transmits: decodeURIComponent(leftProps.transmits || '') })
                    };
                }
                else if ((universal === null || universal === void 0 ? void 0 : universal.user_source) == 'attr') {
                    user_source = {
                        user_attrs: leftProps
                    };
                }
                else {
                    user_source = {
                        user_source: (_a = {},
                            _a[universal['user_source']] = leftProps,
                            _a)
                    };
                }
                return user_source;
            }
        }
        var subPackageInfo = (0, utils_2.customGetStorageSync)('rx_sub_package_info');
        if (!(0, is_1.isEmpty)(subPackageInfo)) {
            user_source = {
                user_source: {
                    sub_package: subPackageInfo
                }
            };
            return user_source;
        }
        return null;
    };
    SdkDouyin.prototype.ActivePrefix = function (reqParams) {
        var loginState = (0, utils_2.customGetStorageSync)('rx-loginState');
        var activeSave = (0, utils_2.customGetStorageSync)('rx-active-result');
        if (loginState || !activeSave) {
            return reqParams;
        }
        else {
            if (activeSave === null || activeSave === void 0 ? void 0 : activeSave.isSuccess) {
                return __assign(__assign({}, reqParams), { activate: { result: activeSave === null || activeSave === void 0 ? void 0 : activeSave.activeResult } });
            }
            else {
                return __assign(__assign({}, reqParams), { activate: { args: activeSave === null || activeSave === void 0 ? void 0 : activeSave.activeResult } });
            }
        }
    };
    /**
     * 用于设置子渠道，通行证记录来源（分包）、子渠道参数
     */
    SdkDouyin.prototype.setSubChannelId = function (subChannelId) {
        try {
            (0, utils_2.customSetStorageSync)('rx_sub_package_info', { sub_channel_id: subChannelId });
            return { code: 0 };
        }
        catch (error) {
            return (0, utils_2.handleTrackError)(PLATFORM, '', error);
        }
    };
    //商业广告
    SdkDouyin.prototype.getAllBusinessData = function (callback) {
        return __awaiter(this, void 0, void 0, function () {
            var data, result;
            return __generator(this, function (_a) {
                try {
                    data = (0, is_1.omit)(this.businessRulesInfo, 'timerId');
                    result = { code: 0, data: data };
                    callback.complete(result);
                }
                catch (error) {
                    callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', error));
                }
                return [2 /*return*/];
            });
        });
    };
    // 条件获取商业化窗口数据
    SdkDouyin.prototype.getBusinessData = function (params, callback) {
        var _a, _b;
        return __awaiter(this, void 0, void 0, function () {
            var checkCache, window_key_1, event_1, _c, before_event, _d, cache_1, cacheKey_1, _e, _f, _g, auto_popups, _h, manual_popups, matchWindows, windows, result, error_6;
            var _this = this;
            return __generator(this, function (_j) {
                switch (_j.label) {
                    case 0:
                        // 如果登录接口内部调用的商业化接口没有返回结果，将此接口按调用次序缓存起来，接口结果回来后一次返回
                        // cp 主动调用更新商业化接口不管，需要他们自己在接口返回后条件获取商业化窗口数据
                        if (this.businessRuleInvoking) {
                            this.businessWindowsQueue.push(function () { return _this.getBusinessData(params, callback); });
                            return [2 /*return*/];
                        }
                        checkCache = function () {
                            var currentDate = (0, day_1.formatDate)('YYYY-MM-DD'); //dayjs().format('YYYY-MM-DD')
                            var cacheKeyPrefix = 'rx_business_popup_';
                            var cacheKey = "".concat(cacheKeyPrefix).concat(currentDate);
                            var cache = (0, utils_2.customGetStorageSync)(cacheKey);
                            if (!cache) {
                                cache = {};
                                (0, utils_2.customSetStorageSync)(cacheKey, {});
                            }
                            // 删除当天之前的商业化窗口缓存
                            (0, utils_2.removeStorageByPrefix)(cacheKeyPrefix, function (key) { return key.startsWith(cacheKeyPrefix) && !key.endsWith(currentDate); });
                            console.info('sdk business window cacheKey: ', cacheKey, ' cache: ', cache);
                            return { cache: cache, cacheKey: cacheKey };
                        };
                        _j.label = 1;
                    case 1:
                        _j.trys.push([1, 3, , 4]);
                        return [4 /*yield*/, (0, paramsValid_1.pubCheck)(checkConfig_1.checkIReqBusinessData, callback, params)];
                    case 2:
                        _j.sent();
                        window_key_1 = params.window_key, event_1 = params.event, _c = params.before_event, before_event = _c === void 0 ? '' : _c;
                        _d = checkCache(), cache_1 = _d.cache, cacheKey_1 = _d.cacheKey;
                        _e = ((_a = this.businessRulesInfo.main_window_list) === null || _a === void 0 ? void 0 : _a.filter(function (window) { return window.window_key === window_key_1; }))[0], _f = _e === void 0 ? {} : _e, _g = _f.auto_popups, auto_popups = _g === void 0 ? {} : _g, _h = _f.manual_popups, manual_popups = _h === void 0 ? {} : _h;
                        matchWindows = [];
                        // 匹配自动窗口, 匹配到自动窗口，忽略前置事件
                        matchWindows = (auto_popups === null || auto_popups === void 0 ? void 0 : auto_popups[event_1]) || [];
                        // 匹配手动窗口
                        if (!(matchWindows === null || matchWindows === void 0 ? void 0 : matchWindows.length)) {
                            matchWindows = ((_b = manual_popups === null || manual_popups === void 0 ? void 0 : manual_popups[event_1]) === null || _b === void 0 ? void 0 : _b[before_event]) || [];
                        }
                        console.info('sdk matchWindows: ', matchWindows);
                        windows = (0, is_1.compact)(matchWindows.map(function (matWindow) {
                            var _a;
                            var windowInfo = (_a = _this.businessRulesInfo.window_list) === null || _a === void 0 ? void 0 : _a.find(function (window) { return window.window_key === matWindow.window_key; });
                            if (windowInfo) {
                                if (!(0, is_1.isNil)(matWindow === null || matWindow === void 0 ? void 0 : matWindow.day_limit)) {
                                    var key = "".concat(window_key_1, "_").concat(event_1, "_").concat(matWindow.window_key);
                                    var count = cache_1[key] || 0;
                                    if ((matWindow === null || matWindow === void 0 ? void 0 : matWindow.day_limit) === count)
                                        return;
                                    cache_1[key] = count + 1;
                                    (0, utils_2.customSetStorageSync)(cacheKey_1, cache_1);
                                }
                                return windowInfo;
                            }
                        }));
                        console.log('result windows: ', windows);
                        result = { code: 0, data: windows };
                        callback.complete(result);
                        return [3 /*break*/, 4];
                    case 3:
                        error_6 = _j.sent();
                        callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', error_6));
                        return [3 /*break*/, 4];
                    case 4: return [2 /*return*/];
                }
            });
        });
    };
    // 更新商业化窗口数据
    SdkDouyin.prototype.refreshBusinessData = function (callback, isRecord) {
        return __awaiter(this, void 0, void 0, function () {
            var res, data, hit_cache, _a, version, _b, refresh_time, _c, main_window_list, _d, window_list, error_7;
            return __generator(this, function (_e) {
                switch (_e.label) {
                    case 0:
                        isRecord && (this.businessRuleInvoking = true);
                        _e.label = 1;
                    case 1:
                        _e.trys.push([1, 3, 4, 5]);
                        return [4 /*yield*/, (0, apis_1.getBusinessRules)(this.businessRulesInfo.version)];
                    case 2:
                        res = _e.sent();
                        data = (res === null || res === void 0 ? void 0 : res.data) || {};
                        hit_cache = data.hit_cache, _a = data.version, version = _a === void 0 ? '' : _a, _b = data.refresh_time, refresh_time = _b === void 0 ? this.businessRuleDefaultRefreshTime : _b;
                        _c = data.main_window_list, main_window_list = _c === void 0 ? [] : _c, _d = data.window_list, window_list = _d === void 0 ? [] : _d;
                        if (!hit_cache) {
                            this.businessRulesInfo.main_window_list = main_window_list;
                            this.businessRulesInfo.window_list = window_list;
                        }
                        this.businessRulesInfo.refresh_time = refresh_time;
                        this.businessRulesInfo.version = version;
                        this.businessRulesInfo.hit_cache = hit_cache;
                        (callback === null || callback === void 0 ? void 0 : callback.complete) && callback.complete({ code: 0 });
                        return [3 /*break*/, 5];
                    case 3:
                        error_7 = _e.sent();
                        (callback === null || callback === void 0 ? void 0 : callback.complete) && callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', error_7));
                        return [3 /*break*/, 5];
                    case 4:
                        isRecord && this.dispatchBusinessWindowsQueue();
                        return [7 /*endfinally*/];
                    case 5: return [2 /*return*/];
                }
            });
        });
    };
    SdkDouyin.prototype.dispatchBusinessWindowsQueue = function () {
        return __awaiter(this, void 0, void 0, function () {
            var execute;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        // console.info('sdk 触发商业化窗口队列释放')
                        this.businessRuleInvoking = false;
                        execute = function () {
                            while (_this.businessWindowsQueue.length) {
                                var queueGetBusinessData = _this.businessWindowsQueue.shift();
                                queueGetBusinessData();
                            }
                        };
                        return [4 /*yield*/, Promise.resolve(execute())];
                    case 1:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        });
    };
    // 商业化下单
    SdkDouyin.prototype.requestBusinessOrder = function (params, callback) {
        return __awaiter(this, void 0, void 0, function () {
            var result, err_13;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 3, , 4]);
                        return [4 /*yield*/, (0, paramsValid_1.pubCheck)(checkConfig_1.checkIReqBusinessOrder, callback, params)];
                    case 1:
                        _a.sent();
                        return [4 /*yield*/, (0, apis_1.businessOrderApi)(params)];
                    case 2:
                        result = _a.sent();
                        callback.complete(result);
                        return [3 /*break*/, 4];
                    case 3:
                        err_13 = _a.sent();
                        callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err_13));
                        return [3 /*break*/, 4];
                    case 4: return [2 /*return*/];
                }
            });
        });
    };
    SdkDouyin.prototype.clearPromoterTimer = function () {
        console.log('clearPromoterTimer');
        if (this.promoInfo.timer) {
            clearTimeout(this.promoInfo.timer);
            this.promoInfo.timer = null;
        }
    };
    // 启动定时器
    SdkDouyin.prototype.startPromoterTimer = function (callback, autoRefresh) {
        var _this = this;
        if (autoRefresh === void 0) { autoRefresh = true; }
        var delay = this.promoInfo.refresh_period_exp < 1 ? (this.promoInfo.polling ? (this.promoInfo.polling * 1000) : 10000) : (this.promoInfo.refresh_period_exp * 1000);
        console.log('startPromoterTimer', delay);
        this.promoInfo.timer = setTimeout(function () {
            _this.getPromoDisplayKEY(callback, autoRefresh, false);
        }, delay);
    };
    // 获取福利码
    SdkDouyin.prototype.getPromoDisplayKEY = function (callback, autoRefresh, immediately) {
        var _this = this;
        if (autoRefresh === void 0) { autoRefresh = false; }
        if (immediately === void 0) { immediately = true; }
        this.clearPromoterTimer();
        var promo_code = this.promoInfo.promo_code;
        (0, apis_1.getPromoterCodeApi)(this.game_id).then(function (res) {
            try {
                if (res.code == 0) {
                    _this.promoInfo.refresh_period_exp = res.data.refresh_period_exp || 0;
                    _this.promoInfo.polling = res.data.polling || 0;
                    promo_code = res.data.promo_code;
                }
            }
            catch (e) {
                _this.promoInfo.refresh_period_exp = 0;
                _this.promoInfo.polling = 0;
            }
            if (autoRefresh) {
                _this.startPromoterTimer(callback, autoRefresh);
            }
            if (!immediately && promo_code == _this.promoInfo.promo_code) {
                return;
            }
            else {
                _this.promoInfo.promo_code = promo_code;
            }
            callback && callback.complete(res);
        }).catch(function (err) {
            if (err.isServerError) {
                _this.clearPromoterTimer();
                callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err));
            }
            else {
                if (autoRefresh) {
                    _this.startPromoterTimer(callback, autoRefresh);
                }
                else {
                    callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err));
                }
            }
        });
    };
    // 兑换福利码
    SdkDouyin.prototype.exchangePromoCDKEY = function (cdkey, callback) {
        (0, apis_1.exchangePromoterCodeApi)(cdkey).then(function (res) {
            callback.complete(res);
        }).catch(function (err) {
            callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err));
        });
    };
    SdkDouyin.prototype.checkIsPromoter = function () {
        return this.isPromoter;
    };
    // 选择视频
    SdkDouyin.prototype.chooseVideo = function (callback) {
        return __awaiter(this, void 0, void 0, function () {
            var _this = this;
            return __generator(this, function (_a) {
                tt.chooseVideo({
                    success: function (res) {
                        (callback === null || callback === void 0 ? void 0 : callback.complete) && callback.complete({ code: 0, data: res });
                    },
                    fail: function (err) { return __awaiter(_this, void 0, void 0, function () {
                        var authSetting, res, openSetting;
                        return __generator(this, function (_a) {
                            switch (_a.label) {
                                case 0:
                                    if (err.errMsg == 'chooseVideo:fail cancel') {
                                        (callback === null || callback === void 0 ? void 0 : callback.complete) && callback.complete({ code: -1, msg: '取消' });
                                        return [2 /*return*/];
                                    }
                                    return [4 /*yield*/, (0, utils_1.asyncFunc)(tt.getSetting)];
                                case 1:
                                    authSetting = (_a.sent()).authSetting;
                                    console.log(authSetting);
                                    return [4 /*yield*/, (0, utils_1.asyncFunc)(tt.showModal, {
                                            title: "\u7533\u8BF7\u4F7F\u7528\u60A8\u7684".concat([authSetting['scope.album'] ? '' : '相册', authSetting['scope.camera'] ? '' : '摄像头'].filter(function (value) { return !!value; }).join('、')),
                                            cancelText: '拒绝',
                                            confirmText: '允许'
                                        })];
                                case 2:
                                    res = _a.sent();
                                    if (!res.cancel) return [3 /*break*/, 3];
                                    (callback === null || callback === void 0 ? void 0 : callback.complete) && callback.complete({ code: -1, msg: '您已拒绝授权' });
                                    return [3 /*break*/, 5];
                                case 3:
                                    if (!res.confirm) return [3 /*break*/, 5];
                                    return [4 /*yield*/, (0, utils_1.asyncFunc)(tt.openSetting)];
                                case 4:
                                    openSetting = _a.sent();
                                    console.log('openSetting', openSetting);
                                    if (openSetting.authSetting['scope.album'] === false) {
                                        (callback === null || callback === void 0 ? void 0 : callback.complete) && callback.complete({ code: -1, msg: '未授权相册' });
                                        return [2 /*return*/, false];
                                    }
                                    if (openSetting.authSetting['scope.camera'] === false) {
                                        (callback === null || callback === void 0 ? void 0 : callback.complete) && callback.complete({ code: -1, msg: '未授权摄像头' });
                                        return [2 /*return*/, false];
                                    }
                                    this.chooseVideo(callback);
                                    _a.label = 5;
                                case 5: return [2 /*return*/];
                            }
                        });
                    }); }
                });
                return [2 /*return*/];
            });
        });
    };
    // 跳转个人抖音号主页
    SdkDouyin.prototype.openAwemeUserProfile = function (callback) {
        tt.openAwemeUserProfile({
            success: function (res) {
                callback && callback.complete({
                    code: 0,
                    msg: res.errMsg,
                    hasFollowed: res.hasFollowed
                });
            },
            fail: function (err) {
                var _a, _b, _c;
                console.log(err);
                var msg = '';
                if ((_a = err.errMsg) === null || _a === void 0 ? void 0 : _a.includes('fail aweme uid is null')) {
                    msg = '未绑定抖音号';
                }
                else if ((_b = err.errMsg) === null || _b === void 0 ? void 0 : _b.includes('fail feature is not supported in app')) {
                    msg = '宿主不支持';
                }
                else if ((_c = err.errMsg) === null || _c === void 0 ? void 0 : _c.includes('fail not login')) {
                    msg = '玩家没有登录';
                }
                callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', __assign(__assign({}, err), { thirdmsg: err.errMsg, errMsg: msg || err.errMsg })));
            }
        });
    };
    // 查看抖音号是否关注
    SdkDouyin.prototype.checkFollowAwemeState = function (callback) {
        tt.checkFollowAwemeState({
            success: function (res) {
                callback && callback.complete({
                    code: 0,
                    msg: res.errMsg,
                    hasFollowed: res.hasFollowed
                });
            },
            fail: function (err) {
                var _a, _b, _c;
                var msg = '';
                if ((_a = err.errMsg) === null || _a === void 0 ? void 0 : _a.includes('fail aweme uid is null')) {
                    msg = '未绑定抖音号';
                }
                else if ((_b = err.errMsg) === null || _b === void 0 ? void 0 : _b.includes('fail feature is not supported in app')) {
                    msg = '宿主不支持';
                }
                else if ((_c = err.errMsg) === null || _c === void 0 ? void 0 : _c.includes('fail not login')) {
                    msg = '玩家没有登录';
                }
                callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', __assign(__assign({}, err), { thirdmsg: err.errMsg, errMsg: msg || err.errMsg })));
            }
        });
    };
    // 设置写入用户的排行榜数据
    SdkDouyin.prototype.setImRankData = function (params, callback) {
        tt.setImRankData(__assign(__assign({}, params), { success: function () {
                callback && callback.complete({
                    code: 0
                });
            }, fail: function (err) {
                callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err));
            } }));
    };
    // 获取排序好的「好友/总榜」数据源
    SdkDouyin.prototype.getImRankData = function (params, callback) {
        tt.getImRankData(__assign(__assign({}, params), { success: function (res) {
                callback && callback.complete({
                    code: 0,
                    data: res
                });
            }, fail: function (err) {
                callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err));
            } }));
    };
    // 获取排行榜列表
    SdkDouyin.prototype.getImRankList = function (params, callback) {
        tt.getImRankList(__assign(__assign({}, params), { success: function (res) {
                callback && callback.complete({
                    code: 0,
                    data: res
                });
            }, fail: function (err) {
                callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err));
            } }));
    };
    // 创建游戏推荐组件实例
    SdkDouyin.prototype.createGridGamePanel = function (params, callback) {
        try {
            var GridGamePanel = tt.createGridGamePanel(params);
            callback && callback.complete({
                code: 0,
                data: GridGamePanel
            });
        }
        catch (err) {
            callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err));
        }
    };
    // 调起客户端订阅消息界面
    SdkDouyin.prototype.requestSubscribeMessage = function (params, callback) {
        console.log('tmplIds', params.tmplIds);
        tt.requestSubscribeMessage({
            tmplIds: params.tmplIds,
            success: function (res) {
                var errMsg = res.errMsg, template_map = __rest(res, ["errMsg"]);
                callback && callback.complete({
                    code: 0,
                    msg: errMsg,
                    data: template_map
                });
            },
            fail: function (err) {
                var _a;
                callback && callback.complete({
                    msg: ((_a = err.errMsg) === null || _a === void 0 ? void 0 : _a.includes('template not exist or invalid')) ? '模版消息不存在或不合法' : err.errMsg,
                    code: err.errCode === 3001 ? 30010 : (err.errCode || err.errNo),
                    thirdcode: (err.errCode || err.errNo),
                    thirdmsg: err.errMsg
                });
            }
        });
    };
    // 查询指定公会是否绑定公开群
    SdkDouyin.prototype.getUnionGroupInfo = function (params, callback) {
        console.log('unionId', params.unionId);
        tt.getUnionGroupInfo({
            unionId: params.unionId,
            success: function (res) {
                callback && callback.complete({
                    code: 0,
                    bindStatus: res.bindStatus,
                    msg: res.errMsg
                });
            },
            fail: function (err) {
                callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err, 8000));
            }
        });
    };
    // 给指定公会绑定公开群
    SdkDouyin.prototype.bindUnionGroup = function (params, callback) {
        var _this = this;
        console.log('unionId', params.unionId);
        tt.bindUnionGroup({
            unionId: params.unionId,
            success: function () {
                _this.getUnionGroupInfo({
                    unionId: params.unionId
                }, {
                    complete: function (res) { return __awaiter(_this, void 0, void 0, function () {
                        var err_14;
                        return __generator(this, function (_a) {
                            switch (_a.label) {
                                case 0:
                                    callback && callback.complete(res);
                                    if (!res.bindStatus) return [3 /*break*/, 4];
                                    _a.label = 1;
                                case 1:
                                    _a.trys.push([1, 3, , 4]);
                                    return [4 /*yield*/, (0, apis_1.bindGroupApi)({
                                            rx_openid: config_1.USER_INFO.openid,
                                            unionid: params.unionId,
                                            is_leader: 1
                                        })];
                                case 2:
                                    _a.sent();
                                    return [3 /*break*/, 4];
                                case 3:
                                    err_14 = _a.sent();
                                    console.log('err', err_14);
                                    return [3 /*break*/, 4];
                                case 4: return [2 /*return*/];
                            }
                        });
                    }); }
                });
            },
            fail: function (err) {
                callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err, 8000));
            }
        });
    };
    // 解除指定公会和公开群的绑定关系
    SdkDouyin.prototype.unbindUnionGroup = function (params, callback) {
        var _this = this;
        console.log('unionId', params.unionId);
        tt.unbindUnionGroup({
            unionId: params.unionId,
            success: function () {
                _this.getUnionGroupInfo({
                    unionId: params.unionId
                }, {
                    complete: function (res) { return __awaiter(_this, void 0, void 0, function () {
                        var err_15;
                        return __generator(this, function (_a) {
                            switch (_a.label) {
                                case 0:
                                    callback && callback.complete(res);
                                    if (!!res.bindStatus) return [3 /*break*/, 4];
                                    _a.label = 1;
                                case 1:
                                    _a.trys.push([1, 3, , 4]);
                                    return [4 /*yield*/, (0, apis_1.unBindGroupApi)({
                                            rx_openid: config_1.USER_INFO.openid,
                                            unionid: params.unionId,
                                            is_leader: 1
                                        })];
                                case 2:
                                    _a.sent();
                                    return [3 /*break*/, 4];
                                case 3:
                                    err_15 = _a.sent();
                                    console.log('err', err_15);
                                    return [3 /*break*/, 4];
                                case 4: return [2 /*return*/];
                            }
                        });
                    }); }
                });
            },
            fail: function (err) {
                callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err, 8000));
            }
        });
    };
    // 加入指定公会绑定的公开群
    SdkDouyin.prototype.joinUnionGroup = function (params, callback) {
        console.log('unionId', params.unionId);
        tt.joinUnionGroup({
            unionId: params.unionId,
            success: function (res) {
                callback && callback.complete({
                    code: 0,
                    msg: res.errMsg
                });
                (0, apis_1.bindGroupApi)({
                    rx_openid: config_1.USER_INFO.openid,
                    unionid: params.unionId,
                    is_leader: 0
                });
            },
            fail: function (err) {
                callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err, 8000));
            }
        });
    };
    SdkDouyin.prototype.payGift = function (params, callback) {
        var _a, _b, _c, _d, _e;
        return __awaiter(this, void 0, void 0, function () {
            var goodsTagResult, result, payResult, pay_data, pay_data, _f, gift_id, gift_code, status_1, err_16;
            return __generator(this, function (_g) {
                switch (_g.label) {
                    case 0:
                        _g.trys.push([0, 8, , 9]);
                        //检查用户的登录态是否有效
                        return [4 /*yield*/, (0, utils_1.asyncFunc)(tt.checkSession)];
                    case 1:
                        //检查用户的登录态是否有效
                        _g.sent();
                        return [4 /*yield*/, (0, apis_1.getThirdGoodsTagApi)({
                                pay_type: 'douyinh5',
                                third_goods_tag: params.goods_tag
                            })];
                    case 2:
                        goodsTagResult = _g.sent();
                        return [4 /*yield*/, (0, apis_1.orderApi)(__assign(__assign(__assign({}, params), { goods_tag: (_a = goodsTagResult.data) === null || _a === void 0 ? void 0 : _a.goods_tag }), (!(0, is_1.isEmpty)(this.deviceInfo) ? { device_info: this.deviceInfo } : {})))];
                    case 3:
                        result = _g.sent();
                        payResult = {};
                        if (!(params.platform == 'ios')) return [3 /*break*/, 5];
                        pay_data = {
                            orderAmount: params.orderAmount,
                            goodName: params.goods_name,
                            zoneId: params.zoneId || '1',
                            customId: (_b = result.data) === null || _b === void 0 ? void 0 : _b.order_no,
                            extraInfo: (_c = result.data) === null || _c === void 0 ? void 0 : _c.transmit_args,
                            goodsId: params.goods_tag,
                            goodType: 3
                        };
                        console.log("ios\u652F\u4ED8\uFF1A", pay_data);
                        return [4 /*yield*/, (0, utils_1.asyncFunc)(tt.openAwemeCustomerService, pay_data)];
                    case 4:
                        payResult = _g.sent();
                        return [3 /*break*/, 7];
                    case 5:
                        pay_data = {
                            mode: 'game',
                            env: params.env || 0,
                            currencyType: 'CNY',
                            platform: 'android',
                            goodType: 3,
                            orderAmount: params.orderAmount,
                            goodsId: params.goods_tag,
                            goodName: params.goods_name,
                            zoneId: params.zoneId || '1',
                            customId: (_d = result.data) === null || _d === void 0 ? void 0 : _d.order_no,
                            extraInfo: (_e = result.data) === null || _e === void 0 ? void 0 : _e.transmit_args
                        };
                        console.log('安卓支付：', pay_data);
                        return [4 /*yield*/, (0, utils_1.asyncFunc)(tt.requestGamePayment, pay_data)];
                    case 6:
                        payResult = _g.sent();
                        _g.label = 7;
                    case 7:
                        _f = payResult.gift, gift_id = _f.gift_id, gift_code = _f.gift_code, status_1 = _f.status;
                        if (status_1 === 'cancel') {
                            callback.complete((0, utils_2.handleTrackError)(PLATFORM, 'rxlog_error_pay', {
                                code: const_1.COMMON_ERROR_CODE.CANCEL_PAY,
                                msg: '已取消',
                                thirdcode: const_1.COMMON_ERROR_CODE.CANCEL_PAY,
                                thirdmsg: '已取消'
                            }));
                        }
                        else if (status_1 === 'finish') {
                            callback.complete((0, utils_2.handleTrackError)(PLATFORM, 'rxlog_error_pay', {
                                code: const_1.COMMON_ERROR_CODE.PAY_GIFT_FINISH,
                                msg: '已核销',
                                thirdcode: const_1.COMMON_ERROR_CODE.PAY_GIFT_FINISH,
                                thirdmsg: '已核销'
                            }));
                        }
                        else {
                            callback.complete({ code: 0 });
                        }
                        return [3 /*break*/, 9];
                    case 8:
                        err_16 = _g.sent();
                        if (err_16.errCode == -2 || err_16.errNo == -2) {
                            err_16.code = const_1.COMMON_ERROR_CODE.CANCEL_PAY;
                            err_16.thirdcode = -2;
                        }
                        if (err_16.errCode == -1 || err_16.errNo == -1) {
                            err_16.code = const_1.COMMON_ERROR_CODE.PAY_ERROR;
                            err_16.thirdcode = -1;
                        }
                        callback.complete((0, utils_2.handleTrackError)(PLATFORM, 'rxlog_error_pay', err_16));
                        return [3 /*break*/, 9];
                    case 9: return [2 /*return*/];
                }
            });
        });
    };
    // 监听到用户打开推送的付费礼包并点击购买时触发事件回调
    SdkDouyin.prototype.onBuyGift = function (params, callback) {
        return __awaiter(this, void 0, void 0, function () {
            var err_17;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        if (!tt.canIUse('onBuyGift')) {
                            callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, 'rxlog_error_pay', {
                                code: -1,
                                thirdcode: -1,
                                msg: '不支持调用 tt.onBuyGift',
                                thirdmsg: '不支持调用 tt.onBuyGift'
                            }));
                            return [2 /*return*/];
                        }
                        //检查用户的登录态是否有效
                        return [4 /*yield*/, (0, utils_1.asyncFunc)(tt.checkSession)];
                    case 1:
                        //检查用户的登录态是否有效
                        _a.sent();
                        tt.onBuyGift(function (param) {
                            console.log('onBuyGift param: ', param);
                            _this.payGift(__assign(__assign({}, params), param), {
                                complete: function (res) {
                                    callback && callback.complete(res);
                                }
                            });
                        });
                        return [3 /*break*/, 3];
                    case 2:
                        err_17 = _a.sent();
                        callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, 'rxlog_error_pay', err_17));
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    // 查询用户直玩订阅的授权情况
    SdkDouyin.prototype.checkFeedSubscribeStatus = function (params, callback) {
        try {
            if (!params.allScene && ![1, 2, 3].includes(params.scene || 0)) {
                throw Error("\u8BA2\u9605\u7684\u573A\u666F ID\uFF08\u975E\u5168\u573A\u666F\u4E0B\u5FC5\u4F20\uFF09, \u4E14\u4E3A\u6570\u5B57 1\u30012\u30013");
            }
            var data = {
                type: params.type || 'play'
            };
            if (params.allScene) {
                data.allScene = true;
            }
            else {
                data.scene = params.scene;
            }
            tt.checkFeedSubscribeStatus(__assign(__assign({}, data), { success: function (res) {
                    callback && callback.complete({
                        code: 0,
                        status: res.status,
                        msg: res.errMsg
                    });
                }, fail: function (err) {
                    callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err));
                } }));
        }
        catch (err) {
            callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err));
        }
    };
    // 向用户请求授权，允许游戏在满足一定的条件后出现在 Feed 流中
    SdkDouyin.prototype.requestFeedSubscribe = function (params, callback) {
        try {
            if (!params.allScene && ![1, 2, 3].includes(params.scene || 0)) {
                throw Error("\u8BA2\u9605\u7684\u573A\u666F ID\uFF08\u975E\u5168\u573A\u666F\u4E0B\u5FC5\u4F20\uFF09, \u4E14\u4E3A\u6570\u5B57 1\u30012\u30013");
            }
            if (!params.allScene && (!params.contentIDs || params.contentIDs.length == 0)) {
                throw Error("\u81EA\u5B9A\u4E49\u6587\u6848 contentID \u6570\u7EC4\uFF08\u975E\u5168\u573A\u666F\u4E0B\u5FC5\u4F20\uFF09, contentID \u5728\u540E\u53F0\u7533\u8BF7\u5F00\u901A\u76F4\u73A9\u80FD\u529B\u540E\u53EF\u83B7\u53D6");
            }
            var data = {
                type: params.type || 'play'
            };
            if (params.allScene) {
                data.allScene = true;
            }
            else {
                data.scene = params.scene;
                data.contentIDs = params.contentIDs;
            }
            tt.requestFeedSubscribe(__assign(__assign({}, data), { success: function (res) {
                    callback && callback.complete({
                        code: 0,
                        success: res.success,
                        msg: res.errMsg
                    });
                }, fail: function (err) {
                    callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err));
                } }));
        }
        catch (err) {
            callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err));
        }
    };
    // 自定义启动场景数据上报
    SdkDouyin.prototype.reportScene = function (params, callback) {
        try {
            tt.reportScene(__assign(__assign({}, params), { success: function (res) {
                    callback && callback.complete({
                        code: 0,
                        data: res.data,
                        msg: res.errMsg
                    });
                }, fail: function (err) {
                    callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err));
                } }));
        }
        catch (err) {
            callback && callback.complete((0, utils_2.handleTrackError)(PLATFORM, '', err));
        }
    };
    return SdkDouyin;
}(SdkCommon_1.default));
exports.default = SdkDouyin;
//# sourceMappingURL=index.douyin.js.map