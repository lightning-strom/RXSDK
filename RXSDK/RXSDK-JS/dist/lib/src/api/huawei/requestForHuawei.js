...
var utils_1 = require("@/utils/huawei/utils");
...
var getHeaders = function (path) {
    ...
    var getDevicecode = function () {
        try {
            var devicecode = utils_1.storage.get('rx_devicecode');
            if (devicecode) {
                // @ts-ignore
                ...
                utils_1.storage.set('rx_devicecode', { code: code, openIds: {} });
                return code;
            }
        }
        catch (err) {
    ...
    var headers = (_a = {},
        ...
        _a);
    var rxToken = utils_1.storage.get('rxToken');
    ...
    if ((0, utils_2.checkNeedAesEncrypt)(path)) {
...
var requestAxios = axios_1.default.create({
    timeout: 60000,
    responseType: 'json',
    withCredentials: true
});
var retryRequest = function (options, resolve, reject) {
    var headers = (0, utils_2.removeKeyFromObject)(options.headers);
    (0, utils_3.printLog)("".concat(options.url));
    (0, utils_3.printLog)("options", JSON.stringify(options));
    requestAxios({
        url: options.url,
        ...
        (0, utils_3.printLog)("".concat(options.url));
        (0, utils_3.printLog)("res", JSON.stringify(res.data));
        resolve(res.data);
    }).catch(function (err) {
        (0, utils_3.printLog)("".concat(options.url));
        (0, utils_3.printLog)("err", JSON.stringify(err));
        reject(err);
    });
};
var myRequest = function (options) {
    var devicecode = (0, utils_2.getDevicecode)();
    var key = (0, utils_2.generateMD5)(devicecode + utils_2.cpkey);
    (0, utils_3.printLog)("".concat(options.url));
    (0, utils_3.printLog)("options", JSON.stringify(options));
    return new Promise(function (resolve, reject) {
        var data = options.data;
        var isAes = (0, utils_2.checkNeedAesEncrypt)(options.url);
        try {
            data = (isAes && options.method.toLowerCase() != 'get') ? (0, utils_2.aesEncryptBase64String)(options.data, key) : options.data;
            if (isAes && options.method.toLowerCase() != 'get') {
                (0, utils_3.printLog)('Encrypt Data:', data);
                (0, utils_3.printLog)('Self Encrypt Data:', (0, utils_2.aesDecryptBase64String)(data, key));
            }
        }
        catch (e) {
            // @ts-ignore
            (0, utils_2.trackEncrypt)(options, 'huawei', key);
        ...
        requestAxios({
            ...
            if (res.status == 500) {
                ...
                (0, utils_3.printLog)('request 解密失败', options.url, (_b = res.data) === null || _b === void 0 ? void 0 : _b.code);
                // @ts-ignore
                (0, utils_2.trackDecrypt)(options, res, 'huawei', key);
                retryRequest(options, resolve, reject);
            }
            else {
                ...
                if (isAes && data_1) {
                    try {
                        if (((_d = res.data) === null || _d === void 0 ? void 0 : _d.code) === 0) {
                            data_1 = (0, utils_2.aesDecryptBase64String)(data_1, key);
                            (0, utils_3.printLog)('Decrypt Data:', data_1);
                            var result = __assign(__assign({}, res.data), { data: (0, utils_2.isJsonString)(data_1) ? JSON.parse(data_1) : data_1 });
                            (0, utils_3.printLog)("".concat(options.url));
                            (0, utils_3.printLog)("res", JSON.stringify(result));
                            resolve(result);
                        }
                        else {
                            resolve(res.data);
                        ...
                        (0, utils_3.printLog)('response 解密失败', options.url, e);
                        // @ts-ignore
                        (0, utils_2.trackDecrypt)(options, res, 'huawei', key);
                        retryRequest(options, resolve, reject);
                    }
                    ...
                    if (refreshCode.includes(res.code)) {
                        if (refreshNum === 5) {
                            ...
                            return [2 /*return*/, refreshTokenReq().then(function (refreshRes) {
                                    utils_1.storage.set('rxToken', refreshRes.data);
...
//# sourceMappingURL=requestForHuawei.js.map