...
import {
  ...
  baiduLoginParamsCheck,
  baiduPayCheckParams, baiduUserInfoCheckParams
...
const PLATFORM = 'minigame_baidu'

declare global {
  var swan: any
}

class SdkBaidu extends SdkCommon {
  ...
  private async getUserInfo(params: RpkBaiduCustomType, callback: RpkMethodParams) {
    await pubCheck(baiduUserInfoCheckParams, callback, params)
    const button = swan.createUserInfoButton(params)
    try {
  ...
  // 同步用户信息
  async infoSync({ headimgurl, nickname, sex }: {
    headimgurl: string,
    nickname: string,
    sex: number
  }, callback: RpkMethodParams) {
    try {
      const loginResult: any = await this.swanLogin()
  ...
  public async login(params: RpkBaiduLoginParam, callback: RpkMethodParams) {
    try {
      await pubCheck(baiduLoginParamsCheck, callback, params)
  ...
  public async pay(params: RpkBaiduPayParam, callback: RpkMethodParams) {
    ...
    try {
      await pubCheck(baiduPayCheckParams, callback, params)
      ...
      await asyncFunc(swan.requestPolymerPayment, {
        orderInfo: {
          dealId: result.data?.ext?.dealId,
          appKey: result.data?.ext?.appKey,
          totalAmount: result.data?.ext?.totalAmount,
          tpOrderId: result.data?.ext?.tpOrderId,
          dealTitle: result.data?.ext?.dealTitle,
          signFieldsRange: result.data?.ext?.signFieldsRange,
          rsaSign: result.data?.ext?.rsaSign
        }
      })
  ...
  public async schedulingAction(params: any, callback: RpkMethodParams) {
    try {
      ...
      if (scheduling_type === 'ad') {
        ...
        const appSid = params.adUnitId || shareData?.data?.ad_content?.ext?.app_sid
  ...
  // 激励广告
  public async rewardedVideoAd(data: any, { complete }: IMethodParams) {
    let adShareData: any = {}
    ...
    const appSid = data.appSid || adShareData?.data?.ad_content?.ext?.app_sid
  ...
  async authorize(scope: string, callback: RpkMethodParams) {
    ...
    try {
      const { authSetting } = await asyncFunc(swan.getSetting)
      ...
      if (authSetting[scope] === true) {
        ...
        await asyncFunc(swan.authorize, { scope })
        callback?.complete && callback.complete({ code: 0 })
        return true
      } else if (authSetting[scope] != undefined && authSetting[scope] != true) {
        // 拒绝过授权 重新去授权
        const res = await asyncFunc(swan.showModal, {
        ...
        if (res.cancel) {
          ...
          const openSetting = await asyncFunc(swan.openSetting)
