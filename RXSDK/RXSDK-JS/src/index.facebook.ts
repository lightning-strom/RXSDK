...
class SdkFacebook extends SdkCommon {
  ...
  private async login(params: H5FacebookLoginParam, callback: H5MethodParams) {
    try {
      let queryData = this.getLoginQsAndGenerateStruct()
      // 获取facebook 分享参数
      let entryPointData = FBInstant.getEntryPointData()
      console.error(entryPointData, 'entryPointData', queryData)
      console.error(qs.parse(entryPointData?.queryParams), 'qs.parse(entryPointData?.queryParams)')
      // 如果facebook 分享参数存在，则合并到user_source
      if(entryPointData?.queryParams){
        queryData = {
          user_source:{
            ...queryData,
            share:{
              ...qs.parse(entryPointData?.queryParams)
            }
          }
        }
      }
      ...
      const requestParams: any = {
        ts: now,
        method: params.method || 'minigame_facebook',
        distinct_id,
        ext: {
          ...params.ext,
          id: this.playerId // 用户唯一标识
        },
        ...queryData
      }
      ...
      try {
        if (this.subChannelId !== null) {
          ...
          requestParams.user_source = {
            guide: { ...queryData, subchannelid: this.subChannelId }
          }
          if (queryJson) {
  ...
  public async share(params: any, callback: RpkMethodParams) {
    let query = null // 组装参数
    let shareData = null // 分享数据
    let isFunc = params?.func // 是否有埋点
    let shareDataDefault = { // 分享数据默认
      type: 'rx',
      user_source: 'share',
    ...
    // 如果func存在，则使用shareData中的query+transmits，否则判断是否有调度埋点，有则使用调度埋点+transmits，没有则使用transmits
    const queryParams = isFunc ? `${query}&${params?.transmits || ''}` : this.scheuleReportProps ? `${query}&${params?.transmits || ''}` : `${params?.transmits || ''}`
    ...
    try {
      FBInstant.shareAsync({
        ...
        text: isFunc ? shareData?.data?.content?.content : params.title,
        data: { queryParams },
      }).then(() => {
        //
        // 分享成功 && 一键分享或变量中存在埋点 && 自动上报 && 上报类型存在
        if((isFunc || this.getShareDataFunc) && params.autoReport) {
          this.shareSchedulingReport({
            func: params.func || this.getShareDataFunc, // 一键分享或变量中存在埋点
  ...
  public async advertisingVideo(params: any, callback: RpkMethodParams) {
    try {
      ...
      if(FBInstant?.getSystemInfoSync()?.SDKVersion && this.versionCheck(FBInstant?.getSystemInfoSync()?.SDKVersion, '12.37.200') && FBInstant?.createCustomAd) {
        console.warn('当前版本不支持激励广告')
        this.supportedAdvertisingVideo(callback)
        return
      } else {
        // 创建美团广告
        const customAd = FBInstant.createCustomAd({
          posId: params?.posId || '10395' // 填入美团提供的 posId
        })
  ...
  public async addShortcut(params: any, callback: IMethodParams) {
    try {
      const result = await asyncFunc(FBInstant.addShortcut, {
