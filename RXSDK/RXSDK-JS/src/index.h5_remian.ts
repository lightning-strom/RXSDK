...
class SdkH5Remian extends SDKUI {
  // 默认刷新时间 10 分钟
  ...
  public override initConfig: any = {}
  // 调度埋点
  ...
  // 登录必要参数
  private loginMandatory = ['user_id', 'token', 'app_id']

  // 初始化url参数
  private initUrlParams: Record<string, any> = {}
  ...
  private async login(params: H5RemianLoginParam, callback: H5MethodParams) {
    try {
      this.initUrlParams = findChannelData(this.loginMandatory)
      ...
      const requestParams: any = {
        ts: now,
        method: params.method || 'remianh5',
        distinct_id,
        ext: {
          ...params.ext,
          env: params.ext?.env || 0, // 1 测试 0 正式
        },
        ...user_source
      }
      ...
      if (params.login_openid) {
        ...
        if (this.initUrlParams?.regionId) {
          reqLogin.ext.region_id = this.initUrlParams?.regionId
        }
  ...
  // 支付
  public async pay(params: H5RemianPayParam, callback: H5MethodParams) {
    try {
      await pubCheck(H5RemianPayCheckParams, callback, params)
      ...
      let reqOrder = {
        ...
        env: params.env || 0, // 1：测试 0：生产
        openid: USER_INFO.openid,
        ext:{
          app_id: this.initUrlParams?.app_id
        }
      }
      // 如果regionId存在，则设置region_id
      if (this.initUrlParams?.regionId) {
        (reqOrder.ext as any).region_id = this.initUrlParams?.regionId
      }
      const result = await orderApi(reqOrder)
      // 打开支付
      try {
        // 参数组装
        let appgame1Params = `${result?.data?.ext?.url}/toPay.html?uid=${result?.data?.ext?.uid}&amount=${result?.data?.ext?.amount}&out_trade_no=${result?.data?.ext?.out_trade_no}&notifyUrl=${result?.data?.ext?.notify_url}&appid=${result?.data?.ext?.appid}&shopid=${result?.data?.ext?.shopid}&callbackInfo=${result?.data?.ext?.callback_info}&opTime=${result?.data?.ext?.op_time}&sign=${result?.data?.ext?.sign}&app_id=${result?.data?.ext?.app_id}`
        if (result?.data?.ext?.region_id) {
          appgame1Params += `&regionId=${result?.data?.ext?.region_id}`
        }
        console.log('appgame1Params', `'appgame1://openWebGame?url=' + ${appgame1Params}`)
