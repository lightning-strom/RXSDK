...
import {
  ...
  alipayLoginParamsCheck,
  alipayPayCheckParams,
...
const PLATFORM = 'minigame_alipay'
...
class SdkAlipay extends SdkCommon {
  ...
  async infoSync(callback: RpkMethodParams) {
    try {
      const loginResult: any = await asyncFunc(my.getAuthCode, { scopes: 'auth_user' })
      const loginUserInfoResult: any = await asyncFunc(my.getAuthUserInfo, { scopes: 'auth_user' })
  ...
  //alipay登录
  public async login(params: RpkAlipayLogin, callback: RpkMethodParams) {
    try {
      await pubCheck(alipayLoginParamsCheck, callback, params)
      ...
      try {
        const getLaunchParams = my.getLaunchOptionsSync()
        reqLogin.open_source = getLaunchParams?.scene ? getLaunchParams?.scene + '' : undefined
      } catch (err: any) {
      ...
      const loginResult: any = await asyncFunc(my.getAuthCode, { scopes: 'auth_user' })
      let loginUserInfoResult: any = {}
      try {
        loginUserInfoResult = await asyncFunc(my.getAuthUserInfo, { scopes: 'auth_user' })
      } catch (e: any) {
  ...
  //cp方主动补单
  public async compensatePayOrder(params: any, callback: RpkMethodParams) {
    try {
      await pubCheck(compensateOrderCheckParams, callback, params)
      let { notify_url } = params
      await postAlipayPayData(notify_url, params)
      ...
      if (expiredVoucherCode.includes(err?.code)) {
        ...
        callback.complete({
          code: 0,
          originErr: handleTrackError(PLATFORM, 'rxlog_error_pay', err)
        })
        return
      }
  ...
  // alipay支付
  public async pay(params: RpkAlipayPayParam, callback: RpkMethodParams) {
    ...
    let result: any = {}
    try {
      await pubCheck(alipayPayCheckParams, callback, params)
      ...
      result = await orderApi(reqOrder)
      ...
      if (result.data?.ext?.callback == 1) {
        await postAlipayPayData(result.data.notify_url, {
      ...
      // iOS虚拟币支付
      if(params.pay_type === 'minigame_alipay_virtual') {
         //判断接口是否可用
        if(my.requestGamePayCoinPayment) {
          my.requestGamePayCoinPayment({
            buyQuantity: result.data?.ext?.buyQuantity,
            zoneId: params?.zoneId || "1",
            goodsName: result.data?.ext?.goodsName,
            customId: result.data?.ext?.customId,
            ...
            fail: (err: any) => {
              ...
              callback.complete(handleTrackError(PLATFORM, 'rxlog_error_pay', {
                code: err?.error === '16001' ? '4001' : '4002',
                msg: err?.error === '16001' ? '支付取消' : '三方支付错误',
                thirdCode: err?.error,
                thirdMsg: err?.errorMessage || ''
              }))
            },
            complete:() => {
        ...
        // android 和 harmony支付
        const payResult = await asyncFunc(my.requestGamePayment, {
          customId: result.data?.ext?.customId,
          buyQuantity: result.data?.ext?.buyQuantity,
          extraInfo: qs.parse(result.data.transmit_args || '')
        })
      ...
      if (isDropOrder(err?.code)) {
        ...
        customSetStorageSync(`rx_${USER_INFO.tid}`, payParams)
        err.data = err.data || {}
        err.data = {
          ...err.data,
          payParams
  ...
  //分享
  public async share(params: RpkAlipayShareParams, callback: IMethodParams) {
    ...
    try {
      my.onShareAppMessage = function() {
        return {
          ...
          desc: params.desc || shareData?.data?.content?.desc,
          content: params.content || shareData?.data?.content?.content,
          bgImgUrl: params.bgImgUrl || shareData?.data?.content?.bg_img_url,
          scImgUrl: params.scImgUrl || shareData?.data?.content?.sc_img_url,
          imageUrl: params.imageUrl || shareData?.data?.content?.image_url,
          searchTip: params.searchTip || shareData?.data?.content?.search_tip,
          query
        }
      }
      my.showSharePanel({
  ...
  // 激励广告
  public async rewardedVideoAd(data: RpkRewardedAdParams, {
    complete,
    fail: failCallback
  }: RpkMethodParams) {
    let adShareData: any = {}
  ...
  // 调起客户端订阅消息界面
  private requestSubscribeMessage(params: { entityIds: string[] }, callback: IMethodParams) {
