...
class SdkH5Simo extends SdkCommon {
  // 默认刷新时间 10 分钟
  ...
  constructor(initParams: InitH5Params) {
    ...
    if (window.LING_H5_GAME_SDK) {
      ...
      this.loadScript('https://res.007sy.cn/js/gamesdk.js')
        .then(() => {
          this.getInitConfig({ complete: initParams.complete })
        })
        .catch((err: any) => {
  ...
  private async login(params: H5SimoLoginParam, callback: H5MethodParams) {
    try {
      ...
      const requestParams: any = {
        ts: now,
        method: params.method || 'minigame_007',
        distinct_id,
        ext: params.ext,
        ...user_source
      }
      ...
      if (params.login_openid) {
        ...
        reqLogin.ext = {
          ...(rest_ext || {}),
          uid: login_info.uid,
          auth: login_info.auth,
          tm: `${login_info.login_time}`
        }
  ...
  // 角色数据上报
  upRole(params: H5SimoUpRoleParam) {
    const login_info: any = getSearchQueries()
  ...
  // 支付
  public async pay(params: H5SimoPayParam, callback: H5MethodParams) {
    ...
    try {
      await pubCheck(H5SimoPayCheckParams, callback, params)
      ...
      LING_H5_GAME_SDK.pay({
        ...
        GAME_ID: result.data?.ext?.GAME_ID,
        product_id: result.data?.ext?.product_id,
        props_name: result.data?.ext?.props_name,
        cp_order_sn: result.data?.ext?.cp_order_sn,
