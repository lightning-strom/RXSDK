...
import {
  ...
  ShareCheckParams,
  taobaoLoginParamsCheck,
  taobaoPayCheckParams
...
const PLATFORM = 'minigame_taobao'
...
class SdkTaobao extends SdkCommon {
  private shareFlag: number = 0
  ...
  async infoSync(callback: RpkMethodParams) {
    try {
      await asyncFunc(my.authorize, { scopes: 'scope.userInfo' })
      let loginUserInfoResult: any = await asyncFunc(my.getAuthUserInfo, { scopes: 'scope.userInfo' })
  ...
  // taobao登录
  public async login(params: RpkTaobaoLogin, callback: RpkMethodParams) {
    try {
      await pubCheck(taobaoLoginParamsCheck, callback, params)
      ...
      if (params.login_openid) {
        ...
        try {
          loginUserInfoResult = await asyncFunc(my.getAuthUserInfo, { scopes: 'scope.userInfo' })
        } catch (e: any) {
  ...
  // taobao支付
  public async pay(params: RpkTaobaoPayParam, callback: RpkMethodParams) {
    ...
    try {
      await pubCheck(taobaoPayCheckParams, callback, params)
      ...
      const payResult = await asyncFunc(my.tb.virtualTrade, {
        itemId: result.data?.ext?.third_tag,
        outOrderId: result.data?.order_no
      })
  ...
  //分享
  public async share(callback: RpkMethodParams, params?: RpkTaobaoShareParams) {
    const onShow = async () => {
    ...
    my.onShow(onShow)
    const sdk = await my.tb.getInteractiveSDK()
    sdk.shareApp({
      title: params?.title || '',
      desc: params?.desc || '',
      thumbImgUrl: params?.imageUrl || '',
      querys: qs.parse(params?.query || '')
    })
      .catch((err: any) => {
