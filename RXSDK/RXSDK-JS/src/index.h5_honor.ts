...
const PLATFORM = 'minigame_honor'

declare global {
  var qg: any
}

class SdkH5Honor extends SdkCommon {
  ...
  async infoSync(callback: RpkMethodParams) {
    try {
      const loginResult: any = await asyncFunc(qg.login, { needAuthCode: true })
  ...
  private async login(params: H5HonorLoginParam, callback: H5MethodParams) {
    try {
      ...
      const loginResult: any = await asyncFunc(qg.login, { needAuthCode: true, isProfileRequired: true })
  ...
  // 支付
  public async pay(params: H5HonorPayParam, callback: H5MethodParams) {
    ...
    try {
      ...
      await asyncFunc(qg.pay, {
        productInfo: {
          ...
          cpId: result.data?.ext?.cpId,
          publicKey: result.data?.ext?.publicKey,
          bizOrderNo: result.data?.ext?.bizOrderNo,
          productId: result.data?.ext?.productId,
          orderAmount: (result.data?.price) * 100
        }
      })
      callback.complete({ code: 0 })
    } catch (err: any) {
      if (err.errCode == 80001) {
  ...
  public async rewardedVideoAd(data: any, { complete, fail }: H5MethodParams) {
    ...
    if (!this._rewardedVideoAd) {
      ...
      const onReward: any = async () => {
        this._rewardedVideoAd.offReward(onReward)
        isEnded = true
      }
      ...
      this._rewardedVideoAd.onReward(onReward)
    }
