...
class SdkMgtv extends SdkCommon {
  ...
  async infoSync(callback: RpkMethodParams) {
   ...
   // 获取系统信息
  getWindowInfo(callback: RpkMethodParams) {
    try {
      const windowInfoResult: any = mgtv.getWindowInfo()
  ...
  public async login(params: RpkMgtvLogin, callback: RpkMethodParams) {
    try {
      await pubCheck(mgtvLoginParamsCheck, callback, params)
      ...
      await asyncFunc(mgtv.login, {})
      try {
        userInfoResult = await asyncFunc(mgtv.getUserProfile, {})
      } catch (e: any) {
  ...
  public async pay(params: RpkBlPayParam, callback: RpkMethodParams) {
    try {
      await pubCheck(mgtvPayCheckParams, callback, params)
      ...
      console.info({
        signData: result.data?.ext?.signData,
        sign: result.data?.ext?.sign,
        timestamp: result.data?.ext?.timestamp
      })

      await asyncFunc(mgtv.requestPaymentGameItem, {
        signData: result.data?.ext?.signData,
        sign: result.data?.ext?.sign,
        timestamp: result.data?.ext?.timestamp
      })
      ...
      console.info(err, 'pay')
      if (err.errMsg.includes('取消')) {
        callback.complete(handleTrackError(PLATFORM, 'rxlog_error_pay', err, COMMON_ERROR_CODE.CANCEL_PAY))
        return
      }
  ...
  private validateTypeList(arr: any) {
    // 允许的值
    const allowedValues = ['moments', 'wechat', 'weibo', 'qq', 'qzone', 'fantuan']
  ...
  public async share(params: any, callback: RpkMethodParams) {
    try {
      ...
      let url = (compare === 0 || compare === 1) ? `https://app.mgtv.com/mgmp-share/?appid=${SYSTEM_INFO.appid}&host=mgtv&path=page/index?` : `https://app.mgtv.com/mgmp-share/?appid=${SYSTEM_INFO.appid}&host=mgtv&path=`

      console.info({
        title: params.title || shareData?.data?.content?.title,
        desc: params.desc || shareData?.data?.content?.content,
        img: params.img || shareData?.data?.content?.image,
        // url: url ? `${url}?${search}` : '',
        url: params.url || `${url}${encodeURIComponent(query)}`,
  ...
  public async rewardedVideoAd(data: {
    ...
    func?: string,
    custom_ext?: any
  }, { complete, fail, onShow }: RpkMethodParams) {
    let adShareData: any = {}
  ...
  async authorize(scope: string, callback: RpkMethodParams) {
    ...
    try {
      const { authSetting } = await asyncFunc(mgtv.getSetting)
      ...
      if (authSetting[scope] === true) {
        ...
        await asyncFunc(mgtv.authorize, { scope })
        callback?.complete && callback.complete({ code: 0 })
        return true
      } else if (authSetting[scope] != undefined && authSetting[scope] != true) {
        // 拒绝过授权 重新去授权
        const res = await asyncFunc(mgtv.showModal, {
        ...
        if (res.cancel) {
          ...
          const openSetting = await asyncFunc(mgtv.openSetting)
