...
const PLATFORM = 'minigame_xunlei'
...
class SdkH5Xunlei extends SdkCommon {
  // 默认刷新时间 10 分钟
  ...
  constructor(initParams: InitH5Params) {
    ...
    if (window.XL_GAME) {
      ...
      this.loadScript('https://static2-youxi.a.88cdn.com/backend/xl-lib/xl-game.min.2.2.0.js')
        .then(() => {
          this.getInitConfig({ complete: initParams.complete })
        })
        .catch((err: any) => {
  ...
  // 游戏横屏
  private changeRotate(type: 0 | 1) {
    // type为0是游戏画面切换为横屏，type为1是游戏画面切换为竖屏
    XL_GAME.callApp('changeRotate', { type })
  }

  // 新增区服同步
  private syncServer(params: H5XunleiSyncServerParam, callback: H5MethodParams) {

  }

  // 更新区服同步
  private updateServer(params: H5XunleiSyncServerParam, callback: H5MethodParams) {

  }

  // 消息同步
  private messageSync(params: H5XunleiMessageSyncParam, callback: H5MethodParams) {

  }

  // 游戏行为上报
  private actionReport(params: H5XunleiActionReportcParam) {
    XL_GAME.callApp(
      'actionReport',
      {
  ...
  private async login(params: H5XunleiLoginParam, callback: H5MethodParams) {
    try {
      ...
      const requestParams: any = {
        ts: now,
        method: params.method || 'minigame_xunlei',
        distinct_id,
        ext: params.ext,
        ...user_source
      }
      ...
      if (params.login_openid) {
        ...
        reqLogin.ext = {
          ...(rest_ext || {}),
          authKey: login_info.authKey,
          server_id: login_info.server_id,
          userId: login_info.userId,
          ext: login_info.ext,
          xlData: login_info.xlData,
  ...
  public async pay(params: H5XunleiPayParam, callback: H5MethodParams) {
    ...
    try {
      ...
      XL_GAME.callApp('superPay', result.data.ext,
        (res: any) => {
          console.log(res)
          if (res.errorCode == 0) {
  ...
  private rewardedVideoAd({ complete }: H5MethodParams) {
    XL_GAME.callApp('getRewardVideo', (res: any) => {
      if (res.errorCode === 0) {
        complete({
          code: 0,
          data: null,
