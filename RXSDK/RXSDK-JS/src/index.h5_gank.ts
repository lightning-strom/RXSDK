...
function findGankSDK(maxDepth = 5) {
...
// 获取url参数
function findChannelData(params: any, optional: string[] = []) {
  let currentWindow: any = window
...
// 获取必要参数的url参数
function getChannelDataFromUrl(url: string, channelMandatory: string[], optional: string[] = []) {
  if (!url) return null
  try {
...
class SdkH5Gank extends SdkCommonUI {
  private uid: any = ''
  ...
  // 渠道gameid
  private channelGameId: string | number = ''
  // 推广员福利码相关信息
  private promoInfo: {
    timer: any
  ...
  // 登录必要参数
  private loginMandatory = ['gameid', 'uid','sign','channel']

  constructor(initParams: InitGankH5Params) {
    super(PLATFORM)
    Object.assign(SYSTEM_INFO, SYSTEM_INFO_H5, { ...initParams })
    // 渠道gameid
    this.channelGameId = initParams.gameid || ''
    // 获取三方渠道数据
    SYSTEM_INFO.third_channel_Data = findChannelData(this.loginMandatory)
  ...
  // gank 上报创建游戏角色信息
  public async reportData(params: any, callback: H5MethodParams) {
    // 调用服务端接口进行数据上报
    try {
     const parasmData = {
       ...
       data:{
         ...params?.data || {},
         deviceid: SYSTEM_INFO?.third_channel_Data?.uid || '', // 链接上的uid 的值
         channelid: SYSTEM_INFO?.third_channel_Data?.channel || '', // 链接上的channel 的值
         subchid: SYSTEM_INFO?.third_channel_Data?.subchid || '', // 链接上的subchid 的值
         extInfo: JSON.stringify(SYSTEM_INFO?.third_channel_Data) || ''
       }
     }
     const result = await unicornRegisterReportApi(parasmData)
     // 如果是创角色上报，需要在创角上报后再次上报登录信息
     if(params?.action == 'register') {
       parasmData.action = 'login'
       const resultLogin = await unicornRegisterReportApi(parasmData)
       callback.complete({ code: resultLogin?.code, data: resultLogin?.data })
       return
     }
     callback.complete({ code: result?.code, data: result?.data })
    } catch (error) {
   ...
   // gank 获取三方渠道数据
  public async getThirdChannelData(callback: H5MethodParams) {
    try {
      callback.complete({ code:0, msg:'获取三方渠道数据成功', data: SYSTEM_INFO?.third_channel_Data || {} })
    } catch (error) {
      callback.complete(handleTrackError(PLATFORM, '', error))
    }
   }
  ...
  // gank 登录
  public async login(params: H5GankLoginParam, callback: H5MethodParams) {
    try {
      ...
      const requestParams: any = {
        ts: now,
        method: params.method || 'unicornh5',
        distinct_id,
        ext: SYSTEM_INFO.third_channel_Data, // 三方渠道url上的信息全部带入登录中
        ...user_source,
      }
  ...
  // gank 支付
  public async pay(params: H5GankPayParam, callback: H5MethodParams) {
    try {
      await pubCheck(H5GankPayCheckParams, callback, params)
      ...
      let reqOrder = {
        ...
        env: params.env || 0,
        userData:{
          ...SYSTEM_INFO?.third_channel_Data,
          openid: USER_INFO.openid,
        }
      }
      const result = await orderApi(reqOrder)
      getGankSDK().pay({
        wid: Number(result?.data?.ext?.third_tag),
        txid: result?.data?.ext?.order_id,
        userdata: JSON.stringify({
          openid: result?.data?.ext?.openid,
          order_id: result?.data?.ext?.order_id,
        }),
  ...
  private roleReport(params: any, callback: H5MethodParams) {
    try {
      getGankSDK().report({
          ...params || {},
          type: params.type || '',
          who: params.userRoleId || '',
          serverid: params.serverId || '',
          level: params.userRoleLevel || '',
          power: params.gameRolePower || '',
          rolename: params.userRoleName || '',
          role_vip: params.vipLevel || 1,
          servername: params.serverName || '',
          appid: this.channelGameId || '',
          deviceid: SYSTEM_INFO?.third_channel_Data?.uid || '', // 链接上的uid 的值
          channelid: SYSTEM_INFO?.third_channel_Data?.channel || '', // 链接上的channel 的值
      })
  ...
  private rewardedVideoAd({ complete }: H5MethodParams) {
    getGankSDK().callApp('getRewardVideo', (res: any) => {
      if (res.errorCode === 0) {
        complete({
          code: 0,
          data: null,
