...
const PLATFORM = 'minigame_lenovo'
...
class SdkH5Lenovo extends SdkCommon {
  // 默认刷新时间 10 分钟
  ...
  // 初始化必要参数
  private mandatoryParams = ['game', 'token', 'failUrl', 'lpms']
  // 初始化选填参数
  private optionalParams = ['serviceId']
  ...
  // 登录
  private async login(params: H5LenovoLoginParam, callback: H5MethodParams) {
    try {
      this.initUrlParams = findChannelCodeInHierarchy(this.mandatoryParams, this.optionalParams)
      ...
      const requestParams: any = {
        ts: now,
        method: params.method || 'minigame_lenovo',
        distinct_id,
        ext: params.ext,
        ...user_source
      }
  ...
  // 支付
  public async pay(params: H5LenovoPayParam, callback: H5MethodParams) {
    try {
      await pubCheck(H5LenovoPayCheckParams, callback, params)
      ...
      const urlParams = `?partner=${result?.data?.ext?.partner}&notifyUrl=${result?.data?.ext?.notifyUrl}&outTradeNo=${result?.data?.ext?.outTradeNo}&encLenovoId=${result?.data?.ext?.encLenovoId}&gameId=${result?.data?.ext?.gameId}&extraCommonParam=${params?.extraCommonParam}&server=${params?.server}&role=${params?.role}&lpms=${result?.data?.ext?.lpms}&totalFee=${result?.data?.ext?.totalFee}`
      const payUrl = 'https://cp.vgs.lenovo.com.cn/h5pay'
      lenovocp.pay({
        url: payUrl + urlParams, // 调用支付生成的最终地址
        type: 'h5', // 默认pc pc,h5
      }).then((res: any) => {
        //创单成功
        console.log(res)
