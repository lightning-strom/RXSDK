...
function findQuickSDK(maxDepth = 5) {
...
function getChannelCodeFromUrl(url: string) {
  if (!url) return null
  try {
...
class SdkH5Quick extends SdkCommonUI {
  private uid: any = ''
  ...
  constructor(initParams: InitH5Params) {
    ...
    SYSTEM_INFO.third_channel_code = findChannelCodeInHierarchy()
    this.initQuick(() => {
      this.getInitConfig({ complete: initParams.complete })
    })
  }

  private getQuickChannelCode() {
    return SYSTEM_INFO.third_channel_code
  }

  private initQuick(callback: any) {
    QuickSDK.init(SYSTEM_INFO.productCode, SYSTEM_INFO.productKey, false, callback)
  }

  // 游戏行为上报
  private roleReport(params: H5QuickRoleReportParam, callback?: H5MethodParams) {
    console.info('uploadGameRoleInfo', {
      ...params,
      uid: this.uid,
      username: this.username
    })

    QuickSDK.uploadGameRoleInfo(JSON.stringify({
      ...params,
      uid: this.uid,
      username: this.username
    }), (res: any) => {
  ...
  private quickLogin() {
    return new Promise((resolve, reject) => {
      QuickSDK.login((callbackData: any) => {
        if (callbackData.status) {
          this.uid = callbackData.data.uid
          this.username = callbackData.data.username
  ...
  private logout(callback?: H5MethodParams) {
    QuickSDK.logout(() => {
  ...
  private userVerify(params: any, callback: H5MethodParams) {
    QuickSDK.getExtendsVal(params, (data: any) => {
      if (data)
        callback.complete({
          code: 0,
          data
  ...
  private async login(params: H5QuickLoginParam, callback: H5MethodParams) {
    try {
      ...
      const requestParams: any = {
        ts: now,
        method: params.method || 'minigame_quick',
        distinct_id,
        ext: params.ext,
        ...user_source
      }
      ...
      if (params.login_openid) {
        ...
        const login_info: any = await this.quickLogin()
        ...
        reqLogin.ext = {
          ...
          parent_channel_type: login_info.channelId
        }
  ...
  public async pay(params: H5QuickPayParam, callback: H5MethodParams) {
    try {
      await pubCheck(H5QuickPayCheckParams, callback, params)
      ...
      console.info('pay', {
        uid: this.uid,
        username: this.username,
        productCode: result.data?.ext?.productCode || '',
        cpOrderNo: result.data?.ext?.cpOrderNo || '',
        amount: result.data?.ext?.amount || '',
        subject: result.data?.ext?.subject || '',
        desc: result.data?.ext?.desc || '',
        goodsId: result.data?.ext?.goodsId || '',
  ...
  private rewardedVideoAd({ complete }: H5MethodParams) {
    QuickSDK.callApp('getRewardVideo', (res: any) => {
      if (res.errorCode === 0) {
        complete({
          code: 0,
          data: null,
