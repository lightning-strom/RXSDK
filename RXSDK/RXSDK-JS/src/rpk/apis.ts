import requestBaidu from './requestBaidu'
import requestAlipay from './requestAlipay'
import requestTaobao from './requestTaobao'
import requestKs from './requestKs'
import requestBilibili from './requestBilibili'
import requestJd from './requestJd'
import requestDouyin from './requestDouyin'
import requestGamebox from './requestGamebox'
import requestMetuan from './requestMetuan'
import { doRequest as requestAxios } from './requestAxios'

let request: any
const isAxiosImportWhiteChannel = process.env.TYPE === 'douyin'

switch (process.env.TYPE) {
  case 'baidu':
    request = requestBaidu
    break
  case 'alipay':
    request = requestAlipay
    break
  case 'taobao':
    request = requestTaobao
    break
  case 'ks':
    request = requestKs
    break
  case 'bilibili':
    request = requestBilibili
    break
  case 'jd':
    request = requestJd
    break
  case 'douyin':
    request = requestDouyin
    break
  case 'gamebox':
    request = requestGamebox
    break
  case '4399':
    request = requestGamebox
    break
  case 'meituan':
    request = requestMetuan
    break
  case 'mgtv':
    if (!isAxiosImportWhiteChannel) request = requestAxios
    break
  case 'facebook':
    if (!isAxiosImportWhiteChannel) request = requestAxios
    break
...
function getURLPath(fullURL: string) {
  const regex = /^https?:\/\/[^\/]+(\/[^?#]*)/
  const match = fullURL.match(regex)
  return match ? match[1] : fullURL
}

export const postAlipayPayData = (_url: string, data: any) => {
  const url = getURLPath(_url)
  return request({
    method: 'POST',
    url: url,
    data
...
// 绑定群组
export function bindGroupApi(data: any): Promise<any> {
  return request({
...
// 解绑群组
export function unBindGroupApi(data: any): Promise<any> {
  return request({
...
// 设置接跳转url并获取短链接
export function getShortUrlApi(data: any): Promise<any> {
  return request({
    url: '/v1/url/short',
    method: 'POST',
    data
...
// 根据三方计费点获取瑞雪计费点
export function getThirdGoodsTagApi(params: any): Promise<any> {
  return request({
...
export const postDouyinPayData = (url: string, data: any) => {
  return request({
    method: 'POST',
    url: url,
    data
...
// 上报订阅消息
export const requestSubscribeMessageApi = (data: any): Promise<any> =>
  request({
...
export const getIpApi = (): Promise<any> =>
  request({
    url: '/',
    method: 'GET'
  }) as any
...
export const getGiftReceiveRewardApi = (data: any): Promise<any> =>
  request({
...
// 美团上报
export const mtReport = (data: any): Promise<any> =>
  request({
