...
const getHeaders = (path: string) => {
  ...
  if (!accessWhiteSpace.includes(path)) {
    // @ts-ignore
    Reflect.set(header, 'ruixue-accesstoken', rxToken?.access || '')
  }
  if (path == '/v1/passport/token/refresh') {
    console.log('refresh')
    header['ruixue-datacount'] = '1'
    // @ts-ignore
    header['ruixue-refreshtoken'] = rxToken?.refresh
  }
  ...
  if (checkNeedAesEncrypt(path)) {
    ...
    header['ruixue-version'] = SYSTEM_INFO.__RX_SDK_VERSION
    header['ruixue-platformid'] = '4'
  }

  if (SYSTEM_INFO.region_tag) {
    header['ruixue-region'] = `${SYSTEM_INFO.region_tag}`
  }

  if (SYSTEM_INFO.cp_role_id) {
    header['ruixue-cp-role-id'] = `${SYSTEM_INFO.cp_role_id}`
  }
    ...
    mgtv.request({
      ...
      success: (res: any) => {
        if ([302015, 302016].includes(res.data?.code)) {
          ...
          trackDecrypt(options, res, 'mgtv', key)
          retryRequest(options, resolve, reject)
        } else {
...
async function doRequest(options: any, urlIndex = 0, refreshNum = 0, enableHttpDNS = false): Promise<any> {
  ...
  const header: any = getHeaders(path)
