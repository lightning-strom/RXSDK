...
class SdkH5Iqiyi extends SdkCommon {
  // 默认刷新时间 10 分钟
  ...
  // 登录必要参数
  private loginMandatory = ['user_id', 'agent', 'time', 'sign']
  ...
  private async login(params: H5IQiYiLoginParam, callback: H5MethodParams) {
    try {
      this.initUrlParams = findChannelCodeInHierarchy(this.loginMandatory)
      ...
      const requestParams: any = {
        ts: now,
        method: params.method || 'minigame_iqiyi',
        distinct_id,
        ext: params.ext,
        ...user_source
      }
  ...
  // 支付
  public async pay(params: H5IQiYiPayParam, callback: H5MethodParams) {
    try {
      await pubCheck(H5IQiYiPayCheckParams, callback, params)
      ...
      let order_amount = Number(result?.data?.ext?.money) / 100 || 0
      let postMessageParams = `game_id=${result?.data?.ext?.game_id}&user_id=${result?.data?.ext?.user_id}&server_id=${result?.data?.ext?.server_id}&money=${order_amount}&extra_param=${result.data?.ext?.extra_param}`
  ...
  public async share(callback: IMethodParams) {
    try {
      // 必须投递固定参数至以下域名（如有报错现象忽略，其中一个域名必定有效）
      var msg = {position:"game_share",data: "show" };
      
      // 游戏中可能有多层级嵌套，寻找最上级
      try {
        if(window?.top){
          window?.top?.postMessage(msg ,'http://togame.pps.tv');
          window?.top?.postMessage(msg ,'http://togame.iqiyi.com');
          window?.top?.postMessage(msg ,'http://playgame.pps.tv');
          window?.top?.postMessage(msg ,'http://playgame.iqiyi.com');
          window?.top?.postMessage(msg ,'http://playgame2.iqiyi.com');
        } else {
