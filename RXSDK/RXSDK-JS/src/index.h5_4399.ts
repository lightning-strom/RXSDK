...
const PLATFORM = 'minigame_4399h5'
...
class SdkH54399 extends SdkCommon {
  // 默认刷新时间 10 分钟
  ...
  constructor(initParams: InitH5Params) {
    ...
    if (window.H5API) {
      ...
      this.loadScript('https://cdn.h5wan.4399sj.com/h5web-frame/dist/static/js/api.js')
        .then(() => {
          this.getInitConfig({ complete: initParams.complete })
        })
        .catch((err: any) => {
  ...
  private async login(params: H54399LoginParam, callback: H5MethodParams) {
    try {
      ...
      const requestParams: any = {
        ts: now,
        method: params.method || 'minigame_4399h5',
        distinct_id,
        ext: params.ext,
        ...user_source
      }
      ...
      if (params.login_openid) {
        ...
        /**
         * 获得玩家头像地址，高宽为120*120像素
         * @param {String} uid 玩家在4399平台的ID
         */
          // const avatar = window.H5API.getUserAvatar(login_info.gameId)
        const { custom_ext, ...rest_ext } = reqLogin.ext || {}
        reqLogin.custom_ext = custom_ext || {}
        reqLogin.ext = {
          ...(rest_ext || {}),
          gameId: login_info.gameId,
  ...
  public async pay(params: H54399PayParam, callback: H5MethodParams) {
    ...
    try {
      await pubCheck(H54399PayCheckParams, callback, params)
      ...
      window.H5API.openPay(Math.floor(result.data.price / 100), result.data.order_no, result.data.ext?.server, result.data.notify_url)
  ...
  private rewardedVideoAd({ complete }: H5MethodParams) {
     ...
     * @param callback 回调函数
     */
    window.H5API.callPlayAd(function(callData: { canPlayAd: boolean, remain: number }) {
      if (callData.canPlayAd) {
        window.H5API.playAd(function(data: { code: number, message: string }) {
        ...
        complete({
          code: -2,
          data: null,
          msg: `不可播放广告，剩余次数${callData.remain}`,
          isEnded: true
        })
      }
    })
