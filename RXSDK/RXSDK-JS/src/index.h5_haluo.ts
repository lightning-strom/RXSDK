...
const PLATFORM = 'minigame_haluo'

declare global {
  var ymyReleaseSdk: any
}
...
class SdkH5Haluo extends SdkCommon {
  // 默认刷新时间 10 分钟
  ...
  // 登录
  private async login(params: H5HaluoLoginParam, callback: H5MethodParams) {
    try {
      ...
      const requestParams: any = {
        ts: now,
        method: params.method || 'minigame_haluo',
        distinct_id,
        ext: params.ext,
        ...user_source
      }
      ...
      if (params.login_openid) {
        ...
        this.openId = await ymyReleaseSdk.login()
  ...
  // 支付
  public async pay(params: H5HaluoPayParam, callback: H5MethodParams) {
    try {
      await pubCheck(H5HaluoPayCheckParams, callback, params)
      ...
      ymyReleaseSdk.pay({
        goodsInfo: {
          itemId: result?.data?.ext?.goodsInfo?.itemId,// 游戏方商品id
          unitPrice: result?.data?.ext?.goodsInfo?.unitPrice,// 游戏方商品价格（分）
          itemName: result?.data?.ext?.goodsInfo?.itemName// 商品名称（目前支付宝需传入）
        },
        amount: result?.data?.ext?.amount,// 金额（分）
        saleNum: result?.data?.ext?.saleNum,// 数量, (支付宝传入规则参考下方buyQuantity 因目前系统价格均以分为单位，根据下面价格列表*100传入)
        openId: this.openId,// 用户openId,
        outOrderNo: result?.data?.ext?.outOrderNo,// 外部游戏订单号,
        notifyUrl: result?.data?.ext?.notifyUrl // 支付回调地址,
      }).then((res: any) => {
        //创单成功
        console.log(res)
  ...
  // 广告
  public async startRewardVideo(callback: IMethodParams) {
    try {
  ...
  // 事件上报
  public async eventReport(params: any, callback: IMethodParams) {
    try {
      ymyReleaseSdk.eventReport({
        openId: this.openId,
        eventName: params?.eventName, // 用户创建角色成功
  ...
  // 获取用户来源信息
  public async getchannelInfo(callback: IMethodParams) {
    try {
      const channelInfo = await ymyReleaseSdk.getchannelInfo()
  ...
  // 获取当前环境信息
  public async getEnv(callback: IMethodParams) {
    try {
      ymyReleaseSdk.getEnv({
        success(callData: any) {
  ...
  private async getInitConfig(callback: H5MethodParams) {
    await ymyReleaseSdk.init({
      gameId: SYSTEM_INFO.gameId,
      platform: SYSTEM_INFO.platform || 'JHY_MINI_GAME'
    })
