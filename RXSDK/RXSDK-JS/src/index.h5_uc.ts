...
const PLATFORM = 'minigame_uc'

declare global {
  var uc: any
}

class SdkH5UC extends SdkCommon {
  // 默认刷新时间 10 分钟
  ...
  private checkLogin(): Promise<{ isLogin: any }> {
  ...
  private checkSession(): Promise<{ code: number }> {
  ...
  private async checkSessionAvailable(): Promise<boolean> {
    const checkSessionRes = await this.checkSession()
    return checkSessionRes.code === 0
  }

  private getSetting(): Promise<{ userInfo: any }> {
    return new Promise((resolve, reject) => {
      uc.getSetting({
        success: (data: any) => resolve(data),
        fail: (data: any) => reject(data)
  ...
  private getGuestInfo(): Promise<any> {
    return new Promise((resolve, reject) => {
      uc.getGuestInfo({
        success: (data: any) => resolve(data),
        fail: (data: any) => reject(data)
  ...
  async infoSync(callback: RpkMethodParams) {
    try {
      const login_info = await this.ucLogin()
      let user_info: any = {}
      try {
  ...
  private async login(params: H5UCLoginParam, callback: H5MethodParams) {
    try {
      ...
      if (params.login_openid) {
        ...
        try {
          ...
          reqLogin.ext.nickname = user_info.nickname
          reqLogin.ext.avatar = user_info.avatar_url
        } catch (e) {
          console.log(e)
          try {
            const user_info = await this.getGuestInfo()
            reqLogin.ext.nickname = user_info.nickname
            reqLogin.ext.avatar = user_info.avatar_url
          } catch (e) {
            console.log(e)
  ...
  //uc支付
  public async pay(params: H5UCPayParam, callback: H5MethodParams) {
    ...
    try {
      const { platform }: any = getUCSystemInfoSync()
      await pubCheck(H5UCPayCheckParams, callback, params)
      ...
      await asyncFunc(uc.requestPayment, {
        biz_id: result.data?.ext?.pay_biz_id,
        token: result.data?.ext?.token,
  ...
  private share(params: H5UCGetShareData, callback: H5MethodParams) {
    uc.shareAppMessage({
      query: params.query || '', // 查询字符串，必须是 key1=val1&key2=val2 的格式。
      target: params.target || '', // wechat:微信好友，qq: qq好友，不设置的话会调起分享面板
      success: (res: any) => {
        callback.complete({
          code: 0,
          data: res
  ...
  private rewardedVideoAd({ complete, fail }: H5MethodParams) {
    ...
    this._rewardedVideoAd.onClose((res: any) => {
      clearTimeout(this.reward_ad_timer)
      this.reward_ad_timer = setTimeout(() => {
        if (res && res.isEnded) {
          complete({
            code: 0,
            data: null,
  ...
  public async getShareData(
    ...
    try {
      ...
      return handleTrackError(PLATFORM, 'rxlog_error_share', err)
    }
  }
