...
import {
  ...
  ShareCheckParams,
  haoyoukuaibaoLoginParamsCheck,
  haoyoukuaibaoPayCheckParams
...
const PLATFORM = 'minigame_haoyou'

declare global {
  var gamebox: any
}

class SdkGameBox extends SdkCommon {
  ...
  public async login(params: RpkGameBoxLogin, callback: RpkMethodParams) {
    try {
      await pubCheck(haoyoukuaibaoLoginParamsCheck, callback, params)
  ...
  public async pay(params: any, callback: RpkMethodParams) {
    ...
    try {
      await pubCheck(haoyoukuaibaoPayCheckParams, callback, params)
      ...
      await asyncFunc(gamebox.requestMgPayment, result.data?.ext)
  ...
  // 激励广告
  public async rewardedVideoAd({ complete, fail }: RpkMethodParams) {
