...
class SdkH5QQ extends SdkCommon {
  // 默认刷新时间 10 分钟
  ...
  constructor(initParams: InitH5Params) {
    ...
    if (window.QQSY_SDK) {
      ...
      this.loadScript(`https://app.77sy.com.cn/static/sdk/77sy_sdk.min.js?t=${Date.now()}`)
        .then(() => {
          this.getInitConfig({ complete: initParams.complete })
        })
        .catch((err: any) => {
  ...
  private getUserInfo(callback?: H5MethodParams) {
    QQSY_SDK.getUserInfo((data: any) => {
      callback?.complete(data)
    })
  }

  private qqLogin(): Promise<any> {
  ...
  private async login(params: H5QiqiLoginParam, callback: H5MethodParams) {
    try {
      ...
      const requestParams: any = {
        ts: now,
        method: params.method || 'minigame_77',
        distinct_id,
        ext: params.ext,
        ...user_source
      }
      ...
      if (params.login_openid) {
        ...
        const login_info: any = await this.qqLogin()
  ...
  public async pay(params: H5QiqiPayParam, callback: H5MethodParams) {
    ...
    try {
      await pubCheck(H5QiqiPayCheckParams, callback, params)
