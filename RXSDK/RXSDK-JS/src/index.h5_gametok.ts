...
class SdkH5Gametok extends SdkCommon {
  // 默认刷新时间 10 分钟
  ...
  // 登录必要参数
  private loginMandatory = ['gameId', 'token']
  ...
  private async login(params: H5GametokLoginParam, callback: H5MethodParams) {
    try {
      ...
      const requestParams: any = {
        ts: now,
        method: params.method || 'gametokh5',
        distinct_id,
        ext: params.ext,
        ...user_source
      }
  ...
  // 支付
  public async pay(params: H5GametokPayParam, callback: H5MethodParams) {
    try {
     
      await pubCheck(H5GametokPayCheckParams, callback, params)
      ...
      if(result?.data?.ext?.status === 'success') {
        callback.complete({ code: 0, data: result?.data, msg: '支付成功' })
      } else {
  ...
  private roleReport(params: any, callback: H5MethodParams) {
    try {
      GameTokSDK.addScore({ score: params?.score || 0, scoreType: params?.scoreType || '',remark:params?.remark || '' })
        .then(() => {
