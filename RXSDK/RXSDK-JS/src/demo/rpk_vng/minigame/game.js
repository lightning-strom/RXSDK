...
import { installCallJsRXBridge } from './rx-calljs-adapter';
function resolveLegacyCtor(mod) {
    const candidates = [
      mod?.channelSDK,
      mod?.default?.channelSDK,
      mod?.default,
      mod,
    ];
    return candidates.find(
      (c) => typeof c === 'function' && typeof c?.prototype?.login === 'function'
    ) || null;
  }

  async function loadVNGGamesSDK(unityInstance) {
    let legacyCtor = null;

    try {
      const legacyMod = await import('./libs/channel-sdk-legacy.js');
      legacyCtor = resolveLegacyCtor(legacyMod) || globalThis.channelSDK || null;
      if (typeof legacyCtor === 'function') {
        globalThis.channelSDK = legacyCtor;
        if (typeof window !== 'undefined') window.channelSDK = legacyCtor;
        console.log('[AUTOCHK] legacy channelSDK ready');
      } else {
        console.warn('[AUTOCHK] legacy loaded but ctor not found');
      }
    } catch (e) {
      console.warn('[AUTOCHK] legacy channel sdk load failed:', e);
    }

    const sdk = await import('./libs/vnggamessdk-mini-v4.js');

    const maybeCtor = globalThis.channelSDK || resolveLegacyCtor(sdk);
    if (typeof maybeCtor === 'function' && typeof maybeCtor?.prototype?.login === 'function') {
      globalThis.channelSDK = maybeCtor;
      if (typeof window !== 'undefined') window.channelSDK = maybeCtor;
    }

    GameGlobal.unityInstance = unityInstance;
    if (typeof window !== 'undefined') window.unityInstance = unityInstance;
  }
const managerConfig = {
    DATA_FILE_MD5: '35afe6b5595acb1e',
    CODE_FILE_MD5: 'f377c09b1ee92702',
    GAME_NAME: 'webgl',
    APPID: '',
    DATA_FILE_SIZE: '26780606',
    DATA_CDN: 'https://uwa-public.oss-cn-beijing.aliyuncs.com/uwa_webgl/buyu/vngnewsdk',
...
checkVersion().then((enable) => {
    if (enable) {
        ...
        gameManager.onModulePrepared(() => {
            ...
            const ui = gameManager.gameInstance;
            // 关键：把同一个实例挂到全局（框架读这个名字）
            globalThis.unityInstance = ui;
            GameGlobal.unityInstance = ui;
            if (typeof window !== 'undefined') window.unityInstance = ui;

            // 关键：先装 CallJsRX，再加载 SDK
            installCallJsRXBridge();
            loadVNGGamesSDK(ui);
        });
