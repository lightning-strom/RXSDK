...
import fetchAdapter from './requestFetchAdapter'
...
const getHeaders = (path: string) => {
  ...
  let headers: any = {
    ['ruixue-language']: SYSTEM_INFO.language || 'zh-CN',
  ...
  if (SYSTEM_INFO.third_channel_code) {
    headers['ruixue-third-channel'] = `${SYSTEM_INFO.third_channel_code}`
  }
...
const requestAxios = axios.create({
  ...
  ...(process.env.TYPE === 'h5_vng' ? { adapter: fetchAdapter } : {})
})
