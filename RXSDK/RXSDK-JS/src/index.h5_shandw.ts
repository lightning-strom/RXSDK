...
// 获取url参数
function findChannelCodeInHierarchy(params: any, optional: string[] = []) {
  let currentWindow: any = window
  let channelMandatory = params
...
// 获取必要参数的url参数
function getChannelCodeFromUrl(url: string, channelMandatory: string[], optional: string[] = []) {
  if (!url) return null
  try {
    ...
    // 检查是否包含所有必需的参数
    const hasAllMandatoryParams = channelMandatory.every(param => params.has(param))
    
    if (hasAllMandatoryParams) {
      // 如果包含所有必需参数，返回完整的参数对象
      const channelData: any = {}
      const channelParamsData = [...channelMandatory, ...optional]
      channelParamsData.forEach(param => {
        channelData[param] = params.get(param)
      })
      return channelData
    }
...
class SdkH5Shandw extends SdkCommon {
  // 默认刷新时间 10 分钟
  ...
  // 初始化必要参数
  private mandatoryParams = ['uid', 'channel', 'appid']
  // 初始化选填参数
  private optionalParams = ['openid', 'nick', 'avatar', 'sex', 'time', 'reurl', 'cburl', 'paydata', 'sign']

  // 支付必要参数
  private payMandatory = ['subject', 'appId', 'gameName', 'accountId', 'amount', 'cpOrderId', 'call_back_url', 'merchant_url', 'sign', 'timestamp', 'channel', 'wxopenid']
  // 支付选填参数
  private payOptional = ['paychannel', 'complete']

  // 支付url参数
  private payUrlParams: any = {}
  // 初始化url参数
  private initUrlParams: any = {}
  ...
  // 登录
  private async login(params: H5ShandwLoginParam, callback: H5MethodParams) {
    try {
      this.initUrlParams = findChannelCodeInHierarchy(this.mandatoryParams, this.optionalParams)
      ...
      const requestParams: any = {
        ts: now,
        method: params.method || 'minigame_shandw',
        distinct_id,
        ext: params.ext,
        ...user_source
      }
  ...
  // 支付
  public async pay(params: H5ShandwPayParam, callback: H5MethodParams) {
    try {
      // this.payUrlParams = findChannelCodeInHierarchy(this.payMandatory, this.payOptional)
      await pubCheck(H5ShandwPayCheckParams, callback, params)
      ...
      let reqOrder = {
        ...
        ext: {
          ...params.ext,
          channel: this.initUrlParams?.channel || '0',
          call_back_url: this.initUrlParams?.cburl || '',
          merchant_url: this.initUrlParams?.cburl || '',
          paychannel: params?.pay_channel || ''
        }
      }
      const result = await orderApi(reqOrder)
      sdw.chooseSDWPay({
        accountId: result?.data?.ext?.accountId, // 游戏账号uid
        amount: result?.data?.ext?.amount, // 支付金额（单位：分）
        appId: result?.data?.ext?.appId, // 游戏ID
        cpOrderId: result?.data?.ext?.cpOrderId, // 游戏自己生成的预订单号（唯一仅字母|数字组合，不超过50位）
        call_back_url: result?.data?.ext?.call_back_url, // 支付成功跳转页面
        merchant_url: result?.data?.ext?.merchant_url, // 操作中断跳转地址
        subject: result?.data?.ext?.subject, // 商品描述
        timestamp: result?.data?.ext?.timestamp, // 时间戳，仅允许1小时内的支付请求，精确到秒
        sign: result?.data?.ext?.sign, // 签名信息，校验值
        gameName: result?.data?.ext?.gameName, // 游戏名称
        wxopenid: result?.data?.ext?.wxopenid || '', // 非微信APP环境为""，微信环境调起支付必填

        // 后端不反，需要前端在url获取 251226
        channel: this.initUrlParams?.channel || '0', // 渠道ID
        paychannel: params?.pay_channel || '', // 支付方式，默认为""，"alipay":调取支付宝支付，"weixin":调取微信支付
        complete: (data: any) => {
          // 处理支付回调
  ...
  // 关闭支付窗口
  public async closePay() {
    sdw.closeSDWPay()
  }

  // 分享
  public async share(params: any, callback: IMethodParams) {
    try {
  ...
  // 实名认证
  public async chooseSDWIdentify(params: any, callback: IMethodParams) {
    try {
      sdw.isSDWIdentify(function(res: any){
        console.log(res)
        if(res.result==1){//已经实名
          callback.complete({ code: 1, data: res, msg: '该用户已经实名认证' })
        }else{//未实名
          sdw.chooseSDWIdentify(function(res: any){
  ...
  // 上报游戏基础信息
  public async roleReport(params: any, callback: IMethodParams) {
    try {
      console.log('sdk reportData:', params)
      const paramsData = {
        ...this.initUrlParams,
        id: params?.userRoleId,
        sid: params?.serverId || '0',
        sname: params?.serverName,
        nick: params?.userRoleName,
        level: params?.userRoleLevel || 0,
        type: params?.gameType,
        vip: params?.vipLevel || 0,
        power: params?.gameRolePower || 0,
        new: params?.createRole || 0
      }
      const res = await channelGatewayAuthApi({paramsData})
      sdw.postGameInfo({
        ...paramsData,
        cpSign: res?.data?.cpSign
      })
  ...
  private rewardedVideoAd({ complete }: H5MethodParams) {
    ...
    sdw.startRewardVideo({
      playSuccess: function(res: any) {
        console.log('播放成功', res)
      },
      playComplete: function(res: any) {
        console.log('播放完成', res)
      },
      close: function(res: any) {
        console.log('关闭视频', res)
      },
      playFail: function(res: any) {
