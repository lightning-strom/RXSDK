...
import {
  ...
  H5ShareCheckParams, H5AwyPayCheckParams
} from '@/h5/checkConfig'
import {
  getPublicProps,
  activated,
...
const PLATFORM = 'minigame_aiweiyou'
...
class SdkH5Awy extends SdkCommon {
  // 默认刷新时间 10 分钟
  ...
  constructor(initParams: InitH5Params) {
    ...
    if (window.AWY_SDK) {
      ...
      this.loadScript('https://cdn.11h5.com/static/js/sdk.min.js')
        .then(() => {
          this.initAwySdk()
          this.getInitConfig({ complete: initParams.complete })
        })
  ...
  private handleSdkPayCallback: any

  private handleSdkShareCallback: any

  private initAwySdk() {
    window.AWY_SDK.config(SYSTEM_INFO.gameId, () => {
      if (typeof this.handleSdkShareCallback == 'function') {
        this.handleSdkShareCallback({
          code: 0,
          msg: '分享成功'
        })
        this.handleSdkShareCallback = null
      }
    })
  }

  private async login(params: H5AwyLoginParam, callback: H5MethodParams) {
    try {
  ...
  // 支付
  public async pay(params: H5AwyPayParam, callback: H5MethodParams) {
    ...
    try {
      await pubCheck(H5AwyPayCheckParams, callback, params)
      ...
      window.AWY_SDK.pay({
        pid: result?.data?.ext.pid,
        serverid: result?.data?.ext.serverid,
        txid: result.data.order_no,
        userdata: result?.data?.ext.userdata,
        product_count: result?.data?.ext.product_count
      })
  ...
  private share(params: H5AwyShareData, callback: H5MethodParams) {
    if (params.query) {
      try {
