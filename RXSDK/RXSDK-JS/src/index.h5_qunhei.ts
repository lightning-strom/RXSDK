...
const PLATFORM = 'minigame_qunhei'
...
class SdkQunhei extends SdkCommon {
  // 默认刷新时间 10 分钟
  ...
  constructor(initParams: InitH5Params) {
    ...
    if (window.qhsdk) {
      ...
      this.loadScript('https:////port.game3.cn/game/qhjssdk')
        .then(() => {
          this.getInitConfig({ complete: initParams.complete })
        })
        .catch((err: any) => {
  ...
  private async login(params: H5QunheiLoginParam, callback: H5MethodParams) {
    try {
      ...
      const requestParams: any = {
        ts: now,
        method: params.method || 'minigame_qunhei',
        distinct_id,
        ext: params.ext,
        ...user_source
      }
      ...
      if (params.login_openid) {
        ...
        reqLogin.ext = {
          ...
          server_id: login_info.serverid,
          time: login_info.time,
          isadult: login_info.isadult,
          uimg: login_info.uimg,
          uname: login_info.nname,
          flag: login_info.flag
        }
  ...
  public async pay(params: H5QunheiPayParam, callback: H5MethodParams) {
    ...
    try {
      await pubCheck(H5QunheiPayCheckParams, callback, params)
  ...
  // 实名验证接口
  private userVerify(type: 1 | 2 | 3, callback: H5MethodParams) {
    qhsdk.userVerify(type, (res: any) => {
      callback.complete(res)
    })
  }

  // 微信关注接口
  private wechatFollow(callback: H5MethodParams) {
    qhsdk.iswxgz((res: any) => {
      callback.complete({
        code: res.code,
  ...
  // 提交开服接口
  private serverAdd(param: H5QunheiServerAddParam, callback: H5MethodParams) {

  }

  // 角色上下线上报
  private upOnline(param: H5QunheiUpOnlineParam, callback: H5MethodParams) {

  }

  // 角色上报接口
  private upRole(param: H5QunheiUpRoleParam, callback: H5MethodParams) {
    qhsdk.role(param)
  }
  ...
  // 平台敏感词检测接口
  private checkWord(words: string, callback: H5MethodParams) {
    console.log('words:', words)
    qhsdk.checkWord(words, (res: any) => {
  ...
  private handleSdkShareCallback: any

  private async getInitConfig(callback: H5MethodParams) {
    const init_info: any = getSearchQueries()
    qhsdk.init({
