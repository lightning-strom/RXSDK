...
class SdkH5Baidu extends SdkCommon {
  // 默认刷新时间 10 分钟
  ...
  private talkType: {
    ...
    RAISEPAY: string
      ...
      SECRET: 'secret', //口令传递
      POP: 'popwindow', //唤起百度充值弹窗，传输数据
      RAISEPAY: 'raisepay' //唤起支付
  }
  // 口令传递
  private secretData: any = {}
  // 登录必要参数
  private loginMandatory = ['api_key', 'user_id', 'server_id', 'cm_flag', 'timestamp', 'sign']
  ...
  constructor(initParams: InitH5Params) {
    ...
    this.initBaiduPayMessageListener()
   ...
   */
  private initBaiduPayMessageListener() {
    this.payMessageHandler = (event: MessageEvent) => {
      try {
        console.log(event,'data==========>')
        // 口令传递
        if(event?.data?.action === this?.talkType?.SECRET) {
          this.secretData = event.data
        }
      } catch (error) {
  ...
  private async login(params: H5NewBaiduLoginParam, callback: H5MethodParams) {
    try {
  ...
  // 支付
  public async pay(params: H5NewBaiduPayParam, callback: H5MethodParams) {
    try {
      await pubCheck(H5NewBaiduPayCheckParams, callback, params)
      ...
      // 组装postMessage参数
      const postData = {
        token: this.secretData?.token || '945a7c1579a8aad460a5dcb7bfee927a', // token
        action: this.talkType?.POP, // action
        data: {
            gameId: params?.gameId || '', //游戏ID
            cpServerId: 1, //区服编号
            activeAccount: result?.data?.ext?.amount, //充值金额，单位元，整数数字，暂只支持到元
            roleId: result?.data?.ext?.order_no, //角色ID，非必填
            cpExtra: `order_no=${result?.data?.ext?.order_no}` //补充信息，非必填。在支付完成回调时，将以cp_extra字段原样返回
          }
      }
      // 发送postMessage
