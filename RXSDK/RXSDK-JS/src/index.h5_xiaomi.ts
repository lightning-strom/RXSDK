...
const PLATFORM = 'minigame_xiaomi'

declare global {
  var qg: any
}

class SdkH5Xiaomi extends SdkCommon {
  ...
  private async login(params: H5XiaomiLoginParam, callback: H5MethodParams) {
    try {
      ...
      const loginResult: any = await asyncFunc(qg.login, {})
      try {
        userInfoResult = await asyncFunc(qg.getUserInfo, {})
      } catch (e: any) {
  ...
  // 支付
  public async pay(params: H5XiaomiPayParam, callback: H5MethodParams) {
    ...
    try {
      ...
      let reqOrder = {
        ...
        ext: {
          ...
          'appAccountId': `${customGetStorageSync('rx_xiaomi_login_app')}`
        }
      }
      ...
      const payResult = await asyncFunc(qg.pay, {
        orderInfo: {
          ...
          appAccountId: result.data?.ext?.appAccountId,
          session: result.data?.ext?.session,
          cpOrderId: result.data?.ext?.cpOrderId,
          cpUserInfo: result.data?.ext?.cpUserInfo,
          displayName: result.data?.ext?.displayName,
          feeValue: result.data?.ext?.feeValue,
  ...
  // 激励广告
  public async rewardedVideoAd(data: any, { complete, fail }: H5MethodParams) {
    let adShareData: any = {}
    ...
    if (!this._rewardedVideoAd) {
      ...
      const onError: any = async (err: any) => {
        ...
        // 激励视频广告Error事件
        console.log(err)
        const _error = handleTrackError(PLATFORM, 'rxlog_error_ad', err)
        complete(_error)
        fail && fail(_error)
      }
    ...
    let p = this._rewardedVideoAd.show()
  ...
  // 保存桌面 API
  public saveToDestop(message: string | null, callback: IMethodParams) {
    qg.getShortcut().hasInstalled({
      success: (ret: any) => {
        if (!ret) {
          qg.getShortcut().install({
            message: message,
            success: (ret: any | null) => {
              if (callback) {
                callback.complete({
                  code: 0,
                  data: null,
  ...
  public exit(callback: IMethodParams) {
    qg.exitApplication({
      success: (ret: any) => {
        if (callback) {
          callback.complete({
