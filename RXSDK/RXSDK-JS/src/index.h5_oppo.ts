...
import {
  ...
  getAdShareDataApi,
  refreshUserInfo,
  channelGatewayAuthApiOppo
} from '@/h5/apis'
import SdkCommon from './h5/SdkCommon'
import { setupStOffsetRefreshForH5 } from '@/utils/stOffset'

const PLATFORM = 'minigame_oppo'

declare global {
  var qg: any
}

class SdkH5Oppo extends SdkCommon {
  private minPlatformVersion = null
  private pkgName = null
  private versionCode = null
  private platformVersionCode = null
  ...
  constructor(initParams: InitH5Params) {
    ...
    qg.getManifestInfo({
      success: (res: any) => {
        const manifest: any = JSON.parse(res.manifest)
        this.minPlatformVersion = manifest.minPlatformVersion
        this.pkgName = manifest.package
    ...
    qg.getSystemInfo({
      success: (res: any) => {
        this.platformVersionCode = res.platformVersionCode
      }
    })
  }

  private async login(params: H5OppoLoginParam, callback: H5MethodParams) {
    try {
  ...
  // 支付
  public async pay(params: H5OppoPayParam, callback: H5MethodParams) {
    try {
  ...
  // 调用渠道接口(需要登录) OPPO
  public async channelGatewayAuthApiOppo(params: any, callback: H5MethodParams) {
    try {
      const res = await channelGatewayAuthApiOppo({...params})
  ...
  //同步用户信息
  async infoSync(callback: H5MethodParams) {
    try {
      const loginResult: any = await asyncFunc(qg.login, {})
