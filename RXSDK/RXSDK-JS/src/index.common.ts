...
import {
  ...
  changePhoneParamsCheck,
  sendCaptchaParamsCheck,
  unbindemailParamsCheck,
  unBindPhoneParamsCheck,
  verifyCodeParamsCheck
} from '@/utils/checkConfig/common'
import { handleError, asyncFunc } from '@/utils/utils'
import { checkActivityVersionParams, checkAppVersionParams, checkGameVersionParams } from '@/utils/checkConfig'
...
class SdkCommon {
  ...
  public async sendCaptcha(params: IsendCaptcha, callback: IMethodParams) {
    try {
      if(params.purpose != 'unbindphone') {
        await pubCheck(sendCaptchaParamsCheck, callback, params)
      }
      if(params.auth) {
        let loginResult = await asyncFunc(wx.login)
        let {auth, ..._params} = params
        let result = await sendCaptchaWithCode({
          ..._params,
          minigame_code: loginResult.code
        })
  ...
  public async validateUnbindCode(params: any, callback: IMethodParams) {
    try {
      await pubCheck(verifyCodeParamsCheck, callback, params)
      let result = await validateUnbindCodeApi(params)
      callback.complete(result)
    } catch (error) {
      callback.complete(handleError(error))
    }
  ...
  public async changePhone(params: any, callback: IMethodParams) {
    try {
      await pubCheck(changePhoneParamsCheck, callback, params)
