...
const PLATFORM = 'minigame_4399'

declare global {
  var gamebox: any
}

class SdkGameBox4399 extends SdkCommon {
  ...
  private setGameId(data: { user_flag: number, cp_user_id: string }) {
    try {
      if ((data?.user_flag & 1) == 1) {
        this.isPromoter = true
        this.game_id = data?.cp_user_id
      }
    } catch (e) {
      console.log(e)
    }
  ...
  async infoSync(callback: RpkMethodParams) {
    try {
      const loginResult: any = await asyncFunc(gamebox.login, {})
  ...
  public async login(params: RpkGameBoxLogin, callback: RpkMethodParams) {
    try {
      ...
      if (params.login_openid) {
        ...
        this.setGameId(loginTokenRx?.data)
  ...
  public async pay(params: any, callback: RpkMethodParams) {
    ...
    try {
      ...
      await asyncFunc(gamebox.requestMgPayment, {
        serverId: result.data?.ext?.serverId,
        gameId: result.data?.ext?.gameId,
        commondity: result.data?.ext?.commondity,
        money: result.data?.ext?.money,
        mark: result.data?.ext?.mark,
        supportExcess: result.data?.ext?.supportExcess,
        extra: result.data?.ext?.extra,
        description: result.data?.ext?.description,
      })
  ...
  // 分享
  public async share(params: { type: 0 | 1 }, callback: RpkMethodParams) {
  ...
  // 激励广告
  public async rewardedVideoAd({ complete, fail }: IMethodParams) {
  ...
  private exit(callback: IMethodParams) {
    gamebox.exitMiniProgram({
      complete: function(ret: any | null) {
        if (callback) {
