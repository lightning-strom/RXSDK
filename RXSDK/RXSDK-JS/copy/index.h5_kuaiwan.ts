...
import SdkCommon from '../src/h5/SdkCommon'

const PLATFORM = 'minigame_kuaiwan'
...
class SdkH5Kuaiwan extends SdkCommon {
  // 默认刷新时间 10 分钟
  ...
  constructor(initParams: InitH5Params) {
    ...
    if (window.KKW_SDK) {
      ...
      this.loadScript(`https://resource.kuaikuaiwan.com/Resource/UserApp/js/sdk/kkw_sdk.min.js?t=${Date.now()}`)
        .then(() => {
          window.addEventListener('message', (event) => {
            switch (event.data.cmd) {
              case 'kkw_jssdk_finished': {
                this.getInitConfig({ complete: initParams.complete })
                break
              }
            }
  ...
  private getUserInfo(callback?: RpkMethodParams) {
    KKW_SDK.getUserInfo((data: any) => {
      callback?.complete(data)
    })
  }

  private kwLogin(): Promise<any> {
  ...
  private async login(params: H5XunleiLoginParam, callback: H5MethodParams) {
    try {
      ...
      const requestParams: any = {
        ts: now,
        method: params.method || 'minigame_kuaiwan',
        distinct_id,
        ext: params.ext,
        ...user_source
      }
      ...
      if (params.login_openid) {
        ...
        const login_info: any = await this.kwLogin()
        const { custom_ext, ...rest_ext } = reqLogin.ext || {}
        reqLogin.custom_ext = custom_ext || {}
        reqLogin.ext = {
          ...(rest_ext || {}),
          uid: login_info.uid,
          realname: login_info.realname,
          idcard: login_info.idcard,
  ...
  public async pay(params: H5XunleiPayParam, callback: H5MethodParams) {
    try {
      await pubCheck(H5XunleiPayCheckParams, callback, params)
      ...
      KKW_SDK.pay({
        ...
        uid: result.data?.ext?.uid,
        amount: result.data?.ext?.amount,
        product_id: result.data?.ext?.product_id,
        game_uid: result.data?.ext?.game_uid,
        game_orderno: result.data?.ext?.game_orderno,
        ext: result.data?.ext?.ext || '',
  ...
  private async track(params: trackParams, callback: H5MethodParams) {
    try {
