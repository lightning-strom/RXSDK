...
const PLATFORM = 'minigame_360'

class SdkH5360 extends SdkCommon {
  // 默认刷新时间 10 分钟
  ...
  private async login(params: H5ZuiyouLoginParam, callback: H5MethodParams) {
    try {
      ...
      const requestParams: any = {
        ts: now,
        method: params.method || 'minigame_360',
        distinct_id,
        ext: params.ext,
        ...user_source
      }
      ...
      if (params.login_openid) {
        ...
        reqLogin.ext = {
          ...
          role_not_auto: `${login_info.role_not_auto}`,
  ...
  public async pay(params: H5ZuiyouPayParam, callback: H5MethodParams) {
    try {
      ...
      const query = qs.stringify({
        ...
        platform: result.data?.ext?.platform,
        gkey: result.data?.ext?.gkey,
        skey: result.data?.ext?.skey,
        amount: result.data?.ext?.amount,
        rid: result.data?.ext?.rid,
        pid: result.data?.ext?.pid,
        exts_cp: result.data?.ext?.exts_cp,
        sign: result.data?.ext?.sign
      })
      callback.complete({ code: 0, pay_url: `https://iap.g.360-api.cn/mini.html?${query}` })
    } catch (err: any) {
